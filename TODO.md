...
- [~] Preserve the small Nushell-first surface.
  ...
  - Typed fixed arrays now share the same ordinary aggregate model when the element layout is explicit, including numeric-array predicates/transforms, bool-array `all` / `any` predicates, scalar and counted `first` / `last`, `get`, `length`, compatible `append` / `prepend`, non-empty math reducers, `math median` / `math mode` / `math abs`, integer identity `math ceil` / `math floor` / `math round`, integer bitwise/shift/rotate commands, default metadata-preserving `compact` for annotated fixed-layout arrays with `--empty` limited to numeric/bool arrays, and documented string/binary/numeric-list/record projection subsets.
