...
- [~] Generalize context modeling by program type.
  ...
  - Recent progress: local-storage map kinds are now modeled for `sk_storage`, `inode_storage`, and `task_storage`, and the real storage helpers materialize fixed-kind map FDs from literal map names without requiring a separate magic helper surface.
