...
- [~] Make unsupported-feature diagnostics actionable.
  ...
  - Typed `global-define` record and array initializer rejects now name nested unexpected-field paths such as `inner.extra` and top-level item paths such as `initializer[0]`; malformed nested record type specs now name paths such as `inner.bad`.
