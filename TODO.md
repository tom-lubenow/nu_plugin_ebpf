...
- [~] Replace opaque helper handling with typed helper semantics.
  ...
  - Built-in typed kfunc signature coverage now includes a core `scx_bpf_*` subset (CPU perf/node queries, DSQ management, CPU picking, task-state helpers, task->cgroup lookup, and CPU-selection kfuncs) with shared task/cpumask pointer-family checks.
