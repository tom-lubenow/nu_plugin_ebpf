...
- [~] Make VCC a practical pre-kernel verifier for modeled features.
  ...
  - Add BTF-aware validation for remaining map-value fields that carry verifier-sensitive objects such as dynptrs, graph roots/nodes, and nested kernel structs. Graph list/rbtree root/node kernel floors are source-backed, and BTF declaration-tag emission is available as the prerequisite for `contains:TYPE:FIELD` graph-root metadata; source syntax still needs a named object-type story before exposing list/rbtree roots, and bare graph root/node tokens produce a targeted unsupported-feature diagnostic instead of implying partial support.
...
- [~] Finish resource-backed map semantics.
  ...
  - Keep extending source-level `map-define` only for real map resource metadata. Key/value layouts, natural fixed-record alignment, `--max-entries`, and verifier-managed `bpf_timer`, `bpf_spin_lock`, `bpf_wq`, `bpf_refcount`, and top-level `kptr:TYPE` slots are modeled; dynptrs and graph object schemas are the next natural BTF-backed validation targets. Do not expose `bpf_list_head`, `bpf_rb_root`, `bpf_list_node`, or `bpf_rb_node` as bare field tokens; they need named object schemas that can emit the contained object struct plus `contains:TYPE:FIELD` BTF declaration tags.
