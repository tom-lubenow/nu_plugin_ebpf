...
- [~] Add kernel-version and feature metadata.
  ...
  - All modeled helpers now have source-backed `BpfHelper` kernel floors derived from Linux UAPI helper IDs; current verifier `helper-call`, known `kfunc-call`, and target-aware direct/helper-backed `$ctx.FIELD` fixtures derive their feature metadata from source text; `ebpf spec` exposes backing helper/kfunc metadata for read-side context fields, write-side context surfaces, and helper-backed projections; source-checked fixture kfuncs plus most cpumask/dynptr, task/cgroup/socket/map, iterator, object/list/rbtree, user-copy, crypto, sched_ext, preempt/IRQ, and RCU kfuncs have Rust metadata; tracepoint payload field specs expose whether their layout came from tracefs or the syscall fallback; and live-load preflight checks helper floors recovered from compiled bytecode plus known-kfunc and context-field floors preserved from source lowering, including direct context-write floors that map to read-side fields. Continue tracking remaining kfuncs, tracepoint field version claims, attach modes, and loader features once each value is source-verified.
...
- [~] Strengthen global/static data support.
  ...
  - Record constants and typed globals can already carry nested string, binary, numeric-list, fixed-array, and record fields when their layout is explicit or inferable; fixed-array record elements can carry nested numeric-list fields, while string-buffer fields inside fixed-array elements still need a dedicated materialization policy.
