...
- [~] Add kernel-version and feature metadata.
  ...
  - Keep verifier fixture target metadata derived from target strings and source-visible forms where possible; simple direct and helper-backed context fields are derived from target-aware context-parameter field roots, including `$ctx.FIELD`, alternate closure parameter names, simple full-context aliases such as `let event = ($ctx); $event.FIELD`, and simple bound root projections such as `let sk = $ctx.sk; $sk.family` or `let sk = ($ctx.sk); $sk.tcp.snd_cwnd`; typed `map-define --value-type` fields derive map-value floors for `bpf_spin_lock`, `bpf_timer`, `kptr:TYPE`, `bpf_wq`, `bpf_refcount`, and graph roots. Tracepoint-specific context fields, helper-only map arguments whose family is neither fixed nor source-visible, complex first-class helper surfaces the scanner cannot imply, unknown kfuncs, or target-specific requirements should remain explicit fixture `kernel_features`.
