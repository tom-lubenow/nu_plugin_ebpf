...
- [~] Expand program type support beyond probes.
  ...
  - Recent progress: `ProgramSpec` now owns typed target parsing plus section-name/attach input derivation shared by loader, compiler, and attach paths, and helper-call legality now mostly resolves through `EbpfProgramType` plus attach-sensitive `ProbeContext` metadata instead of ad hoc validator matches.
...
- [~] Generalize context modeling by program type.
  ...
  - Recent progress: started moving context legality and aliases behind explicit program-type metadata/helpers so packet/socket/cgroup families no longer rely on a single probe-centric validation match, target-sensitive checks now consult typed `ProgramSpec` parsing instead of open-coded target-string splits or suffix checks, `ctx.sk.*` post-bind gating now lives on `ProbeContext` rather than inside MIR lowering, BTF-backed `ctx.arg*` / `ctx.retval` name/type/spec/field resolution is now centralized on `ProbeContext` instead of being reimplemented across hinting, type inference, lowering, and codegen, sched_ext callback-scoped kfunc legality now also lives on `ProbeContext` instead of type-inference-only matches, per-program return-action aliases now live on `EbpfProgramType` instead of MIR-lowering-local matches, socket family/tuple/type/protocol/ref layout routing now comes from `EbpfProgramType` selectors instead of repeated codegen-side `probe_type` matches, ingress-ifindex plus mark/priority context routing now also comes from `EbpfProgramType` selectors, cgroup attach-shape predicates now live on typed `ProgramSpec` targets instead of open-coded `ProbeContext` attach-type matches, raw-tracepoint arg handling now keys off program-model arg-access metadata instead of explicit `RawTracepoint` name checks, remaining context name/store-target resolution now lives behind `EbpfProgramType` / `ProbeContext` so MIR lowering only parses cell-path shape instead of owning per-program field tables, helper-surface rules that depend only on program type now live on `EbpfProgramType` while attach-sensitive helper checks stay on `ProbeContext`, XDP-specific helper arg rules such as `bpf_redirect` flag constraints now also hang off model metadata rather than type-inference-local `probe_type` checks, struct_ops callback main-function return contracts now resolve through `ProbeContext` instead of a MIR-to-eBPF-local `probe_type` special case, writable context-store target legality now also comes from `ProbeContext` / `CtxStoreTarget` instead of duplicated type-infer and codegen-side matches, and socket-program context modeling now covers `ctx.sk` on `cgroup_sock_addr` / `sock_ops` plus attach-sensitive `bpf_bind` / `bpf_getsockopt` / `bpf_setsockopt` helper legality through the same program model.
