...
- [~] Replace opaque helper handling with typed helper semantics.
  ...
  - Built-in typed kfunc signature coverage now includes a core `scx_bpf_*` subset (CPU perf/node queries, DSQ management including `dsq_move*`, CPU picking, task-state helpers, task->cgroup lookup, CPU-selection kfuncs, and cpumask getter/put helpers) with shared task/cpumask pointer-family checks.
