...
- [~] Add kernel-version and feature metadata.
  ...
  - Common helper floors used by current verifier fixtures and first-class surfaces now have source-backed `BpfHelper` metadata; current verifier `helper-call` and known `kfunc-call` fixtures derive their feature metadata from source text; source-checked fixture kfuncs have Rust metadata; and live-load preflight checks helper floors recovered from compiled bytecode. Continue tracking remaining helpers/kfuncs, context fields, attach modes, loader features, and kfunc name preservation once each value is source-verified.
