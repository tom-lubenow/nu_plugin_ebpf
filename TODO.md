...
- [~] Keep the user-facing surface small and Nushell-first.
  ...
  - Recent progress: ordinary `random int` now lowers to `bpf_get_prandom_u32` for the zero-argument form and compile-time bounded ranges, keeping pseudo-randomness on a Nushell primitive instead of adding a bespoke helper wrapper.
...
- [ ] Add end-to-end non-tracing fixtures.
  ...
  - Recent progress: attach compile fixtures now also cover the `ctx.protocol` skb protocol alias across `socket_filter`, `tc`, `cgroup_skb`, `sk_skb`, and `sk_skb_parser`, plus the `ctx.len` packet-length alias across XDP, skb-backed, and `sk_msg` lowering paths.
