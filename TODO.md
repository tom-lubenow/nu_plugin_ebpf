...
- [ ] Add end-to-end non-tracing fixtures.
  ...
  - Recent progress: the attach compile harness now covers representative compile-only non-tracing families (`xdp`, `tc`, `cgroup_skb`, `cgroup_device`, `cgroup_sysctl`, `sock_ops`, `sk_msg`, `sk_skb`, `sk_skb_parser`, `lirc_mode2`) plus attach-sensitive target variants (`cgroup_sock:post_bind4`, `cgroup_sock:sock_release`, `cgroup_sockopt:set`, `cgroup_sock_addr:connect4`, `cgroup_sock_addr:sendmsg4`) using safe context-field fixtures; it now also exercises direct packet/context reads (`xdp.data[0]`, `xdp.data_meta[0]`, `sk_msg.data[0]`, `cgroup_sock:sock_release.bound_dev_if`, `cgroup_sock:sock_release.priority`), writable `ctx.*` surfaces (`tc.mark`, `tc.data[0]`, `tc.cb[2]`, `cgroup_skb:egress.tstamp`, `cgroup_sock:sock_create.mark`, `cgroup_sock:sock_release.bound_dev_if`, `cgroup_sock:sock_release.priority`, `cgroup_sysctl.file_pos`, `cgroup_sockopt:set.level`, `sk_skb.tc_index`, `sk_skb_parser.priority`, `xdp.data.eth.ethertype`, `xdp.data.eth.ipv6.udp.dst`, `xdp.data_meta[0]`), non-tracing generic-map flows (for example `cgroup_sysctl` queue `map-push`/`map-peek`/`count` and `sock_ops` stack `map-push`/`map-pop`/`count`), first-class helper-heavy paths (`xdp adjust-packet --meta`, `tc redirect --peer`, `sk_msg adjust-message --pull`), loop-heavy control flow (`xdp` bounded numeric list iteration feeding `count` plus descending inclusive range iteration), and branch-sensitive/null-sensitive control-flow fixtures (`sock_ops` packet-aware callback guards around `ctx.packet_len`, `ctx.data[0]`, and `ctx.skb_tcp_flags`, guarded/branch-refined `cgroup_sockopt:get` `ctx.optval` reads and `ctx.optval[0]` writes, and `cgroup_sock_addr:connect4` nullable `ctx.sk` branching with `return early`) through compile-only lowering/codegen coverage without adding risky live attach tests.
