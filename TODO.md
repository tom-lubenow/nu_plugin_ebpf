...
- [~] Make VCC a practical pre-kernel verifier for modeled features.
  ...
  - Add BTF-aware validation for remaining map-value fields that carry verifier-sensitive objects such as nested kernel structs. Dynptr fields are rejected as stack-only verifier objects; source graph root schemas can declare `bpf_list_head:TYPE:FIELD` / `bpf_rb_root:TYPE:FIELD`, emit object BTF plus `contains:TYPE:FIELD` declaration tags, validate roots as top-level aligned fields, project graph roots and `bpf_spin_lock` fields from the same map-value record, and require graph-root kfuncs to run under a held `bpf_spin_lock` from the same map lookup root when provenance is available. Bare graph root/node tokens still produce a targeted unsupported-feature diagnostic instead of implying partial support; richer object payload fields remain future work.
...
- [~] Expand typed helper/kfunc semantics where it materially improves safety.
  ...
  - Map-value `bpf_spin_lock` unlock identity, graph-root kfunc same-map-root lock checks, resource spin-lock same-lock duplicate acquire / ordered unlocks, and active-lock call restrictions are now tracked; richer validation that independently looked-up map pointers with equivalent keys refer to the same concrete entry, and that resource spin-lock pointers actually target a `bpf_res_spin_lock` field, still depends on fuller object/map-value schemas.
