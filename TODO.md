...
- [ ] Expand map support to the broader eBPF map ecosystem.
  ...
  - Recent progress: added `BPF_MAP_TYPE_CGROUP_ARRAY` map definitions plus fixed map-FD materialization for `bpf_skb_under_cgroup` and `bpf_current_task_under_cgroup`, keeping cgroup arrays on the raw helper surface rather than generic map commands.
