...
- [~] Add kernel-version and feature metadata.
  ...
  - Keep verifier fixture target metadata derived from target strings where possible; simple source-backed direct context fields are derived from `$ctx.FIELD` roots, while helper-backed or tracepoint-specific context fields, helper-only map arguments, complex first-class helper surfaces the scanner cannot imply, unknown kfuncs, or target-specific requirements should remain explicit fixture `kernel_features`.
