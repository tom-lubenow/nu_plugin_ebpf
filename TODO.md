...
- [~] Finish resource-backed map semantics.
  ...
  - Finish map-in-map support. Source-level `map-define --kind array-of-maps|hash-of-maps --inner-map INNER` now reserves and validates the inner-template contract against a previously declared inner map, pinned/shared attach metadata preserves unambiguous outer-to-inner templates across peers, and emitted objects include libbpf-compatible `values` BTF metadata when the inner template is also emitted as a runtime map. Live loader `inner_map_fd` materialization, map operation typing, and verifier differential coverage remain pending.
