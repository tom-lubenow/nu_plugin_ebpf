...
- [~] Add kfunc and richer BTF-driven typing support.
  ...
  - Expanded unknown-kfunc name heuristics for stack-object lifecycle transitions (init: `*_init*`/`*_new*`/`*_create*`/`*_alloc*`; destroy: `*_destroy*`/`*_release*`/`*_drop*`/`*_cleanup*`/`*_deinit*`/`*_fini*`), plus release transitions (`*_destroy*`, `*_cleanup*`, `*_deinit*`, `*_fini*`, `*_delete*`, `*_detach*`, `*_close*`, `*_unref*`, `*_dec*`) and out-parameter inference (`dst*` / `*dst`) while still requiring unambiguous BTF pointer-family evidence.
