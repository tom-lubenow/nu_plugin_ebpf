...
- [~] Finish resource-backed map semantics.
  ...
  - Local-storage map fixtures currently type-check locally but fail kernel map creation with `-EINVAL`; fix emitted map/BTF metadata for task, inode, cgroup, and socket storage before promoting them to kernel verifier coverage.
