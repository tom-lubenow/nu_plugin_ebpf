...
- [~] Add kernel-version and feature metadata.
  ...
  - Typed map-value field floors are source-backed for `bpf_spin_lock`, `bpf_timer`, `kptr:TYPE`, and `bpf_wq`, aggregated from typed map schemas, and checked before live load; extend this to graph roots/nodes and refcount fields as each floor is verified from primary kernel sources.
  ...
  - Iterator target floors are source-backed for the mainline targets currently found in kernel sources. `iter:dmabuf` and `iter:kmem_cache` roots remain unversioned beyond generic iterator support until their target registrations are found or the exposed surface is corrected.
  - Keep verifier fixture target metadata derived from target strings and source-visible forms where possible; simple direct and helper-backed context fields are derived from target-aware `$ctx.FIELD` roots, and typed `map-define --value-type` fields derive map-value floors for `bpf_spin_lock`, `bpf_timer`, `kptr:TYPE`, and `bpf_wq`. Tracepoint-specific context fields, helper-only map arguments, complex first-class helper surfaces the scanner cannot imply, unknown kfuncs, or target-specific requirements should remain explicit fixture `kernel_features`.
...
- [~] Finish resource-backed map semantics.
  ...
  - Keep extending source-level `map-define` only for real map resource metadata. Key/value layouts, natural fixed-record alignment, `--max-entries`, and verifier-managed `bpf_timer`, `bpf_spin_lock`, `bpf_wq`, and top-level `kptr:TYPE` slots are modeled; dynptrs, graph nodes, graph roots, and refcount fields are the next natural BTF-backed validation targets.
