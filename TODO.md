...
- [~] Add kernel-version and feature metadata.
  ...
  - Common helper floors used by current verifier fixtures and first-class surfaces now have source-backed `BpfHelper` metadata; verifier fixtures derive known `helper-call` features from source text. Continue wiring helper usage into compiled-object diagnostics and track remaining helpers, kfuncs, context fields, attach modes, and loader features once each value is source-verified.
  - Keep verifier fixture target metadata derived from target strings where possible; use explicit fixture `kernel_features` for kfuncs, context fields, helper-only map arguments, first-class helper surfaces the scanner cannot imply, or target-specific requirements the target/source scanner cannot imply.
