...
- [~] Add kernel-version and feature metadata.
  ...
  - All modeled helpers now have source-backed `BpfHelper` kernel floors derived from Linux UAPI helper IDs; current verifier `helper-call`, known `kfunc-call`, and target-aware direct/helper-backed `$ctx.FIELD` fixtures derive their feature metadata from source text; source-checked fixture kfuncs plus most cpumask/dynptr, task/cgroup/socket/map, iterator, object/list/rbtree, user-copy, crypto, sched_ext, preempt/IRQ, and RCU kfuncs have Rust metadata; tracepoint payload field specs expose whether their layout came from tracefs or the syscall fallback; and live-load preflight checks helper floors recovered from compiled bytecode plus known-kfunc and context-field floors preserved from source lowering. Continue tracking remaining kfuncs, tracepoint field version claims, attach modes, and loader features once each value is source-verified.
