...
- [~] Generalize context modeling by program type.
  ...
  - Recent progress: the `cgroup_sock` raw `ctx.sk` alias now routes through modeled `ProgramSpec` / `ProbeContext` context-pointer policy instead of verifier/VCC-local `program_type == CgroupSock` checks.
