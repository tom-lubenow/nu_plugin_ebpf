...
- [ ] Add end-to-end non-tracing fixtures.
  ...
  - Recent progress: attach compile fixtures now also cover the `ctx.protocol` skb protocol alias across `socket_filter`, `tc`, `cgroup_skb`, `sk_skb`, and `sk_skb_parser`, plus packet-length aliases across XDP, skb-backed, `sk_msg`, and guarded packet-aware `sock_ops` lowering paths (`ctx.len`, plus kernel-native `ctx.size` on `sk_msg`).
