...
- [~] Add kernel-version and feature metadata.
  ...
  - Add program-specific kfunc compatibility floors before widening allowlists when a kfunc expands to new program families after its original introduction. `bpf_dynptr_from_skb` is the current concrete case: the compiler models the source-verified Linux 6.4 skb-backed packet family, while later tracing/cgroup/sock_ops expansions should wait until compiled-object metadata can report the program-specific floor rather than only the kfunc's global floor.
