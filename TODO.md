...
- [~] Finish resource-backed map semantics.
  ...
  - Map key/value type-spec parser rejects now name the map schema context or offending record field rather than using global wording for invalid array lengths, capacities, unsupported specs, record/array candidate brace errors, graph-root payload brace/schema errors, and nested dynptr/graph/kptr object specs.
  - Finish map-in-map support. Source-level `map-define --kind array-of-maps|hash-of-maps --inner-map INNER` now validates the inner-template contract against a previously declared inner map, emits declared inner and outer object map definitions even without operations, preserves unambiguous outer-to-inner templates across pinned/shared attach peers, emits libbpf-compatible `values` BTF metadata when the inner template is also emitted as a runtime map, models outer `map-get` results as nullable `bpf_map*` pointers, and supports outer `map-contains` plus guarded dynamic inner `map-get $inner_ptr`, `VALUE | map-put $inner_ptr KEY`, `KEY | map-delete $inner_ptr`, and `KEY | map-contains $inner_ptr` operations in dry-run/object generation. Live `kprobe`, `kretprobe`, `tracepoint`, `raw_tracepoint`, `raw_tracepoint.w`, `fentry`, `fexit`, `fmod_ret`, `tp_btf`, `lsm`, path-qualified `lsm_cgroup`, `netfilter`, `netkit`, `flow_dissector`, and cgroup FD attach families (`cgroup_device`, `sock_ops`, `cgroup_skb`, `cgroup_sock`, `cgroup_sysctl`, `cgroup_sockopt`, and `cgroup_sock_addr`) route map-in-map objects through libbpf so libbpf can materialize `inner_map_fd` from BTF `values` metadata; Aya-backed and other non-libbpf live attach families still reject before Aya load because Aya 0.13's map creation path does not materialize that metadata.
...
- [~] Preserve the small Nushell-first surface.
  ...
  - Keep broadening ordinary stack-backed numeric-list, tracked-string, and metadata-backed fixed-record commands when they can preserve verifier-friendly bounds and honest fixed layouts; current coverage includes common `where` / `each` / `all` / `any` / `first` / `last` / `take` / `skip` / `drop` / `reverse` / `uniq` / small-list `sort` / `compact` / `find` / `get` / `length` on stack-backed numeric lists, `length` / `take` / `skip` / `drop` / `reverse` / `append` / `prepend` / `uniq` / `sort` / `compact` / `find` / scalar and counted `first` / `last` / constant-index `get` on compile-time known fixed lists / selected non-empty `math` reducers / compiler-known integer scalar/list `math abs` / first-class integer `seq` / `is-empty` / `is-not-empty` list paths, no-input or compiler-known `describe`, compile-time known binary `bytes length` / `bytes starts-with` / `bytes ends-with` / `bytes index-of` / `bytes build` / `bytes reverse` / `bytes at` / `bytes add` / `bytes remove` / `bytes replace` when scalar or when list outputs can be materialized or folded by `bytes collect`, scalar `first` / `last`, constant-index `get`, `length`, or empty predicates / `bytes collect` / `bytes split` when its parts can be materialized or folded by `bytes collect`, scalar `first` / `last`, constant-index `get`, `length`, or empty predicates / compile-time known binary-list `bytes starts-with` / `bytes ends-with` / `bytes index-of` / `bytes reverse` / `bytes remove` / `bytes replace` / compile-time known binary scalar/list `bits and` / `bits or` / `bits xor` / `bits not` / `bits shl` / `bits shr` / `bits rol` / `bits ror` when scalar or when list outputs can be materialized or folded by `bytes collect`, scalar `first` / `last`, constant-index `get`, `length`, or empty predicates, byte and compile-time known grapheme-cluster `str length` / literal-prefix `str starts-with` / known-length literal-suffix `str ends-with` / known-length literal-substring `str contains` / compile-time known `str distance` / compile-time known scalar/list `str join` / compile-time known `seq char` / compile-time known `str stats` fixed integer records / compile-time known bounded `str expand` string lists / compile-time known byte and grapheme-cluster `str index-of` / compile-time known `--ignore-case` string predicates / compile-time known byte and grapheme-cluster `str substring` / compile-time known substring and regex `str replace` including `--all`, `--no-expand`, and `--multiline` / compile-time known `str trim` default/`--left`/`--right`/`--char` / compile-time known default `str downcase` / `str upcase` / `str reverse` / `str capitalize` / compile-time known default case conversions such as `str camel-case` / `str kebab-case` / `str pascal-case` / `str screaming-snake-case` / `str snake-case` / `str title-case` / `is-empty` / `is-not-empty` / selected `default` string paths, and `get` / `select` / `reject` / `rename` / `merge` / field-name `columns` / integer-field `values` / `is-empty` / `is-not-empty` / `insert` / `update` / `upsert` / selected `default` record paths.
  - Typed fixed arrays now share the same ordinary aggregate model when the element layout is explicit, including representation-preserving numeric-array transforms, stack-list materializing predicates/transforms and math paths for signed integer or <=32-bit unsigned integer elements, integer/bool `all` / `any` predicates, scalar and counted `first` / `last`, `get`, `length`, compatible `append` / `prepend`, integer bitwise/shift/rotate commands, default metadata-preserving `compact` for annotated fixed-layout arrays with `--empty` limited to numeric/bool arrays, and documented string/binary/numeric-list/record projection subsets.
...
- [~] Make unsupported-feature diagnostics actionable.
  ...
  - Typed `global-define` record and array initializer rejects now name nested unexpected-field paths such as `inner.extra` and top-level item paths such as `initializer[0]`; nested record type-spec rejects now name malformed fields, duplicate/reserved fields, invalid length/capacity specs, unsupported field specs, and unbalanced record/array candidate braces using paths such as `inner.bad`, `inner.pid`, or `items`.
