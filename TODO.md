...
- [~] Add kernel-version and feature metadata.
  ...
  - Map-kind kernel floors are now source-backed in `MapKind`, aggregated from compiled object map lists, checked before live load, and derived by verifier fixtures for first-class `--kind` arguments, supported raw `helper-call ... --kind KIND` forms whose ambiguous map family is explicit in source, `tail-call`, and reserved helper map arguments such as `events`, `user_events`, `perf_events`, `kstacks`, and `ustacks`; ad hoc helper-only maps that do not expose a supported `--kind` should stay explicit until the source scanner can infer them honestly.
