...
- [~] Add kfunc and richer BTF-driven typing support.
  ...
  - Tightened stack-argument semantics so stack-required kfunc args (`local_irq*`, `res_spin_*_irq*`, `iter_task_vma*`, `iter_task*`, `iter_num*`, `iter_bits*`, `iter_dmabuf*`, `iter_kmem_cache*`, `iter_scx_dsq*`) must be slot base pointers (offset 0), not interior stack addresses.
