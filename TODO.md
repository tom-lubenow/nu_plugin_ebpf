...
- [~] Generalize context modeling by program type.
  ...
  - Recent progress: per-program return-action aliases now also come from explicit alias tables instead of open-coded per-program matches, while preserving packet-length aliases for socket filters and allow/deny families for cgroup/socket policy programs.
  - Recent progress: callback-sensitive `sock_ops` packet-field policy (`ctx.packet_len`, `ctx.data`, `ctx.skb_tcp_flags`, `ctx.skb_hwtstamp`) now lives in the modeled per-program context schema as a shared load-guard description consumed by type inference, verifier_types, and VCC instead of probe-context-local special cases.
  - Recent progress: the base `ctx.*` field schema itself is now described once as explicit per-field metadata, so type queries, projection queries, and `sock_ops` packet-field load guards all read from the same source of truth instead of separate partially-overlapping matches.
  ...
  - Recent progress: the same modeled `cgroup_sock_addr` surface now also normalizes the attach-sensitive tuple aliases more completely, exposing `ctx.remote_ip*` / `ctx.remote_port` on `connect*`, `getpeername*`, `sendmsg*`, and `recvmsg*`, plus `ctx.local_ip*` / `ctx.local_port` on `bind*` and `getsockname*`, while mapping `sendmsg*` `ctx.local_ip*` onto the explicit source-address fields without inventing a non-existent `ctx.local_port` there.
  ...
  - Recent progress: modeled `ctx.*` name resolution now routes builtin aliases, non-tracepoint special fields, and tracepoint preserved builtin names through explicit tables instead of repeated open-coded matches, including coverage for tracepoint builtin aliases like `tgid` and `arg3`.
