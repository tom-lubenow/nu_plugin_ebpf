...
- [~] Add kernel-version and feature metadata.
  - Keep the program-family and parsed-target compatibility requirement registry authoritative for feature-style requirements. Base program families, BTF tracing/LSM floors, cgroup families, TCX, netfilter links, LWT, struct_ops, sched_ext, iterator targets, XDP multi-buffer sections, cgroup v2, and cgroup UNIX socket-address hooks now carry source-verified minimums where a kernel version is a meaningful claim.
  ...
  - Typed map-value field floors are source-backed for `bpf_spin_lock`, `bpf_timer`, `kptr:TYPE`, `bpf_wq`, and `bpf_refcount`, aggregated from typed map schemas, and checked before live load; extend this to graph roots/nodes as each floor is verified from primary kernel sources.
  ...
  - Kfunc compatibility can now express source-checked upper bounds for removed/renamed kfunc windows; resource spin-lock kfuncs are source-backed from the Linux 6.15 verifier special-kfunc list. Remaining no-hit kfunc signatures are currently sched_ext node variants, and should stay unmodeled until found in primary kernel sources.
