...
- [~] Add kfunc and richer BTF-driven typing support.
  ...
  - Extended kernel-pointer pointer-space checks for unknown kfuncs to consult local kernel BTF pointee-type metadata (task/cgroup/cpumask/file/inode/socket/crypto families) via a cached query path, while preserving explicit compiler-side semantics for built-in typed kfuncs.
