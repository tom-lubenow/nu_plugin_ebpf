...
- [~] Support global/static data sections.
  ...
  - Recent progress: explicit `global-define --type ...` declarations now also expose fixed arrays with `array{type:N}`, including scalar and homogeneous aggregate list initializers that are zero-padded to the declared length and zero-initialized fixed-layout aggregate elements such as records.
