...
- [~] Expand typed helper/kfunc semantics where it materially improves safety.
  ...
  - Map-value `bpf_spin_lock` unlock identity, graph-root kfunc lock-required checks, resource spin-lock same-lock duplicate acquire / ordered unlocks, and active-lock call restrictions are now tracked; richer validation that graph root operations use the same concrete map entry lock as the root, and that resource spin-lock pointers actually target a `bpf_res_spin_lock` field, still depends on fuller object/map-value schemas.
