...
- [~] Generalize context modeling by program type.
  ...
  - Recent progress: typed helper arg-shape parity now also rejects anonymous raw kernel pointers for `bpf_sock_from_file` (`struct file *`) and `bpf_task_pt_regs` (`struct task_struct *`), while still accepting tracked refs and explicitly typed pointers across type inference, verifier_types, and VCC.
