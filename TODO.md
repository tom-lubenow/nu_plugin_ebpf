...
- [~] Generalize context modeling by program type.
  ...
  - Recent progress: modeled `sock_ops` helper callback guards now route through the same shared proof path as packet-field guards across type inference, verifier_types, and VCC: `bpf_sock_ops_cb_flags_set` / `bpf_load_hdr_opt` require locked sock_ops callbacks, while `bpf_store_hdr_opt` / `bpf_reserve_hdr_opt` require the concrete `WRITE_HDR_OPT` / `HDR_OPT_LEN` callbacks.
  - Recent progress: modeled `cgroup_sysctl` helper write-mode guards now use that same shared proof path too: `bpf_sysctl_get_new_value` and `bpf_sysctl_set_new_value` require a proven `ctx.write == 1` branch across type inference, verifier_types, and VCC.
  - Recent progress: shared helper scalar-flag constraints now also model `bpf_sysctl_get_name` flags, rejecting unknown bits and accepting `0` / `BPF_F_SYSCTL_BASE_NAME` across type inference, verifier_types, and VCC.
  - Recent progress: the base helper program surface now routes through explicit helper-surface metadata instead of one large open-coded policy match, and `bpf_get_socket_cookie` arg-policy grouping is modeled through the same style of per-program lists.
