...
- [~] Add kernel-version and feature metadata.
  ...
  - Typed map-value field floors have started with source-backed `bpf_wq` support, aggregated from typed map schemas and checked before live load; extend this to timer, spin-lock, kptr, graph roots/nodes, and refcount fields as each floor is verified from primary kernel sources.
  ...
  - Keep verifier fixture target metadata derived from target strings where possible; simple direct and helper-backed context fields are derived from target-aware `$ctx.FIELD` roots, while tracepoint-specific context fields, helper-only map arguments, complex first-class helper surfaces the scanner cannot imply, unknown kfuncs, or target-specific requirements should remain explicit fixture `kernel_features`.
