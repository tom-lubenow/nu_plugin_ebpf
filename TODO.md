...
- [~] Add kfunc and richer BTF-driven typing support.
  ...
  - Added typed signature coverage for task iterators (`bpf_iter_task_new` / `bpf_iter_task_next` / `bpf_iter_task_destroy`) with stack-pointer argument enforcement and stack-slot identity/lifetime tracking across type inference, verifier_types, and VCC.
  - Tightened stack-argument semantics so stack-required kfunc args (`local_irq*`, `res_spin_*_irq*`, `iter_task_vma*`, `iter_task*`, `iter_scx_dsq*`, `iter_num*`, `iter_bits*`) must be slot base pointers (offset 0), not interior stack addresses.
