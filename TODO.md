...
- [~] Generalize context modeling by program type.
  ...
  - Recent progress: modeled helper-surface legality now also constrains `bpf_get_socket_cookie` to the concrete upstream-supported tracing/socket program families instead of allowing false positives like `sk_lookup`, and helper arg-shape policy now distinguishes raw-context families (`socket_filter`/`tc`/`cgroup_skb`/`cgroup_sock_addr`/`sock_ops`/`sk_skb*`) from tracing socket-pointer families (`fentry`/`fexit`/`tp_btf`), with `cgroup_sock` explicitly modeled as a context/socket alias because its runtime ctx is itself a `bpf_sock *`; remaining work here is narrower nullable/BTF-subtype parity for the tracing `sock_common` form.
