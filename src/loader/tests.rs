...
fn test_raw_tracepoint_map_in_map_routes_to_libbpf_loader() {
    ...
    let err = state
        .attach(&object)
        .expect_err("raw_tracepoint map-in-map should route to libbpf loader");
    let message = err.to_string();

    assert!(
        !message.contains("Aya-backed live loading of map-in-map runtime map"),
        "raw_tracepoint map-in-map should not hit the Aya-only rejection: {err:?}"
    );
    assert!(
        message.contains("libbpf") || message.contains("raw_tracepoint"),
        "unexpected raw_tracepoint map-in-map libbpf dispatch error: {err:?}"
    );
}
...
fn test_btf_trace_map_in_map_routes_to_libbpf_loader() {
    for (prog_type, target, label) in [
        ...
        let err = state
            .attach(&object)
            .expect_err("BTF tracing map-in-map should route to libbpf loader");
        let message = err.to_string();
...
#[test]
fn test_lsm_map_in_map_routes_to_libbpf_loader() {
    let state = EbpfState::new();
    let (inner_ref, _, outer_ref) = map_in_map_fixture_refs();
    let object = live_map_in_map_fixture_for_program(
        EbpfProgramType::Lsm,
    ...
    let err = state
        .attach(&object)
        .expect_err("lsm map-in-map should route to libbpf loader");
    let message = err.to_string();

    assert!(
        !message.contains("Aya-backed live loading of map-in-map runtime map"),
        "lsm map-in-map should not hit the Aya-only rejection: {err:?}"
    );
    assert!(
        message.contains("libbpf") || message.contains("lsm"),
        "unexpected lsm map-in-map libbpf dispatch error: {err:?}"
    );
}
...
fn test_attach_rejects_compile_only_programs_before_loading() {
    ...
    for (prog_type, target, label, detail) in [
        ...
        (
            ...
            "demo-route",
            "lwt_in",
            "route/link configuration plus a map/event-preserving loader path",
        ),
        (
