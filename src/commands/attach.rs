...
Context parameter syntax (recommended):
  ...
  Packet-context fields:
    ...
    {|ctx| $ctx.tc_classid } - Get the skb tc_classid on tc_action, tc, tcx, and netkit programs
    {|ctx| $ctx.cgroup_classid } - Get skb cgroup class ID on lwt_*, tc_action, netkit, and tc/tcx egress
    {|ctx| $ctx.route_realm } - Get skb route realm on lwt_*, tc_action, netkit, and tc/tcx egress
    {|ctx| $ctx.csum_level } - Query skb checksum level on lwt_xmit, tc_action, tc, tcx, netkit, sk_skb, and sk_skb_parser
    {|ctx| $ctx.skb_cgroup_id } - Get skb cgroup ID on tc_action, tc egress, and tcx egress
    {|ctx| $ctx.skb_ancestor_cgroup_id.0 } - Get skb ancestor cgroup ID on tc_action, tc egress, and tcx egress at constant level 0
    {|ctx| $ctx.napi_id } - Get the skb napi_id on skb-backed packet programs
    {|ctx| $ctx.wire_len } - Get the skb wire_len on tc_action, tc, tcx, and netkit programs
    ...
    {|ctx| $ctx.tstamp } - Get the skb timestamp on tc_action, tc, tcx, netkit, and cgroup_skb programs
    {|ctx| $ctx.tstamp_type } - Get the skb timestamp type on tc_action, tc, tcx, and netkit programs
    {|ctx| $ctx.hwtstamp } - Get the skb hardware timestamp on tc_action, tc, tcx, netkit, and cgroup_skb programs
    {|ctx| $ctx.data }    - Get packet data pointer on xdp, flow_dissector, lwt_*, tc_action, tc, tcx, netkit, cgroup_skb, sk_reuseport, sk_msg, sk_skb, sk_skb_parser, and packet-aware sock_ops callbacks
    {|ctx| $ctx.data_end } - Get packet end pointer on xdp, flow_dissector, lwt_*, tc_action, tc, tcx, netkit, cgroup_skb, sk_reuseport, sk_msg, sk_skb, sk_skb_parser, and packet-aware sock_ops callbacks
    ...
    {|ctx| $ctx.hash_recalc } - Get skb hash via bpf_get_hash_recalc on lwt_*, tc_action, tc, tcx, netkit, sk_skb, and sk_skb_parser
    {|ctx| $ctx.recalc_hash } - Alias for ctx.hash_recalc on lwt_*, tc_action, tc, tcx, netkit, sk_skb, and sk_skb_parser
    {|ctx| $ctx.socket_cookie } - Get the stable socket cookie on supported socket-backed contexts
    {|ctx| $ctx.socket_uid } - Get the socket owner UID on socket_filter, tc_action, tc, tcx, netkit, cgroup_skb, sk_skb, and sk_skb_parser
    {|ctx| $ctx.netns_cookie } - Get the stable network-namespace cookie on supported socket-backed contexts
    {|ctx| $ctx.mark }    - Get the socket or skb mark on cgroup_sock sock_create/sock_release, socket_filter, lwt_*, tc_action, tc, tcx, netkit, and cgroup_skb programs
    {|ctx| $ctx.priority } - Get the socket or skb priority on cgroup_sock sock_create/sock_release, socket_filter, lwt_*, tc_action, tc, tcx, netkit, cgroup_skb, sk_skb, and sk_skb_parser programs
    ...
    `next`, `pass`, `drop`, and `redirect`. cgroup_skb closures can return
    ...
    `redirect --peer IFINDEX` is modeled on `tc_action`, `tc:...:ingress`,
    `tcx:...:ingress`, and netkit, and requires `FLAGS = 0`.
    `redirect --neigh IFINDEX` is modeled on tc_action/tc/tcx/netkit
    ...
    echo `echo_id` / `echo_sequence`, and common fixed-offset subtype
    fields such as ICMP redirect `gateway`, `next_hop_mtu`, timestamp
    words, ICMPv6 `mtu`, `pointer`, neighbor-discovery `nd_target`, and
    ...
    hooks. When the timestamp type must also change, tc_action, tc, tcx, and netkit
    ...
    target program FD/BTF compatibility and live attach are not modeled yet,
  ...
  cgroup_sysctl fields:
    ...
    {|ctx| $ctx.sysctl_name } - Get the full sysctl path as a NUL-terminated 256-byte buffer
  ...
  cgroup_sock fields:
    ...
    {|ctx| $ctx.state }   - Get the current socket state
  ...
  Function fields:
    ...
    {|ctx| $ctx.retval }  - Get return value (kretprobe/kretprobe.multi/kretsyscall/uretprobe/uretprobe.multi/fexit/fmod_ret)

    Note: entry probes (`kprobe`, `kprobe.multi`, `ksyscall`, `uprobe`,
    `uprobe.multi`) expose pt_regs-style ctx.arg0-5. Return probes
...
Advanced commands:
  ...
  map-contains      - Test lookup-map, local-storage, bloom-filter, or cgroup-array membership
  ...
  - Run `ebpf setup` to configure capabilities"#
    ...
    fn examples(&self) -> Vec<Example<'_>> {
        vec![
            ...
            Example {
                example: "ebpf attach --dry-run 'tcx:lo:ingress' {|ctx| $ctx.packet_len | count; 'pass' }",
                description: "Compile a TCX classifier program without loading it",
                result: None,
            },
            Example {
                example: "ebpf attach --dry-run 'netkit:nk0:primary' {|ctx| $ctx.packet_len | count; 'pass' }",
                description: "Compile a Netkit classifier program without loading it",
                result: None,
            },
            Example {
            ...
            Example {
                example: "ebpf attach --dry-run 'freplace:replace_me' {|| 0 }",
                description: "Compile a freplace extension section without live-loading it",
                result: None,
            },
            Example {
            ...
            Example {
                example: "ebpf attach --dry-run 'iter:task' {|ctx| $ctx.meta.seq_num | count; if $ctx.task != 0 { 1 } else { 0 } }",
                description: "Compile a BPF task iterator section using iterator metadata and the iterated task pointer",
                result: None,
            },
            Example {
