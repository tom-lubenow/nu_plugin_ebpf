...
use nu_protocol::{
    Category, Example, LabeledError, PipelineData, Signature, Span, Type, Value, record,
};
...
impl PluginCommand for EbpfTrace {
...
fn run_trace(engine: &EngineInterface, call: &EvaluatedCall) -> Result<PipelineData, LabeledError> {
    ...
    let file = File::open(trace_pipe_path).map_err(|e| {
        let help = if e.kind() == std::io::ErrorKind::PermissionDenied {
            Some("Try running with sudo or ensure debugfs is mounted")
        } else if e.kind() == std::io::ErrorKind::NotFound {
            Some("Ensure debugfs is mounted: mount -t debugfs none /sys/kernel/debug")
        } else {
            None
        };
        let mut err =
