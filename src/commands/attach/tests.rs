...
fn test_map_leading_annotated_mut_globals_supports_constant_list_find_invert_initializer() {
    ...
    let globals = super::map_leading_annotated_mut_globals(source, &ir_block, Span::test_data())
        .expect("constant list find --invert initializer should map cleanly");
    ...
    match &globals[0].initial_value {
        Value::List { vals, .. } => {
            let ints = vals
                .iter()
                .map(|value| value.as_int().expect("find --invert should keep integers"))
                .collect::<Vec<_>>();
            assert_eq!(ints, vec![2, 4]);
        }
        other => panic!("expected list initializer, got {other:?}"),
...
#[test]
fn test_map_leading_annotated_mut_globals_supports_constant_math_min_max_float_fill() {
    let cases = [
        (
            r#"{|| mut value: string = ([1.5, 2, 3] | math min | fill --width 4 --alignment right --character "0"); $value }"#,
            "01.5",
            "math min selected float should feed fill",
        ),
        (
            r#"{|| mut value: string = ([1, 2, 3.5] | math max | fill --width 4 --alignment right --character "0"); $value }"#,
            "03.5",
            "math max selected float should feed fill",
        ),
        (
            r#"{|| mut value: string = ([1, 2, 3.5] | ^math max | fill --width 4 --alignment right --character "0"); $value }"#,
            "03.5",
            "external math max selected float should feed fill",
        ),
    ];
