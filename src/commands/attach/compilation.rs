...
use crate::named_chars::{NAMED_CHAR_HEX, known_named_char_hex};
...
fn eval_supported_constant_external_call(
    ...
    match cmd_name {
        ...
        "find" => {
            eval_supported_constant_list_find_external_call(working_set, input, args, env, span)
        }
        "length" => {
...
fn eval_supported_constant_known_named_char(name: &str) -> Option<String> {
    let hex = known_named_char_hex(name)?;
...
fn eval_supported_constant_named_char_list_value(span: Span) -> Result<Value, LabeledError> {
    let mut rows = Vec::with_capacity(NAMED_CHAR_HEX.len());
    for (name, hex) in NAMED_CHAR_HEX {
        let mut record = Record::new();
            ...
            nu_protocol::ast::Argument::Named(named) => match named.0.item.as_str() {
                "invert" | "v" => {
                    if named.2.is_some() {
                        ...
                        .with_label(
                            "`find --invert` cannot receive a value in compile-time global initializers",
                            arg.span(),
                        ));
                    }
                    invert = true;
...
pub(super) fn compile_closure_with_context(
    ...
    let external_map_key_types = pin_group
        ...
        .transpose()?;
    let external_map_key_semantics = pin_group
        .map(|group| {
            state
                .pinned_generic_map_key_semantics(group)
                .map_err(|e| match e {
                    LoadError::LockPoisoned => LabeledError::new("Failed to attach eBPF probe")
                        .with_label("loader state lock poisoned", call_head),
                    other => LabeledError::new("Failed to attach eBPF probe")
    ...
    let lower_result = lower_hir_to_mir_with_hints_key_value_maps_and_semantics(
        ...
        external_map_key_semantics.as_ref(),
