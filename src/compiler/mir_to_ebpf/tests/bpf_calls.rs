...
fn test_helper_call_rejects_unencodable_helper_id() {
    ...
    let helper = i32::MAX as u32 + 1;

    let err = compiler
        .compile_instruction(&LirInst::CallHelper {
            helper,
            args: vec![],
            ret: VReg(0),
        })
        .expect_err("unencodable helper ID should be rejected");
...
fn test_bpf_loop_callback_load_emits_text_relocation() {
    ...
    let type_hints = MirTypeHints {
        ...
        generic_map_key_semantics: std::collections::HashMap::new(),
        ...
        generic_map_extras: std::collections::HashMap::new(),
