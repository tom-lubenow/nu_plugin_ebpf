...
#[test]
fn test_stack_pointer_descending_loop_counter_range_in_bounds() {
    let mut func = MirFunction::new();
    let entry = func.alloc_block();
    let header = func.alloc_block();
    let body = func.alloc_block();
...
fn test_packet_load_with_data_end_guard_passes() {
    ...
    verify_mir(&func, &types).expect("guarded packet byte load should verify");
}
...
fn test_packet_store_with_data_end_guard_passes() {
    ...
    verify_mir(&func, &types).expect("guarded packet byte store should verify");
}
...
#[test]
fn test_packet_store_without_data_end_guard_fails() {
    let mut func = MirFunction::new();
    let entry = func.alloc_block();
    func.entry = entry;
    ...
    let err = verify_mir(&func, &types).expect_err("unguarded packet store should fail");
    assert!(
        err.iter()
            .any(|e| e.message.contains("preceding packet end-pointer guard")),
        "unexpected errors: {:?}",
...
#[test]
fn test_packet_load_rejects_guard_that_is_too_small() {
    let mut func = MirFunction::new();
    let entry = func.alloc_block();
    let load = func.alloc_block();
    let done = func.alloc_block();
    ...
    let err = verify_mir(&func, &types).expect_err("undersized packet guard should fail");
    assert!(
...
#[test]
fn test_joined_typed_null_copy_preserves_pointer_nullability() {
    let mut func = MirFunction::new();
    let entry = func.alloc_block();
