use super::*;
use crate::compiler::mir::StringAppendType;
...
#[test]
fn test_string_append_integer_rejects_pointer_value() {
    let mut func = MirFunction::new();
    let entry = func.alloc_block();
    func.entry = entry;
    ...
    assert!(
        err.iter()
            .any(|e| e.message.contains("string append integer expects scalar")),
        "unexpected errors: {:?}",
        err
    );
}
...
#[test]
fn test_string_append_rejects_pointer_length() {
    let mut func = MirFunction::new();
    let entry = func.alloc_block();
    func.entry = entry;
    ...
    assert!(
        err.iter()
            .any(|e| e.message.contains("string append length expects scalar")),
        "unexpected errors: {:?}",
        err
    );
}
...
#[test]
fn test_int_to_string_rejects_pointer_value() {
    let mut func = MirFunction::new();
    let entry = func.alloc_block();
    func.entry = entry;
    ...
    assert!(
        err.iter()
            .any(|e| e.message.contains("int to string value expects scalar")),
        "unexpected errors: {:?}",
        err
    );
}
