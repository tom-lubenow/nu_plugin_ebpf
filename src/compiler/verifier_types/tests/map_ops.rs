...
#[test]
fn test_dynamic_map_lookup_requires_outer_lookup_null_check() {
    let mut func = MirFunction::new();
    let entry = func.alloc_block();
    func.entry = entry;
...
#[test]
fn test_dynamic_map_lookup_accepts_null_checked_outer_lookup() {
    let mut func = MirFunction::new();
    let entry = func.alloc_block();
    let ok = func.alloc_block();
...
#[test]
fn test_dynamic_map_update_requires_outer_lookup_null_check() {
    let mut func = MirFunction::new();
    let entry = func.alloc_block();
    func.entry = entry;
...
#[test]
fn test_dynamic_map_delete_requires_outer_lookup_null_check() {
    let mut func = MirFunction::new();
    let entry = func.alloc_block();
    func.entry = entry;
...
#[test]
fn test_dynamic_map_update_and_delete_accept_null_checked_outer_lookup() {
    let mut func = MirFunction::new();
    let entry = func.alloc_block();
    let ok = func.alloc_block();
    let done = func.alloc_block();
...
#[test]
fn test_map_update_rejects_user_key_pointer() {
    let mut func = MirFunction::new();
    let entry = func.alloc_block();
    ...
    func.block_mut(call).instructions.push(MirInst::Copy {
        dst: val,
        src: MirValue::StackSlot(val_slot),
    });
    func.block_mut(call).instructions.push(MirInst::MapUpdate {
        map: MapRef {
            name: "m".to_string(),
            kind: MapKind::Hash,
        },
...
#[test]
fn test_map_update_rejects_user_value_pointer() {
    let mut func = MirFunction::new();
    let entry = func.alloc_block();
    let call = func.alloc_block();
    let done = func.alloc_block();
...
fn test_map_update_rejects_oversized_key_layout() {
    ...
    assert!(
        err.iter().any(|e| {
            e.message.contains("key size") && e.message.contains("u32 eBPF map definition range")
        }),
        "unexpected errors: {:?}",
        err
    );
