...
impl EbpfProgramType {
    ...
    pub(crate) fn resolve_ctx_field_name(&self, field_name: &str) -> Result<CtxField, String> {
        if let Some(field) = self.ctx_field_alias(field_name) {
            return Ok(field);
        }

        if matches!(self, EbpfProgramType::Tracepoint) {
            return Self::generic_ctx_field_from_name(field_name);
        }

        if let Some(field) = Self::non_tracepoint_ctx_field_from_name(field_name) {
            return Ok(field);
        }

        Self::generic_ctx_field_from_name(field_name)
    }
