...
use crate::program_spec::{
    ProgramLiveAttachUnsupportedReason, ProgramSpec,
    struct_ops_callback_is_sleepable as program_spec_struct_ops_callback_is_sleepable,
};
...
impl ProgramAttachKind {
    ...
    pub fn loader_supports_live_attach(self) -> bool {
        self.unsupported_live_attach_reason().is_none()
    }

    pub fn unsupported_live_attach_reason(self) -> Option<ProgramLiveAttachUnsupportedReason> {
        Some(match self {
            Self::RawTracepointWritable => {
                ProgramLiveAttachUnsupportedReason::RawTracepointWritable
            }
            Self::FmodRet => ProgramLiveAttachUnsupportedReason::FmodRet,
            ...
            Self::Iter => ProgramLiveAttachUnsupportedReason::Iter,
            _ => return None,
        })
    }

    pub fn unsupported_live_attach_detail(self) -> Option<&'static str> {
        self.unsupported_live_attach_reason()
...
impl GlobalCompatibilityRequirement {
    ...
    fn effective_minimum_kernel_requirement(requirements: &[Self]) -> Option<&Self> {
        ...
        for requirement in requirements {
            ...
            let should_replace = match minimum {
                Some(current) => {
                    !Self::kernel_version_at_least(current.minimum_kernel(), candidate)
                }
                None => true,
            };
            if should_replace {
...
impl ContextFieldDirectLoad {
    ...
    pub(crate) const fn u16(offset: i16) -> Self {
        Self {
            width: ContextFieldDirectLoadWidth::U16,
            offset,
        }
    }
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub(crate) struct ContextFieldIndexedStore {
    pub(crate) offset: i16,
    pub(crate) convert_to_big_endian: bool,
}

impl ContextFieldIndexedStore {
    pub(crate) const fn new(offset: i16) -> Self {
        Self {
            offset,
            convert_to_big_endian: false,
        }
    }

    pub(crate) const fn big_endian_u32(offset: i16) -> Self {
        Self {
            offset,
            convert_to_big_endian: true,
        }
    }
}
