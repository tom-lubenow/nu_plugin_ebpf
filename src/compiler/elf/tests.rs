...
#[test]
fn test_elf_map_btf_emits_graph_root_decl_tag() {
    use crate::compiler::instruction::{EbpfBuilder, EbpfInsn, EbpfReg};
    ...
    let program = EbpfProgram::with_maps(
        ...
        None,
        None,
        HashMap::from([(map_ref.clone(), value_ty)]),
        HashMap::new(),
    )
    .with_generic_map_key_types(HashMap::from([(map_ref, key_ty)]));
    ...
    assert!(
        btf.id_by_type_name_kind("bpf_list_head", BtfKind::Struct)
            .is_ok()
    );
    assert!(
        btf.id_by_type_name_kind("bpf_list_node", BtfKind::Struct)
            .is_ok()
    );
    assert!(
        btf.id_by_type_name_kind("node_data", BtfKind::Struct)
            .is_ok()
    );
    assert!(
        btf_data
    ...
    assert!(
        btf_data
            .windows(b"__nu_bpf_graph_root:".len())
            .all(|w| w != b"__nu_bpf_graph_root:"),
        "internal graph root wrapper should not leak into emitted BTF"
    );
}

#[test]
fn test_elf_map_btf_emits_bpf_wq_value_type() {
    use crate::compiler::instruction::{EbpfBuilder, EbpfInsn, EbpfReg};
    ...
    assert!(btf.id_by_type_name_kind("bpf_wq", BtfKind::Struct).is_ok());
}
...
#[test]
fn test_ebpf_program_reports_map_value_compatibility_requirements() {
    let map_ref = MapRef {
        name: "work_items".to_string(),
        kind: MapKind::Array,
    };
...
fn test_ebpf_program_reports_context_field_compatibility_requirements() {
    ...
    assert!(requirements.iter().all(|requirement| {
        matches!(
            ...
            "ctx:data" | "ctx:data_end" | "ctx:packet_len" | "ctx:skb_len" | "ctx:skb_tcp_flags"
