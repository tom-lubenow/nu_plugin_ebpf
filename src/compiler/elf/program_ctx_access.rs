...
const SOCKET_FILTER_CTX_FIELD_ACCESS_SURFACES: &[ContextFieldAccessSurfaceSpec] = &[
    ContextFieldAccessSurfaceSpec::new(
        CtxField::TcClassid,
        "tc_classid",
        ContextFieldAccessRequirement::AllowedProgramsLabel("lwt_*, tc_action, and tc programs"),
    ),
    ContextFieldAccessSurfaceSpec::new(
        CtxField::WireLen,
        "wire_len",
    ...
    ContextFieldAccessSurfaceSpec::new(
        ...
        ContextFieldAccessRequirement::AllowedProgramsLabel(
            "lwt_*, tc_action, tc, and cgroup_skb programs",
        ),
    ),
    ContextFieldAccessSurfaceSpec::new(
        CtxField::TstampType,
    ...
    fn error(self, program_type: EbpfProgramType, field: &CtxField) -> String {
        match self {
            ...
            Self::TracingHelperFields => format!(
                "ctx.{} is only available on kprobe, kretprobe, kprobe.multi, kretprobe.multi, ksyscall, kretsyscall, uprobe, uretprobe, perf_event, raw_tracepoint, raw_tracepoint.w, tracepoint, fentry, fexit, fmod_ret, and tp_btf programs",
                field.display_name()
            ),
            ...
            Self::SkbFields => format!(
                "ctx.{} is only available on socket_filter, lwt_*, tc_action, tc, cgroup_skb, sk_skb, and sk_skb_parser programs",
                field.display_name()
            ),
            Self::DataMetaField => {
                format!(
            ...
            Self::ArgFields => format!(
                "ctx.{} is only available on contexts with argument access (kprobe, kprobe.multi, ksyscall, uprobe, fentry, fexit, fmod_ret, tp_btf, lsm, struct_ops, and raw_tracepoint)",
                field.display_name()
            ),
            Self::ArgCountField => {
