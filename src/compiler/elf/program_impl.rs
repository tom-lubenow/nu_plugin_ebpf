...
fn bytecode_has_local_subprogram_call(bytecode: &[u8]) -> bool {
    bytecode.chunks_exact(8).any(|instruction| {
        instruction[0] == opcode::CALL && {
            let src_reg = instruction[1] >> 4;
            src_reg == 1
        }
    })
...
fn bytecode_has_backward_jump(bytecode: &[u8]) -> bool {
    bytecode.chunks_exact(8).any(|instruction| {
        let class = instruction[0] & 0x07;
        if class != opcode::BPF_JMP && class != opcode::BPF_JMP32 {
            return false;
        }
        let op = instruction[0] & 0xf0;
        if op == opcode::BPF_CALL || op == opcode::BPF_EXIT {
            return false;
        }
...
fn compiled_feature_compatibility_requirements_for_section(
    program: &EbpfProgramSection,
) -> Vec<CompiledFeatureCompatibilityRequirement> {
    let mut requirements = Vec::new();

    if !program.subfunctions.is_empty() || bytecode_has_local_subprogram_call(&program.bytecode) {
        requirements.push(CompiledFeatureCompatibilityRequirement::BpfSubprogramCalls);
    }
    if bytecode_has_backward_jump(&program.bytecode) {
        requirements.push(CompiledFeatureCompatibilityRequirement::BoundedLoops);
    }
...
fn compiled_feature_compatibility_requirements_for_programs(
    programs: &[EbpfProgramSection],
) -> Vec<CompiledFeatureCompatibilityRequirement> {
    let mut seen = HashSet::new();
    let mut requirements = Vec::new();
    ...
    for program in programs {
        for requirement in compiled_feature_compatibility_requirements_for_section(program) {
            if seen.insert(requirement) {
                requirements.push(requirement);
            }
        }
...
impl EbpfProgram {
    ...
    /// Attach logical generic map key semantics recovered during lowering.
    pub fn with_generic_map_key_semantics(
        mut self,
        generic_map_key_semantics: HashMap<MapRef, AnnotatedValueSemantics>,
    ) -> Self {
        self.generic_map_key_semantics = generic_map_key_semantics;
        self
    }
...
impl EbpfProgramSection {
    ...
    pub fn helper_compatibility_minimum_kernel_source(&self) -> Option<&'static str> {
        HelperCompatibilityRequirement::effective_minimum_kernel_source(
            &self.helper_compatibility_requirements(),
        )
    }
    ...
    pub fn compiled_feature_compatibility_requirements(
        ...
        compiled_feature_compatibility_requirements_for_section(self)
    }
    ...
    pub fn kfunc_compatibility_minimum_kernel_source(&self) -> Option<&'static str> {
        KfuncCompatibilityRequirement::effective_minimum_kernel_source(
            &self.kfunc_compatibility_requirements(),
        )
    }
    ...
    pub fn context_field_compatibility_minimum_kernel_source(&self) -> Option<&'static str> {
        ContextFieldCompatibilityRequirement::effective_minimum_kernel_source(
            &self.context_field_compatibility_requirements(),
        )
    }
    ...
    /// compiler-generated bytecode feature, kfunc, and source-preserved
...
impl EbpfObject {
    ...
    pub fn map_compatibility_minimum_kernel_source(&self) -> Option<&'static str> {
        MapCompatibilityRequirement::effective_minimum_kernel_source(
            &self.map_compatibility_requirements(),
        )
    }
    ...
    pub fn global_compatibility_minimum_kernel_source(&self) -> Option<&'static str> {
        GlobalCompatibilityRequirement::effective_minimum_kernel_source(
            &self.global_compatibility_requirements(),
        )
    }
    ...
    pub fn compiled_feature_compatibility_requirements(
        ...
        compiled_feature_compatibility_requirements_for_programs(&self.programs)
    }
    ...
    /// map-value, helper, compiler-generated bytecode feature, kfunc, and
...
impl EbpfProgram {
    ...
    pub fn compiled_feature_compatibility_requirements(
        ...
        compiled_feature_compatibility_requirements_for_section(&section)
    }
