...
impl<'a> HirToMirLowering<'a> {
    pub(super) fn lower_binary_op(
        ...
        if matches!(
            op,
            Operator::Comparison(Comparison::EndsWith | Comparison::NotEndsWith)
        ) {
            let invert = matches!(op, Operator::Comparison(Comparison::NotEndsWith));
            self.lower_runtime_string_ends_with_operator(lhs_dst, rhs, invert, constant_value)?;
            return Ok(());
        }
    ...
    fn lower_runtime_string_equality(
        ...
        if lhs_source.exact_len.is_none() && rhs_source.exact_len.is_none() {
            self.lower_runtime_string_equality_dynamic(
                lhs_dst,
                result_vreg,
                invert,
                &lhs_source,
    ...
    fn lower_runtime_string_equality_dynamic(
        &mut self,
        dst: RegId,
        dst_vreg: VReg,
        invert: bool,
        ...
        self.emit(MirInst::BinOp {
            ...
            rhs: MirValue::VReg(rhs_source.len_vreg),
        });
        self.vreg_type_hints.insert(len_matches, MirType::Bool);
    ...
    fn lower_guarded_same_offset_string_byte_match(
        ...
        for offset in 0..compare_bound {
            ...
            self.vreg_type_hints.insert(lhs_in_range, MirType::Bool);

            let rhs_in_range = self.func.alloc_vreg();
            self.emit(MirInst::BinOp {
                dst: rhs_in_range,
                op: BinOpKind::Gt,
                lhs: MirValue::VReg(rhs_source.len_vreg),
                rhs: MirValue::Const(offset as i64),
            });
            self.vreg_type_hints.insert(rhs_in_range, MirType::Bool);

            let in_range = self.func.alloc_vreg();
            self.emit(MirInst::BinOp {
                dst: in_range,
                op: BinOpKind::And,
                lhs: MirValue::VReg(lhs_in_range),
                rhs: MirValue::VReg(rhs_in_range),
            });
    ...
    fn lower_runtime_string_ends_with_operator(
        ...
        let lhs_source = lhs_meta
            ...
            .ok_or_else(|| {
                CompileError::UnsupportedInstruction(
                    "ends-with operator requires tracked string left operand in eBPF".into(),
                )
            })?;
        let suffix_source = rhs_meta
            .as_ref()
            .and_then(|meta| self.string_equality_source(meta))
            .ok_or_else(|| {
                CompileError::UnsupportedInstruction(
        ...
        if suffix_len > lhs_source.max_len
            || lhs_source.exact_len.is_some_and(|len| len < suffix_len)
        {
            self.lower_runtime_string_equality_const(lhs_dst, result_vreg, invert, false);
            return Ok(());
        }
        ...
        if let Some(lhs_len) = lhs_source.exact_len {
            ...
            for lhs_len in suffix_len..=lhs_source.max_len {
                let len_matches = self.func.alloc_vreg();
                self.emit(MirInst::BinOp {
                    dst: len_matches,
                    op: BinOpKind::Eq,
    ...
    fn lower_runtime_string_contains_operator(
        ...
        if let Some(needle) = self.source_literal_string(needle_reg) {
            ...
            let last_offset =
                haystack_source.exact_len.unwrap_or(haystack_source.max_len) - needle_len;
            ...
            self.lower_runtime_string_dynamic_contains_operator_match(
                contains_vreg,
                &needle_source,
                &haystack_source,
                context,
    ...
    fn lower_runtime_string_dynamic_contains_operator_match(
        &mut self,
        contains_vreg: VReg,
        needle_source: &StringEqualitySource,
        haystack_source: &StringEqualitySource,
        context: &str,
    ) -> Result<(), CompileError> {
        let haystack_max_len = haystack_source.exact_len.unwrap_or(haystack_source.max_len);
        let needle_max_len = needle_source.exact_len.unwrap_or(needle_source.max_len);
        let max_needle_len = haystack_max_len.min(needle_max_len);
        let candidate_count = (0..haystack_max_len)
            .map(|offset| max_needle_len.min(haystack_max_len - offset))
            .sum::<usize>();
        if candidate_count > MAX_DYNAMIC_STRING_CONTAINS_CANDIDATES {
            return Err(CompileError::UnsupportedInstruction(format!(
                "{context} operator dynamic string needle has {candidate_count} candidate offsets; eBPF lowering supports at most {MAX_DYNAMIC_STRING_CONTAINS_CANDIDATES}"
            )));
        }
        ...
        self.emit(MirInst::BinOp {
            dst: needle_empty,
            op: BinOpKind::Eq,
            lhs: MirValue::VReg(needle_source.len_vreg),
            rhs: MirValue::Const(0),
        });
        self.vreg_type_hints.insert(needle_empty, MirType::Bool);
        self.emit(MirInst::Copy {
            dst: contains_vreg,
            src: MirValue::VReg(needle_empty),
        });
        self.vreg_type_hints.insert(contains_vreg, MirType::Bool);

        for offset in 0..haystack_max_len {
            let max_len_at_offset = max_needle_len.min(haystack_max_len - offset);
            for needle_len in 1..=max_len_at_offset {
                ...
                let haystack_len_matches = self.func.alloc_vreg();
                self.emit(MirInst::BinOp {
                    dst: haystack_len_matches,
                    op: BinOpKind::Ge,
                    lhs: MirValue::VReg(haystack_source.len_vreg),
                    rhs: MirValue::Const((offset + needle_len) as i64),
                });
                self.vreg_type_hints
                    .insert(haystack_len_matches, MirType::Bool);
                ...
                let length_matches = self.func.alloc_vreg();
                self.emit(MirInst::BinOp {
                    dst: length_matches,
                    op: BinOpKind::And,
                    lhs: MirValue::VReg(needle_len_matches),
                    rhs: MirValue::VReg(haystack_len_matches),
                });
                self.vreg_type_hints.insert(length_matches, MirType::Bool);

                let candidate = self.func.alloc_vreg();
                self.emit(MirInst::BinOp {
                    dst: candidate,
                    op: BinOpKind::And,
                    lhs: MirValue::VReg(length_matches),
                    rhs: MirValue::VReg(bytes_match),
                });
                self.vreg_type_hints.insert(candidate, MirType::Bool);
