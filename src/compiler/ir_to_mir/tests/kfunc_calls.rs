...
#[test]
fn test_helper_d_path_uses_kernel_addr_for_trampoline_struct_field() {
    use crate::compiler::ir_to_mir::tests::helpers::string_member;

    let ctx_var = VarId::new(0);
    let helper_decl = DeclId::new(42);
    let func = HirFunction {
        blocks: vec![HirBlock {
            id: HirBlockId(0),
            stmts: vec![
                ...
                HirStmt::LoadLiteral {
                    dst: RegId::new(2),
                    lit: HirLiteral::String(b"bpf_d_path".to_vec()),
                },
                HirStmt::LoadLiteral {
                    dst: RegId::new(3),
                    lit: HirLiteral::String(
    ...
    let hir_program = HirProgram::new(func, HashMap::new(), vec![], Some(ctx_var));
    let decl_names = HashMap::from([(helper_decl, "helper-call".to_string())]);
    let probe_ctx = ProbeContext::new(EbpfProgramType::Fentry, "security_file_open");
    ...
    .expect("bpf_d_path should use original kernel field address");
    ...
    let call_args = result
        ...
        .flat_map(|block| block.instructions.iter())
        .find_map(|inst| match inst {
            MirInst::CallHelper { helper, args, .. } if *helper == BpfHelper::DPath as u32 => {
                Some(args.clone())
            }
            _ => None,
        })
        .expect("expected bpf_d_path helper call");

    let MirValue::VReg(path_arg) = call_args[0] else {
        panic!("expected bpf_d_path arg0 to be a vreg");
    };
    assert!(
        matches!(
            result.type_hints.main.get(&path_arg),
            Some(MirType::Ptr {
                pointee,
                address_space: AddressSpace::Kernel,
            }) if matches!(
...
#[test]
fn test_helper_call_timer_set_callback_requires_projected_map_timer_field() {
    let closure_block_id = nu_protocol::BlockId::new(6);
    let closure = HirFunction {
        blocks: vec![HirBlock {
            id: HirBlockId(0),
            stmts: vec![HirStmt::LoadLiteral {
    ...
    .expect_err("expected timer_set_callback helper-call to reject non-map timer pointer");

    match err {
        CompileError::UnsupportedInstruction(msg) => assert!(
            msg.contains("bpf_timer_set_callback")
                && msg.contains(
                    "requires arg0 to be a bpf_timer field projected from a concrete map value",
                ),
...
#[test]
fn test_helper_call_timer_set_callback_closure_lowers_to_callback_subprogram() {
        ...
        blocks: vec![HirBlock {
            ...
            stmts: vec![
                ...
                HirStmt::Call {
                    ...
                    src_dst: RegId::new(7),
                    args: HirCallArgs {
                        positional: vec![RegId::new(5), RegId::new(0), RegId::new(6)],
                        ..Default::default()
                    },
                },
            ],
    ...
    let mut decl_names = HashMap::new();
    decl_names.insert(DeclId::new(42), "map-get".to_string());
    decl_names.insert(DeclId::new(43), "helper-call".to_string());
    let result = lower_hir_to_mir_with_hints_and_maps(
        &hir_program,
        None,
        &decl_names,
        None,
        Some(&HashMap::from([(map_ref.clone(), value_ty.clone())])),
        &HashMap::new(),
        &HashMap::new(),
    )
    .expect("bpf_timer_set_callback helper-call with projected timer field should lower");

    assert_eq!(result.program.subfunctions.len(), 1);
    assert_eq!(result.program.subfunctions[0].param_count, 3);
    let entry = result.program.main.entry;
    let block = result.program.main.block(entry);
    assert!(block.instructions.iter().any(|inst| matches!(
        inst,
...
fn test_helper_call_timer_init_reuses_observed_map_kind_for_projected_timer_field() {
    ...
    let main = HirFunction {
        blocks: vec![HirBlock {
            ...
            stmts: vec![
                ...
                HirStmt::LoadLiteral {
                    dst: RegId::new(5),
                    lit: HirLiteral::String(b"bpf_timer_init".to_vec()),
                },
                HirStmt::LoadLiteral {
                    dst: RegId::new(7),
                    lit: HirLiteral::Int(1),
                ...
                HirStmt::LoadLiteral {
                    dst: RegId::new(13),
                    lit: HirLiteral::String(b"bpf_timer_cancel".to_vec()),
                },
                HirStmt::Call {
                    decl_id: DeclId::new(43),
                    src_dst: RegId::new(14),
                    args: HirCallArgs {
                        positional: vec![RegId::new(13), RegId::new(0)],
                        ..Default::default()
                    },
                },
            ],
    ...
    .expect("timer init/start/cancel helper-calls should lower");
    ...
    assert!(block.instructions.iter().any(|inst| matches!(
        ...
        MirInst::CallHelper {
            ...
            args,
            ..
        } if *helper == BpfHelper::TimerInit as u32 && args.len() == 3
    )));
    assert!(block.instructions.iter().any(|inst| matches!(
        inst,
        MirInst::CallHelper {
            ...
            args,
            ..
        } if *helper == BpfHelper::TimerStart as u32 && args.len() == 3
    )));
    assert!(block.instructions.iter().any(|inst| matches!(
        inst,
        MirInst::CallHelper {
...
#[test]
fn test_helper_call_timer_init_rejects_mismatched_map() {
    use nu_protocol::ast::CellPath;
    ...
    let main = HirFunction {
        blocks: vec![HirBlock {
            ...
            stmts: vec![
                ...
                HirStmt::LoadLiteral {
                    dst: RegId::new(6),
                    lit: HirLiteral::String(b"other_timers".to_vec()),
                },
                HirStmt::LoadLiteral {
                    dst: RegId::new(7),
                    lit: HirLiteral::Int(1),
    ...
    .expect_err("timer_init should reject map mismatch");

    match err {
        CompileError::UnsupportedInstruction(msg) => assert!(
            msg.contains("bpf_timer_init")
                && msg.contains("requires arg1 map 'other_timers'")
                && msg.contains("demo_timers"),
            "unexpected error: {msg}"
        ),
        other => panic!("unexpected error: {other:?}"),
    }
...
#[test]
fn test_kfunc_call_bpf_wq_init_materializes_owning_map_fd() {
    use nu_protocol::ast::CellPath;
    ...
    .expect("bpf_wq_init kfunc-call should lower");
    ...
    let main = HirFunction {
        blocks: vec![HirBlock {
            ...
            stmts: vec![
                ...
                HirStmt::LoadLiteral {
                    dst: RegId::new(5),
                    lit: HirLiteral::String(b"bpf_wq_set_callback_impl".to_vec()),
                },
                HirStmt::LoadLiteral {
                    dst: RegId::new(6),
                    lit: HirLiteral::Closure(closure_block_id),
...
fn test_assign_socket_intrinsic_rejects_tc_egress_at_intrinsic_validation() {
    ...
    let hir_types = infer_hir_types(&hir_program, &decl_names)
        .expect("assign-socket intrinsic should type-check before attach-aware validation");
    ...
    .expect_err("assign-socket should reject tc egress before helper lowering");

    match err {
        CompileError::UnsupportedInstruction(msg) => {
            assert!(msg.contains("assign-socket is not supported on tc programs"));
            assert!(
                msg.contains("helper 'bpf_sk_assign' is only valid in tc/tcx ingress programs")
            );
        }
        other => panic!("unexpected error: {:?}", other),
    }
