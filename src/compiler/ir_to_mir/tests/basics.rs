...
use crate::compiler::mir::{AddressSpace, BYTES_COUNTER_MAP_NAME, BlockId, COUNTER_MAP_NAME};
...
fn integer_range_expr_match_pattern_with_step(
    start: i64,
    next: i64,
    end: Option<i64>,
    inclusion: RangeInclusion,
    ...
    let emitted_eq = program
        ...
        .any(|inst| {
            matches!(
                ...
                MirInst::BinOp {
                    op: BinOpKind::Eq,
                    rhs: MirValue::Const(42),
                    ..
                }
            )
        });
    assert!(emitted_eq, "integer match should emit scalar equality");
...
#[test]
fn test_lower_match_integer_explicit_step_range_pattern_compiles() {
    let hir = make_integer_match_program(integer_range_expr_match_pattern_with_step(
        0,
        2,
        Some(10),
        RangeInclusion::Inclusive,
    ...
    let program = lower_hir_to_mir(&hir, None, &HashMap::new())
        .expect("integer explicit-step range match pattern should lower to MIR");
    let instructions: Vec<_> = program
        .main
        .blocks
        .iter()
    ...
    compile_mir_to_ebpf_with_hints(&program, None, None)
        .expect("integer explicit-step range match pattern should compile through codegen");
}

#[test]
fn test_lower_match_integer_right_exclusive_explicit_step_range_pattern_compiles() {
    let hir = make_integer_match_program(integer_range_expr_match_pattern_with_step(
        0,
        2,
        Some(10),
    ...
    let program = lower_hir_to_mir(&hir, None, &HashMap::new())
        .expect("integer right-exclusive explicit-step range match pattern should lower to MIR");
    let instructions: Vec<_> = program
        .main
        .blocks
        .iter()
    ...
    compile_mir_to_ebpf_with_hints(&program, None, None).expect(
        "integer right-exclusive explicit-step range match pattern should compile through codegen",
    );
}

#[test]
fn test_lower_match_integer_open_upper_explicit_step_range_pattern_compiles() {
    let hir = make_integer_match_program(integer_range_expr_match_pattern_with_step(
        0,
        2,
        None,
    ...
    let program = lower_hir_to_mir(&hir, None, &HashMap::new())
        .expect("integer open-upper explicit-step range match pattern should lower to MIR");
    let instructions: Vec<_> = program
        .main
        .blocks
        .iter()
    ...
    compile_mir_to_ebpf_with_hints(&program, None, None).expect(
        "integer open-upper explicit-step range match pattern should compile through codegen",
    );
}

#[test]
fn test_lower_match_integer_zero_step_range_pattern_rejects() {
    let hir = make_integer_match_program(integer_range_expr_match_pattern_with_step(
        10,
        10,
        Some(12),
    ...
    let err = lower_hir_to_mir(&hir, None, &HashMap::new())
        .expect_err("integer zero-step range match pattern should be rejected");
    assert!(
        err.to_string().contains("non-zero explicit step in eBPF"),
        "unexpected error: {err}"
    );
}

#[test]
fn test_lower_match_integer_descending_explicit_step_range_pattern_is_false() {
    let hir = make_integer_match_program(integer_range_expr_match_pattern_with_step(
        10,
        7,
        Some(0),
    ...
    let program = lower_hir_to_mir(&hir, None, &HashMap::new())
        .expect("integer descending explicit-step range match pattern should lower to MIR");
    let entry = &program.main.blocks[0];
    assert!(matches!(
        entry.terminator,
        MirInst::Jump { target: BlockId(2) }
    ));
    ...
    compile_mir_to_ebpf_with_hints(&program, None, None).expect(
        "integer descending explicit-step range match pattern should compile through codegen",
    );
}
...
#[test]
fn test_lower_dynamic_bounded_end_range_iterate_uses_runtime_guard() {
    let random_decl = DeclId::new(29_220);
    let bits_decl = DeclId::new(29_221);
    let func = HirFunction {
        blocks: vec![
            HirBlock {
                id: HirBlockId(0),
    ...
    .expect("dynamic bounded-end range should lower");
    ...
    let (counter, start, step, limit, guard_block, exit_block) = result
        ...
        .expect("expected dynamic bounded-end loop header");
    assert_eq!(start, 1);
    assert_eq!(step, 1);
    assert_eq!(limit, 4);
    ...
    let (body_block, branch_exit) = match guard.terminator {
        ...
        ref other => panic!("expected dynamic range guard branch, got {other:?}"),
    };
        ...
        block.instructions.iter().any(|inst| {
            matches!(
                ...
                MirInst::CallHelper {
                    helper,
                    ..
                } if *helper == BpfHelper::GetPrandomU32 as u32
            )
        })
    }));
    assert!(result.program.main.blocks.iter().any(|block| {
    ...
    compile_mir_to_ebpf_with_hints(&result.program, None, Some(&result.type_hints))
        .expect("dynamic bounded-end range should compile through codegen");
}
...
#[test]
fn test_lower_kprobe_comm_str_join_scalar_passthrough() {
    let join_decl = DeclId::new(25_110);
    let length_decl = DeclId::new(25_111);
    let ctx_var = VarId::new(0);
    let hir = HirProgram::new(
        HirFunction {
            blocks: vec![HirBlock {
                ...
                stmts: vec![
                    ...
                    HirStmt::LoadLiteral {
                        dst: RegId::new(2),
                        lit: HirLiteral::String(b":".to_vec()),
                    },
                    HirStmt::Call {
                        decl_id: join_decl,
                        src_dst: RegId::new(3),
    ...
    .expect("str join should pass through tracked scalar strings");
    ...
    assert!(
        ...
        "expected ctx.comm to remain a tracked stack string"
    );
    compile_mir_to_ebpf_with_hints(&result.program, Some(&probe_ctx), Some(&result.type_hints))
        .expect("str join scalar passthrough consumed by str length should compile");
}
...
#[test]
fn test_lower_xdp_eth_ipv4_icmp_echo_sequence_projection_uses_be16_load() {
    let hir = make_ctx_path_program(CellPath {
        members: vec![
            string_member("data"),
            string_member("eth"),
    ...
    .expect("xdp eth ipv4 icmp echo_sequence projection should lower");
...
#[test]
fn test_lower_xdp_eth_ipv4_icmp_next_hop_mtu_projection_uses_be16_load() {
    let hir = make_ctx_path_program(CellPath {
        members: vec![
            string_member("data"),
            string_member("eth"),
    ...
    .expect("xdp eth ipv4 icmp next_hop_mtu projection should lower");
...
#[test]
fn test_lower_xdp_eth_ipv4_icmp_timestamp_projection_uses_be32_load() {
    let hir = make_ctx_path_program(CellPath {
        members: vec![
            string_member("data"),
            string_member("eth"),
            string_member("ipv4"),
            string_member("icmp"),
            string_member("receive_timestamp"),
        ],
    });
    let probe_ctx = ProbeContext::new(EbpfProgramType::Xdp, "lo");
    ...
    .expect("xdp eth ipv4 icmp receive_timestamp projection should lower");
...
#[test]
fn test_lower_xdp_eth_ipv6_icmpv6_mtu_projection_uses_be32_load() {
    let hir = make_ctx_path_program(CellPath {
        members: vec![
            string_member("data"),
            string_member("eth"),
    ...
    .expect("xdp eth ipv6 icmpv6 mtu projection should lower");
...
#[test]
fn test_lower_xdp_eth_ipv6_icmpv6_neighbor_advertisement_flag_extracts_bitfield() {
    let hir = make_ctx_path_program(CellPath {
        members: vec![
            string_member("data"),
            string_member("eth"),
    ...
    .expect("xdp eth ipv6 icmpv6 na_solicited projection should lower");
...
#[test]
fn test_lower_record_spread_preserves_fields_through_local_binding() {
    let rec_var = VarId::new(80);
    let func = HirFunction {
        blocks: vec![HirBlock {
            id: HirBlockId(0),
            stmts: vec![
    ...
    .expect("record spread through a local binding should preserve compiler-known fields");
    ...
    assert!(
        result
            ...
            .flat_map(|block| block.instructions.iter())
            .any(|inst| matches!(inst, MirInst::StringAppend { .. })),
        "expected spread field loaded through a local binding to retain string semantics"
    );
}
...
#[test]
fn test_lower_metadata_record_numeric_list_index_uses_list_get() {
    let func = HirFunction {
        blocks: vec![HirBlock {
            id: HirBlockId(0),
            stmts: vec![
    ...
    .expect("metadata-only numeric list cell path projection should lower");
    ...
    assert!(
        ...
        "expected metadata-backed $rec.a.0 projection to lower through ListGet, not raw array index 0"
    );
}

#[test]
fn test_lower_nested_metadata_record_numeric_list_index_uses_list_get() {
    let func = HirFunction {
        blocks: vec![HirBlock {
            id: HirBlockId(0),
            stmts: vec![
                ...
                HirStmt::LoadLiteral {
                    dst: RegId::new(1),
                    lit: HirLiteral::CellPath(Box::new(CellPath {
                        members: vec![string_member("a"), string_member("b"), int_member(0)],
                    })),
                },
                HirStmt::LoadLiteral {
                    dst: RegId::new(2),
    ...
    .expect("nested metadata-only numeric list cell path projection should lower");
    ...
    assert!(
        ...
        "expected metadata-backed $rec.a.b.0 projection to lower through ListGet, not raw array index 0"
    );
}
...
#[test]
fn test_lower_nested_metadata_record_numeric_list_append_grows_capacity() {
    let func = HirFunction {
        blocks: vec![HirBlock {
            id: HirBlockId(0),
            stmts: vec![
                ...
                HirStmt::LoadLiteral {
                    dst: RegId::new(3),
                    lit: HirLiteral::CellPath(Box::new(CellPath {
                        members: vec![string_member("a"), string_member("b"), int_member(1)],
                    })),
                },
                HirStmt::LoadLiteral {
                    dst: RegId::new(4),
    ...
    .expect("nested metadata-only numeric list append should grow capacity and lower");
    ...
    assert!(
        ...
        "expected appended $rec.a.b.1 projection to lower through ListGet with logical index 1"
    );
}

#[test]
fn test_lower_nested_metadata_record_numeric_list_existing_index_upsert_skips_length_slot() {
    let func = HirFunction {
        blocks: vec![HirBlock {
            id: HirBlockId(0),
            stmts: vec![
    ...
    .expect("nested metadata-only numeric list existing-index upsert should lower");
    ...
    assert!(
        ...
        "expected $rec.a.b.0 = ... to store at first nested list item slot offset 8, not the length slot"
    );
}
...
#[test]
fn test_lower_leading_annotated_mut_empty_fixed_array_rejects_with_layout_hint() {
    ...
    hir.annotated_mut_globals = vec![AnnotatedMutGlobal {
        ...
        initial_value: Value::list(vec![], Span::test_data()),
    }];
    ...
    .expect_err("empty annotated mutable fixed arrays should explain missing layout");

    assert!(
        err.to_string().contains(
            "annotated mutable fixed-array global initializer is empty and cannot infer element layout"
        ),
        "unexpected error: {err}"
    );
    assert!(
        err.to_string()
            .contains("global-define --type 'array{...:N}'"),
        "unexpected error: {err}"
    );
}
...
#[test]
fn test_lower_leading_annotated_mut_record_partial_initializer_reports_nested_omitted_field_path() {
    let global_var = VarId::new(363);
    let func = HirFunction {
        blocks: vec![HirBlock {
            id: HirBlockId(0),
    ...
    let mut initial = Record::new();
    initial.push("stats", Value::record(Record::new(), Span::test_data()));
    ...
    .expect_err("nested omitted list field without explicit capacity should be rejected");

    assert!(
        err.to_string()
            .contains("omitted record field 'stats.samples'"),
        "unexpected error: {err}"
    );
    assert!(
        err.to_string()
    ...
    assert!(
        err.to_string()
            .contains("provide a concrete value for 'stats.samples'"),
        "unexpected error: {err}"
    );
}
...
#[test]
fn test_lower_leading_annotated_mut_null_fixed_array_list_is_rejected_helpfully() {
    let global_var = VarId::new(358);
    let func = HirFunction {
        blocks: vec![HirBlock {
            id: HirBlockId(0),
            stmts: vec![HirStmt::LoadVariable {
    ...
    .expect_err("annotated mutable null fixed-array list should be rejected");

    assert!(
        err.to_string()
            .contains("plain Nushell list type annotations do not carry fixed-array length"),
        "unexpected error: {err}"
    );
    assert!(
        err.to_string().contains("concrete list initializer"),
        "unexpected error: {err}"
    );
    assert!(
        err.to_string()
...
#[test]
fn test_lower_leading_annotated_mut_null_record_with_fixed_array_field_is_rejected_helpfully() {
    let global_var = VarId::new(359);
    let func = HirFunction {
        blocks: vec![HirBlock {
            id: HirBlockId(0),
            stmts: vec![HirStmt::LoadVariable {
    ...
    .expect_err("annotated mutable null record with fixed-array field should be rejected");

    assert!(
        err.to_string().contains("nested field 'entries'"),
        "unexpected error: {err}"
    );
    assert!(
        err.to_string().contains("explicit fixed-array length"),
        "unexpected error: {err}"
    );
    assert!(
        err.to_string().contains("array{...:N}"),
        "unexpected error: {err}"
    );
}
