...
fn make_record_spread_context_upsert_program(
    record_field: &str,
    path: CellPath,
    lit: HirLiteral,
) -> HirProgram {
...
fn make_nested_returned_record_spread_context_upsert_program(
    record_field: &str,
    path: CellPath,
    lit: HirLiteral,
) -> (HirProgram, HashMap<DeclId, HirFunction>) {
    let ctx_var = VarId::new(0);
    let inner_arg = VarId::new(10);
    let outer_arg = VarId::new(20);
    let inner_decl = DeclId::new(1);
    let outer_decl = DeclId::new(2);
    let mut members = vec![string_member(record_field)];
    members.extend(path.members);
    ...
    let mut user_functions = HashMap::new();
    user_functions.insert(inner_decl, inner_func);
    user_functions.insert(outer_decl, outer_func);
    (hir, user_functions)
}
...
fn make_transformed_record_context_upsert_program(
    transform: ContextRecordTransform,
    command_decl: DeclId,
) -> HirProgram {
    let ctx_var = VarId::new(0);
    let mut stmts = Vec::new();
    let output_field = match transform {
        ContextRecordTransform::Rename => "alias",
        _ => "event",
    };
    ...
    match transform {
        ...
        | ContextRecordTransform::Rename => {
            ...
            if matches!(transform, ContextRecordTransform::Reject) {
                stmts.push(HirStmt::LoadLiteral {
                    dst: RegId::new(3),
            ...
            let arg_name = match transform {
                ContextRecordTransform::Select => "event",
                ContextRecordTransform::Reject => "other",
                ContextRecordTransform::Rename => "alias",
                _ => unreachable!(),
            };
            stmts.push(HirStmt::LoadLiteral {
                dst: RegId::new(5),
                lit: HirLiteral::String(arg_name.as_bytes().to_vec()),
            });
            stmts.push(HirStmt::Call {
        ...
        ContextRecordTransform::Default => {
            stmts.push(HirStmt::LoadLiteral {
                dst: RegId::new(0),
                lit: HirLiteral::Record { capacity: 0 },
            });
            ...
            stmts.push(HirStmt::Call {
                ...
                src_dst: RegId::new(8),
                args: HirCallArgs {
                    positional: vec![RegId::new(2), RegId::new(5)],
                    pipeline_input: Some(RegId::new(0)),
                    ..HirCallArgs::default()
                },
            });
    ...
    stmts.push(HirStmt::LoadLiteral {
        ...
        lit: HirLiteral::CellPath(Box::new(CellPath {
            members: vec![string_member(output_field), string_member("new_value")],
        })),
    });
    stmts.push(HirStmt::LoadLiteral {
        dst: RegId::new(10),
...
fn assert_transformed_record_sysctl_new_value_metadata(
    ...
    assert!(
        ...
        "{context} should preserve ctx.new_value metadata through lowering"
    );
    assert!(
        ...
        "{context} should lower record-held ctx.new_value assignment to bpf_sysctl_set_new_value"
    );
    ...
    assert!(
        ...
        "{context} should report bpf_sysctl_set_new_value compatibility metadata"
    );
    assert!(
        program
        ...
        "{context} should report sysctl_new_value context compatibility metadata"
    );
}
...
#[test]
fn test_lower_select_record_context_sysctl_new_value_assignment_preserves_metadata() {
    let select_decl = DeclId::new(901);
    let hir =
        make_transformed_record_context_upsert_program(ContextRecordTransform::Select, select_decl);
    let decl_names = HashMap::from([(select_decl, "select".to_string())]);

    assert_transformed_record_sysctl_new_value_metadata(
        hir,
        decl_names,
        "select-transformed record-held ctx.new_value assignment",
    );
}

#[test]
fn test_lower_reject_record_context_sysctl_new_value_assignment_preserves_metadata() {
    let reject_decl = DeclId::new(902);
    let hir =
        make_transformed_record_context_upsert_program(ContextRecordTransform::Reject, reject_decl);
    let decl_names = HashMap::from([(reject_decl, "reject".to_string())]);

    assert_transformed_record_sysctl_new_value_metadata(
        hir,
        decl_names,
        "reject-transformed record-held ctx.new_value assignment",
    );
}

#[test]
fn test_lower_rename_record_context_sysctl_new_value_assignment_preserves_metadata() {
    let rename_decl = DeclId::new(903);
    let hir =
        make_transformed_record_context_upsert_program(ContextRecordTransform::Rename, rename_decl);
    let decl_names = HashMap::from([(rename_decl, "rename".to_string())]);

    assert_transformed_record_sysctl_new_value_metadata(
        hir,
        decl_names,
        "rename-transformed record-held ctx.new_value assignment",
    );
}

#[test]
fn test_lower_merge_record_context_sysctl_new_value_assignment_preserves_metadata() {
    let merge_decl = DeclId::new(904);
    let hir =
        make_transformed_record_context_upsert_program(ContextRecordTransform::Merge, merge_decl);
    let decl_names = HashMap::from([(merge_decl, "merge".to_string())]);

    assert_transformed_record_sysctl_new_value_metadata(
        hir,
        decl_names,
        "merge-transformed record-held ctx.new_value assignment",
    );
}

#[test]
fn test_lower_default_record_context_sysctl_new_value_assignment_preserves_metadata() {
    let default_decl = DeclId::new(905);
    let hir = make_transformed_record_context_upsert_program(
        ContextRecordTransform::Default,
        default_decl,
    );
...
#[test]
fn test_lower_record_spread_context_sun_path_assignment_records_kfunc_metadata() {
    let hir = make_record_spread_context_upsert_program(
        "event",
        CellPath {
            members: vec![string_member("sun_path")],
        },
    ...
    .expect("record-spread cgroup_sock_addr ctx.sun_path assignment should lower");
    ...
    let compiled =
        compile_mir_to_ebpf_with_hints(&result.program, Some(&probe_ctx), Some(&result.type_hints))
            .expect("record-spread cgroup_sock_addr ctx.sun_path assignment should compile");
    assert!(compiled.used_kfuncs.contains("bpf_sock_addr_set_sun_path"));
    ...
    let requirement = program
        ...
        .find(|requirement| requirement.name() == "bpf_sock_addr_set_sun_path")
        .expect(
            "record-spread ctx.sun_path assignment should report bpf_sock_addr_set_sun_path metadata",
        );
    assert_eq!(requirement.minimum_kernel(), "6.7");
    assert_eq!(program.kfunc_compatibility_minimum_kernel(), Some("6.7"));
    assert_eq!(program.compatibility_minimum_kernel(), Some("6.7"));
...
#[test]
fn test_lower_nested_record_spread_context_sun_path_assignment_records_kfunc_metadata() {
    let (hir, user_functions) = make_nested_returned_record_spread_context_upsert_program(
        "event",
        CellPath {
            members: vec![string_member("sun_path")],
        },
    ...
    .expect("nested returned record-spread cgroup_sock_addr ctx.sun_path assignment should lower");
    ...
    .expect(
        "nested returned record-spread cgroup_sock_addr ctx.sun_path assignment should compile",
    );
    assert!(compiled.used_kfuncs.contains("bpf_sock_addr_set_sun_path"));
    ...
    let requirement = program
        ...
        .find(|requirement| requirement.name() == "bpf_sock_addr_set_sun_path")
        .expect(
            "nested returned record-spread ctx.sun_path assignment should report bpf_sock_addr_set_sun_path metadata",
        );
    assert_eq!(requirement.minimum_kernel(), "6.7");
    assert_eq!(program.kfunc_compatibility_minimum_kernel(), Some("6.7"));
    assert_eq!(program.compatibility_minimum_kernel(), Some("6.7"));
...
fn test_lower_cgroup_sock_ctx_socket_family_field() {
    for root in ["sk", "sock", "socket"] {
        ...
        .unwrap_or_else(|err| panic!("cgroup_sock ctx.{root}.family should lower: {err}"));
...
fn test_lower_cgroup_sockopt_ctx_socket_family_field() {
    for root in ["sk", "sock", "socket"] {
        ...
        .unwrap_or_else(|err| panic!("cgroup_sockopt ctx.{root}.family should lower: {err}"));
...
fn test_lower_cgroup_sock_addr_ctx_socket_family_field() {
    for root in ["sk", "sock", "socket"] {
        ...
        .unwrap_or_else(|err| panic!("cgroup_sock_addr ctx.{root}.family should lower: {err}"));
...
#[test]
fn test_lower_record_spread_context_sock_ops_cb_flags_assignment_preserves_metadata() {
    let hir = make_record_spread_context_upsert_program(
        "event",
        CellPath {
            members: vec![string_member("cb_flags")],
    ...
    .expect("record-spread sock_ops ctx.cb_flags assignment should lower");
    ...
    assert!(
        result
            ...
            .used_ctx_fields
            .contains(&CtxField::SockOpsCbFlags),
        "record-spread ctx.cb_flags assignment should preserve context compatibility metadata"
    );
    assert!(result.program.main.blocks.iter().any(|block| {
        block.instructions.iter().any(|inst| {
            matches!(
    ...
    let compiled =
        compile_mir_to_ebpf_with_hints(&result.program, Some(&probe_ctx), Some(&result.type_hints))
            .expect("record-spread sock_ops ctx.cb_flags assignment should compile");
    assert!(compiled.used_ctx_fields.contains(&CtxField::SockOpsCbFlags));
}

#[test]
fn test_lower_nested_record_spread_context_sock_ops_cb_flags_assignment_preserves_metadata() {
    let (hir, user_functions) = make_nested_returned_record_spread_context_upsert_program(
        "event",
        CellPath {
            members: vec![string_member("cb_flags")],
    ...
    .expect("nested returned record-spread sock_ops ctx.cb_flags assignment should lower");
    ...
    assert!(
        result
            ...
            .used_ctx_fields
            .contains(&CtxField::SockOpsCbFlags),
        "nested returned record-spread ctx.cb_flags assignment should preserve context compatibility metadata"
    );
    assert!(result.program.main.blocks.iter().any(|block| {
        block.instructions.iter().any(|inst| {
            matches!(
    ...
    let compiled =
        compile_mir_to_ebpf_with_hints(&result.program, Some(&probe_ctx), Some(&result.type_hints))
            .expect(
                "nested returned record-spread sock_ops ctx.cb_flags assignment should compile",
            );
    assert!(compiled.used_ctx_fields.contains(&CtxField::SockOpsCbFlags));
}
...
#[test]
fn test_lower_record_context_field_projection_preserves_context_metadata() {
    let ctx_var = VarId::new(0);
    let hir = HirProgram::new(
        HirFunction {
            blocks: vec![HirBlock {
                ...
                stmts: vec![
                    ...
                    HirStmt::LoadLiteral {
                        dst: RegId::new(3),
                        lit: HirLiteral::CellPath(Box::new(CellPath {
                            members: vec![string_member("k"), string_member("pid")],
                        })),
                    },
                    HirStmt::FollowCellPath {
                        src_dst: RegId::new(0),
    ...
    .expect("record-held context field projection should lower");
...
#[test]
fn test_lower_record_context_map_get_rejects_pointer_key_escape() {
    let ctx_var = VarId::new(0);
                ...
                stmts: vec![
                    ...
                    HirStmt::Call {
                        ...
                        args: HirCallArgs {
                            ...
                            named: vec![(b"kind".to_vec(), RegId::new(4))],
                            ..HirCallArgs::default()
                        },
                    },
                ],
    ...
    .expect_err("context pointers inside records must not escape into map keys");

    assert!(
        err.to_string()
            .contains("map-get key cannot use context pointers as values")
    );
}

#[test]
fn test_lower_record_context_global_set_rejects_pointer_escape() {
    let ctx_var = VarId::new(0);
    let global_set_decl = DeclId::new(42);
    let decl_names = HashMap::from([(global_set_decl, "global-set".to_string())]);
    let hir = HirProgram::new(
        HirFunction {
            blocks: vec![HirBlock {
                id: HirBlockId(0),
                stmts: vec![
                    ...
                    HirStmt::LoadLiteral {
                        dst: RegId::new(3),
                        lit: HirLiteral::String(b"state".to_vec()),
                    },
                    HirStmt::Call {
    ...
    .expect_err("context pointers inside records must not escape into globals");
...
#[test]
fn test_lower_record_tc_ctx_data_byte_assignment_adds_guarded_packet_store() {
    let ctx_var = VarId::new(0);
    let hir = HirProgram::new(
        HirFunction {
            blocks: vec![HirBlock {
                ...
                stmts: vec![
                    ...
                    HirStmt::LoadLiteral {
                        dst: RegId::new(1),
                        lit: HirLiteral::String(b"data".to_vec()),
                    },
                    HirStmt::LoadVariable {
                        dst: RegId::new(2),
                        var_id: ctx_var,
    ...
    .expect("record-held tc ctx.data byte assignment should lower");
    ...
    assert!(
        ...
        "expected record-held ctx.data alias to originate from the context field"
    );
    assert!(
        blocks
            .iter()
        ...
        "expected record-held ctx.data byte write to guard against data_end"
    );
    assert!(
        blocks
            .iter()
        ...
        "expected record-held ctx.data byte write to emit a guarded u8 store"
    );
}
