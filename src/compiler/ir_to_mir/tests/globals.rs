...
#[test]
fn test_lower_global_get_string_contains_operators_compare_two_runtime_strings() {
    let define_decl = DeclId::new(10_660);
    let get_decl = DeclId::new(10_661);
    let decl_names = HashMap::from([
        (define_decl, "global-define".to_string()),
        (get_decl, "global-get".to_string()),
    ]);
    ...
    for (op, context) in [
        (nu_protocol::ast::Comparison::In, "in"),
        (nu_protocol::ast::Comparison::NotIn, "not-in"),
        (nu_protocol::ast::Comparison::Has, "has"),
        (nu_protocol::ast::Comparison::NotHas, "not-has"),
    ] {
        let func = HirFunction {
            blocks: vec![HirBlock {
                id: HirBlockId(0),
                stmts: vec![
                    ...
                    HirStmt::BinaryOp {
                        lhs_dst: RegId::new(5),
                        op: nu_protocol::ast::Operator::Comparison(op),
                        rhs: RegId::new(6),
                    },
                ],
                terminator: HirTerminator::Return { src: RegId::new(5) },
        ...
        .unwrap_or_else(|err| {
            panic!("{context} operator should compare two runtime strings: {err}")
        });
        ...
        assert!(
            result
                ...
                .flat_map(|block| block.instructions.iter())
                .any(|inst| matches!(inst, MirInst::StrCmp { len: 1, .. })),
            "expected {context} operator to compare one-byte substring candidates"
        );
        assert!(
            result
                .program
            ...
            "expected {context} operator to compare multi-byte substring candidates"
        );
        compile_mir_to_ebpf_with_hints(&result.program, None, Some(&result.type_hints))
...
#[test]
fn test_lower_global_get_string_starts_with_runtime_prefix() {
    let define_decl = DeclId::new(10_652);
    let get_decl = DeclId::new(10_653);
    let decl_names = HashMap::from([
        (define_decl, "global-define".to_string()),
        (get_decl, "global-get".to_string()),
    ]);
        ...
        blocks: vec![HirBlock {
            ...
            stmts: vec![
                ...
                HirStmt::BinaryOp {
                    lhs_dst: RegId::new(5),
                    op: nu_protocol::ast::Operator::Comparison(
                        nu_protocol::ast::Comparison::StartsWith,
                    ),
                    rhs: RegId::new(6),
                },
                HirStmt::Call {
                ...
                HirStmt::BinaryOp {
                    lhs_dst: RegId::new(7),
                    op: nu_protocol::ast::Operator::Comparison(
                        nu_protocol::ast::Comparison::NotStartsWith,
                    ),
                    rhs: RegId::new(8),
                },
            ],
    ...
    .expect("runtime starts-with prefixes should lower through guarded byte checks");
    ...
    assert!(
        result
            ...
            .iter()
            .any(|block| matches!(block.terminator, MirInst::Branch { .. })),
        "expected runtime-prefix starts-with to guard byte comparisons"
    );
    assert!(
        result
            .program
            ...
            .flat_map(|block| block.instructions.iter())
            .any(|inst| matches!(inst, MirInst::StrCmp { len: 1, .. })),
        "expected runtime-prefix starts-with to compare bounded bytes"
    );
    assert!(
        result
            .program
        ...
        "expected runtime-prefix starts-with to compare dynamic lengths"
    );
    compile_mir_to_ebpf_with_hints(&result.program, None, Some(&result.type_hints))
        .expect("runtime-prefix starts-with should compile through codegen");
}
...
#[test]
fn test_lower_global_get_string_ends_with_runtime_suffix() {
    let define_decl = DeclId::new(10_656);
    let get_decl = DeclId::new(10_657);
    let decl_names = HashMap::from([
        (define_decl, "global-define".to_string()),
        (get_decl, "global-get".to_string()),
    ]);
        ...
        blocks: vec![HirBlock {
            ...
            stmts: vec![
                ...
                HirStmt::BinaryOp {
                    lhs_dst: RegId::new(5),
                    op: nu_protocol::ast::Operator::Comparison(
                        nu_protocol::ast::Comparison::EndsWith,
                    ),
                    rhs: RegId::new(6),
                },
                HirStmt::Call {
                ...
                HirStmt::BinaryOp {
                    lhs_dst: RegId::new(7),
                    op: nu_protocol::ast::Operator::Comparison(
                        nu_protocol::ast::Comparison::NotEndsWith,
                    ),
                    rhs: RegId::new(8),
                },
            ],
    ...
    .expect("runtime suffix ends-with operators should lower through guarded candidates");
    ...
    assert!(
        result
            ...
            .iter()
            .any(|block| matches!(block.terminator, MirInst::Branch { .. })),
        "expected runtime-suffix ends-with operators to branch on candidate lengths"
    );
    assert!(
        result
            .program
            ...
            .flat_map(|block| block.instructions.iter())
            .any(|inst| matches!(inst, MirInst::StrCmp { len: 1, .. })),
        "expected runtime-suffix ends-with operators to compare suffix candidates"
    );
    compile_mir_to_ebpf_with_hints(&result.program, None, Some(&result.type_hints))
        .expect("runtime-suffix ends-with operators should compile through codegen");
}
...
#[test]
fn test_lower_typed_bytes_global_default_empty_is_passthrough() {
    let define_decl = DeclId::new(16_910);
    let global_get_decl = DeclId::new(16_911);
    let default_decl = DeclId::new(16_912);
    let bytes_length_decl = DeclId::new(16_913);
    let decl_names = HashMap::from([
        (define_decl, "global-define".to_string()),
        (global_get_decl, "global-get".to_string()),
    ...
    .expect("default --empty on non-empty typed bytes globals should pass through");
    ...
    assert!(
        ...
        "expected bytes length after default --empty to use the declared bytes:N length"
    );
    compile_mir_to_ebpf_with_hints(&result.program, None, Some(&result.type_hints)).expect(
        "typed bytes global default --empty consumed by bytes length should compile through codegen",
    );
}
...
#[test]
fn test_lower_bytes_starts_with_on_typed_bytes_global_array_returns_bool_list() {
    let define_decl = DeclId::new(11_000);
    let global_get_decl = DeclId::new(11_001);
    let starts_with_decl = DeclId::new(11_002);
    let get_decl = DeclId::new(11_003);
    let decl_names = HashMap::from([
        (define_decl, "global-define".to_string()),
        (global_get_decl, "global-get".to_string()),
        (starts_with_decl, "bytes starts-with".to_string()),
    ...
    .expect("bytes starts-with should lower on typed fixed-array binary input");
    ...
    assert!(
        result
            ...
            .flat_map(|block| block.instructions.iter())
            .any(|inst| matches!(inst, MirInst::ListNew { max_len: 2, .. })),
        "expected bytes starts-with on array{{bytes:4:2}} to materialize a two-item bool list"
    );
    assert!(
        result
            .program
        ...
        "expected bytes starts-with to inspect the first byte of each fixed binary element"
    );
    compile_mir_to_ebpf_with_hints(&result.program, None, Some(&result.type_hints)).expect(
        "typed fixed-array bytes starts-with consumed by get should compile through codegen",
    );
}

#[test]
fn test_lower_bytes_ends_with_on_typed_bytes_global_array_returns_bool_list() {
    let define_decl = DeclId::new(11_004);
    let global_get_decl = DeclId::new(11_005);
    let ends_with_decl = DeclId::new(11_006);
    let get_decl = DeclId::new(11_007);
    let decl_names = HashMap::from([
        (define_decl, "global-define".to_string()),
        (global_get_decl, "global-get".to_string()),
        (ends_with_decl, "bytes ends-with".to_string()),
        (get_decl, "get".to_string()),
    ]);
    ...
    .expect("bytes ends-with should lower on typed fixed-array binary input");
    ...
    assert!(
        result
            ...
            .flat_map(|block| block.instructions.iter())
            .any(|inst| matches!(inst, MirInst::ListNew { max_len: 2, .. })),
        "expected bytes ends-with on array{{bytes:4:2}} to materialize a two-item bool list"
    );
    assert!(
        result
            .program
        ...
        "expected bytes ends-with to inspect the last byte of each fixed binary element"
    );
    compile_mir_to_ebpf_with_hints(&result.program, None, Some(&result.type_hints))
        .expect("typed fixed-array bytes ends-with consumed by get should compile through codegen");
}

#[test]
fn test_lower_bytes_index_of_on_typed_bytes_global_array_returns_index_list() {
    let define_decl = DeclId::new(11_008);
    let global_get_decl = DeclId::new(11_009);
    let index_of_decl = DeclId::new(11_010);
    let get_decl = DeclId::new(11_011);
    let decl_names = HashMap::from([
        (define_decl, "global-define".to_string()),
        (global_get_decl, "global-get".to_string()),
        (index_of_decl, "bytes index-of".to_string()),
    ...
    .expect("bytes index-of should lower on typed fixed-array binary input");
    ...
    assert!(
        result
            ...
            .flat_map(|block| block.instructions.iter())
            .any(|inst| matches!(inst, MirInst::ListNew { max_len: 2, .. })),
        "expected bytes index-of on array{{bytes:4:2}} to materialize a two-item index list"
    );
    assert!(
        result
            .program
        ...
        "expected bytes index-of to inspect the first candidate byte of each fixed binary element"
    );
    compile_mir_to_ebpf_with_hints(&result.program, None, Some(&result.type_hints))
        .expect("typed fixed-array bytes index-of consumed by get should compile through codegen");
}

#[test]
fn test_lower_bytes_index_of_end_on_typed_bytes_global_array_returns_index_list() {
    let define_decl = DeclId::new(11_012);
    let global_get_decl = DeclId::new(11_013);
    let index_of_decl = DeclId::new(11_014);
    let get_decl = DeclId::new(11_015);
    let decl_names = HashMap::from([
        (define_decl, "global-define".to_string()),
        (global_get_decl, "global-get".to_string()),
        (index_of_decl, "bytes index-of".to_string()),
    ...
    .expect("bytes index-of --end should lower on typed fixed-array binary input");
    ...
    assert!(
        result
            ...
            .flat_map(|block| block.instructions.iter())
            .any(|inst| matches!(inst, MirInst::ListNew { max_len: 2, .. })),
        "expected bytes index-of --end on array{{bytes:4:2}} to materialize a two-item index list"
    );
    assert!(
        result
            .program
        ...
        "expected bytes index-of --end to inspect the last candidate byte of each fixed binary element"
    );
    compile_mir_to_ebpf_with_hints(&result.program, None, Some(&result.type_hints)).expect(
        "typed fixed-array bytes index-of --end consumed by get should compile through codegen",
    );
}

#[test]
fn test_lower_bytes_reverse_on_typed_bytes_global_array_reverses_elements() {
    let define_decl = DeclId::new(11_016);
    let global_get_decl = DeclId::new(11_017);
    let reverse_decl = DeclId::new(11_018);
    let get_decl = DeclId::new(11_019);
    let starts_with_decl = DeclId::new(11_020);
    let decl_names = HashMap::from([
        (define_decl, "global-define".to_string()),
        (global_get_decl, "global-get".to_string()),
        (reverse_decl, "bytes reverse".to_string()),
    ...
    .expect("bytes reverse should lower on typed fixed-array binary input");
    ...
    let byte_load_offsets = result
        ...
        .collect::<Vec<_>>();
    assert!(
        byte_load_offsets.contains(&3) && byte_load_offsets.contains(&7),
        "expected bytes reverse to read the last byte of each fixed binary element first, got {byte_load_offsets:?}"
    );

    let byte_store_offsets = result
        .program
        .main
        .blocks
        .iter()
        ...
        .collect::<Vec<_>>();
    assert!(
        byte_store_offsets.contains(&0) && byte_store_offsets.contains(&4),
        "expected bytes reverse to write the first byte of each reversed element first, got {byte_store_offsets:?}"
    );
    compile_mir_to_ebpf_with_hints(&result.program, None, Some(&result.type_hints))
        .expect("typed fixed-array bytes reverse consumed by get should compile through codegen");
}

#[test]
fn test_lower_bytes_at_on_typed_bytes_global_array_slices_elements() {
    let define_decl = DeclId::new(11_021);
    let global_get_decl = DeclId::new(11_022);
    let bytes_at_decl = DeclId::new(11_023);
    let get_decl = DeclId::new(11_024);
    let starts_with_decl = DeclId::new(11_025);
    let decl_names = HashMap::from([
        (define_decl, "global-define".to_string()),
        (global_get_decl, "global-get".to_string()),
        (bytes_at_decl, "bytes at".to_string()),
    ...
    .expect("bytes at should lower on typed fixed-array binary input");
    ...
    let byte_load_offsets = result
        ...
        .collect::<Vec<_>>();
    assert!(
        byte_load_offsets.contains(&1) && byte_load_offsets.contains(&5),
        "expected bytes at to read the slice start from each fixed binary element, got {byte_load_offsets:?}"
    );
    ...
    let byte_store_offsets = result
        ...
        .collect::<Vec<_>>();
    assert!(
        byte_store_offsets.contains(&0) && byte_store_offsets.contains(&2),
        "expected bytes at to write each sliced element into a compact fixed array, got {byte_store_offsets:?}"
    );
    compile_mir_to_ebpf_with_hints(&result.program, None, Some(&result.type_hints))
        .expect("typed fixed-array bytes at consumed by get should compile through codegen");
}

#[test]
fn test_lower_bytes_add_on_typed_bytes_global_array_inserts_per_element() {
    let define_decl = DeclId::new(11_026);
    let global_get_decl = DeclId::new(11_027);
    let add_decl = DeclId::new(11_028);
    let get_decl = DeclId::new(11_029);
    let starts_with_decl = DeclId::new(11_030);
    let decl_names = HashMap::from([
        (define_decl, "global-define".to_string()),
        (global_get_decl, "global-get".to_string()),
        (add_decl, "bytes add".to_string()),
    ...
    .expect("bytes add should lower on typed fixed-array binary input");

    let inserted_offsets = result
        .program
        .main
        .blocks
        .iter()
        ...
        .collect::<Vec<_>>();
    assert!(
        inserted_offsets.contains(&2) && inserted_offsets.contains(&8),
        "expected bytes add to store 0xff at index 2 within each output element, got {inserted_offsets:?}"
    );
    assert!(
        result.program.main.blocks.iter().any(|block| {
            block.instructions.iter().any(|inst| {
        ...
        "expected bytes add to store second inserted byte 0xee after the first element prefix"
    );
    compile_mir_to_ebpf_with_hints(&result.program, None, Some(&result.type_hints))
        .expect("typed fixed-array bytes add consumed by get should compile through codegen");
}

#[test]
fn test_lower_bytes_replace_on_typed_bytes_global_array_rewrites_per_element() {
    let define_decl = DeclId::new(11_031);
    let global_get_decl = DeclId::new(11_032);
    let replace_decl = DeclId::new(11_033);
    let get_decl = DeclId::new(11_034);
    let starts_with_decl = DeclId::new(11_035);
    let decl_names = HashMap::from([
        (define_decl, "global-define".to_string()),
        (global_get_decl, "global-get".to_string()),
        (replace_decl, "bytes replace".to_string()),
    ...
    .expect("bytes replace should lower on typed fixed-array binary input");

    let replacement_offsets = result
        .program
        .main
        .blocks
        .iter()
        ...
        .collect::<Vec<_>>();
    assert!(
        replacement_offsets.contains(&0) && replacement_offsets.contains(&4),
        "expected bytes replace to store replacement bytes in both fixed-array elements, got {replacement_offsets:?}"
    );
    assert!(
        result
            .program
            ...
            .iter()
            .any(|block| matches!(block.terminator, MirInst::Branch { .. })),
        "expected bytes replace to branch on per-element fixed binary comparisons"
    );
    compile_mir_to_ebpf_with_hints(&result.program, None, Some(&result.type_hints))
        .expect("typed fixed-array bytes replace consumed by get should compile through codegen");
}

#[test]
fn test_lower_bytes_remove_on_typed_bytes_global_array_accepts_impossible_match() {
    let define_decl = DeclId::new(11_036);
    let global_get_decl = DeclId::new(11_037);
    let remove_decl = DeclId::new(11_038);
    let get_decl = DeclId::new(11_039);
    let starts_with_decl = DeclId::new(11_040);
    let decl_names = HashMap::from([
        (define_decl, "global-define".to_string()),
        (global_get_decl, "global-get".to_string()),
        (remove_decl, "bytes remove".to_string()),
    ...
    .expect(
        "bytes remove should lower typed fixed-array binary input when the pattern cannot match",
    );
    ...
    assert!(
        ...
        "expected bytes remove no-match lowering to materialize a stack-backed fixed array"
    );
    compile_mir_to_ebpf_with_hints(&result.program, None, Some(&result.type_hints)).expect(
        "typed fixed-array bytes remove no-match consumed by get should compile through codegen",
    );
}
...
#[test]
fn test_lower_global_define_type_string_str_trim_char_feeds_length() {
    let define_decl = DeclId::new(10_640);
    let global_get_decl = DeclId::new(10_641);
    let trim_decl = DeclId::new(10_642);
    let length_decl = DeclId::new(10_643);
    let decl_names = HashMap::from([
        (define_decl, "global-define".to_string()),
        (global_get_decl, "global-get".to_string()),
    ...
    .expect("typed string global str trim --char should lower as a tracked string");
    ...
    assert!(
        ...
        "expected str trim --char to shift tracked string bytes for left trimming"
    );
    compile_mir_to_ebpf_with_hints(&result.program, None, Some(&result.type_hints))
        .expect("tracked string str trim --char result consumed by str length should compile");
}

#[test]
fn test_lower_global_define_type_string_str_trim_default_whitespace_feeds_length() {
    let define_decl = DeclId::new(11_051);
    let global_get_decl = DeclId::new(11_052);
    let trim_decl = DeclId::new(11_053);
    let length_decl = DeclId::new(11_054);
    let decl_names = HashMap::from([
        (define_decl, "global-define".to_string()),
        (global_get_decl, "global-get".to_string()),
        (trim_decl, "str trim".to_string()),
    ...
    .expect("typed string global default str trim should lower as a tracked string");
    ...
    assert!(
        ...
        "expected default str trim to combine ASCII whitespace byte predicates"
    );
    compile_mir_to_ebpf_with_hints(&result.program, None, Some(&result.type_hints))
        .expect("tracked string default str trim result consumed by str length should compile");
}
...
#[test]
fn test_lower_global_define_type_string_array_str_trim_left_char_feeds_join() {
    let define_decl = DeclId::new(11_041);
    let global_get_decl = DeclId::new(11_042);
    let trim_decl = DeclId::new(11_043);
    let join_decl = DeclId::new(11_044);
    let length_decl = DeclId::new(11_045);
    let decl_names = HashMap::from([
        (define_decl, "global-define".to_string()),
        (global_get_decl, "global-get".to_string()),
        (trim_decl, "str trim".to_string()),
    ...
    .expect("typed string array str trim --left --char should lower as a fixed string array");
    ...
    assert!(
        ...
        "expected str trim --left --char to shift fixed string bytes in the stack output"
    );
    assert!(
        result
            .program
        ...
        "expected str trim --left --char to subtract the trimmed prefix from runtime lengths"
    );
    compile_mir_to_ebpf_with_hints(&result.program, None, Some(&result.type_hints)).expect(
        "typed string array str trim --left --char result consumed by str join should compile through codegen",
    );
}
...
fn test_lower_global_define_type_string_array_str_trim_supports_nul_char_and_feeds_join() {
    ...
    let join_decl = DeclId::new(11_049);
    let length_decl = DeclId::new(11_050);
    let decl_names = HashMap::from([
        (define_decl, "global-define".to_string()),
        (global_get_decl, "global-get".to_string()),
        (trim_decl, "str trim".to_string()),
    ...
    .expect("typed string array str trim --char should lower as a fixed string array");
    ...
    assert!(
        ...
        "expected str trim --char to shift fixed string bytes for left trimming"
    );
    assert!(
        result
            .program
        ...
        "expected str trim --char to store fixed suffix-trimmed lengths"
    );
    compile_mir_to_ebpf_with_hints(&result.program, None, Some(&result.type_hints)).expect(
        "typed string array str trim --char result consumed by str join should compile through codegen",
    );
}

#[test]
fn test_lower_global_define_type_string_array_str_trim_default_whitespace_feeds_join() {
    let define_decl = DeclId::new(10_630);
    let global_get_decl = DeclId::new(10_631);
    let trim_decl = DeclId::new(10_632);
    let join_decl = DeclId::new(10_633);
    let length_decl = DeclId::new(10_634);
    let decl_names = HashMap::from([
        (define_decl, "global-define".to_string()),
        (global_get_decl, "global-get".to_string()),
        (trim_decl, "str trim".to_string()),
    ...
    .expect("typed string array default str trim should lower as a fixed string array");
    ...
    assert!(
        ...
        "expected default str trim to shift fixed string bytes"
    );
    compile_mir_to_ebpf_with_hints(&result.program, None, Some(&result.type_hints)).expect(
        "typed string array default str trim result consumed by str join should compile through codegen",
    );
}
...
#[test]
fn test_lower_global_define_type_u32_array_large_initializer_sort_first_folds_constant() {
    let define_decl = DeclId::new(11_340);
    let global_get_decl = DeclId::new(11_341);
    let sort_decl = DeclId::new(11_342);
    let first_decl = DeclId::new(11_343);
    let decl_names = HashMap::from([
        (define_decl, "global-define".to_string()),
        (global_get_decl, "global-get".to_string()),
        (sort_decl, "sort".to_string()),
    ...
    let func = HirFunction {
        blocks: vec![HirBlock {
            ...
            stmts: vec![
                ...
                HirStmt::LoadLiteral {
                    dst: RegId::new(2),
                    lit: HirLiteral::String("array{u32:17}".into()),
                },
                HirStmt::Call {
                    decl_id: define_decl,
                    src_dst: RegId::new(0),
    ...
    .expect("compile-time initialized u32 fixed-array sort should fold before runtime capacity");
    ...
    assert!(
        ...
        "expected sorted first element to fold to zero"
    );
    assert!(
        ...
        "compile-time initialized large sort should not emit runtime u32 compare/swap stores"
    );
    compile_mir_to_ebpf_with_hints(&result.program, None, Some(&result.type_hints))
        .expect("compile-time initialized u32 fixed-array sort should compile through codegen");
}
...
#[test]
fn test_lower_global_define_type_bool_array_uniq_materializes_numeric_list() {
    let define_decl = DeclId::new(10_681);
    let global_get_decl = DeclId::new(10_682);
    let uniq_decl = DeclId::new(10_683);
    let length_decl = DeclId::new(10_684);
    let decl_names = HashMap::from([
        (define_decl, "global-define".to_string()),
        (global_get_decl, "global-get".to_string()),
        (uniq_decl, "uniq".to_string()),
    ...
    .expect("global-define --type array{bool:N} uniq should materialize a numeric list");
    ...
    assert!(
        result
            ...
            .flat_map(|block| block.instructions.iter())
            .any(|inst| matches!(inst, MirInst::ListPush { .. })),
        "expected typed bool fixed-array uniq to push unique items into a stack numeric list"
    );
    compile_mir_to_ebpf_with_hints(&result.program, None, Some(&result.type_hints))
        .expect("typed bool array uniq consumed by length should compile through codegen");
}
...
#[test]
fn test_lower_global_define_type_u32_array_where_initialized_length_folds_constant() {
    let define_decl = DeclId::new(11_330);
    let global_get_decl = DeclId::new(11_331);
    let where_decl = DeclId::new(11_332);
    let length_decl = DeclId::new(11_333);
    let closure_block_id = nu_protocol::BlockId::new(12);
    let row_param_var = VarId::new(76);
    let decl_names = HashMap::from([
        (define_decl, "global-define".to_string()),
        (global_get_decl, "global-get".to_string()),
        (where_decl, "where".to_string()),
    ...
    .expect("initialized u32 fixed-array where feeding length should fold");
    let instructions = result
        .program
        .main
        .blocks
    ...
    assert!(
        instructions
            .iter()
            .all(|inst| !matches!(inst, MirInst::ListPush { .. } | MirInst::ListNew { .. })),
        "initialized fixed-array where length should not materialize a runtime list"
    );
    assert!(
        instructions.iter().all(|inst| !matches!(
            inst,
        ...
        "initialized fixed-array where predicate should fold at compile time"
    );
    assert!(
        instructions.iter().any(|inst| matches!(
            inst,
        ...
        "expected filtered length to fold to one"
    );
    compile_mir_to_ebpf_with_hints(&result.program, None, Some(&result.type_hints))
        .expect("initialized u32 fixed-array where length should compile through codegen");
}
...
#[test]
fn test_lower_global_define_type_bool_array_where_materializes_numeric_list() {
    let define_decl = DeclId::new(10_685);
    let global_get_decl = DeclId::new(10_686);
    let where_decl = DeclId::new(10_687);
    let length_decl = DeclId::new(10_688);
    let closure_block_id = nu_protocol::BlockId::new(1);
    let decl_names = HashMap::from([
        (define_decl, "global-define".to_string()),
        (global_get_decl, "global-get".to_string()),
    ...
    .expect("global-define --type array{bool:N} where should materialize a numeric list");
    let instructions = result
        .program
        .main
        .blocks
    ...
    assert!(
        instructions
            .iter()
            .any(|inst| matches!(inst, MirInst::ListPush { .. })),
        "expected typed bool fixed-array where to push matching items into a stack numeric list"
    );
    assert!(
        instructions.iter().any(|inst| matches!(
            inst,
        ...
        "expected typed bool fixed-array where predicate to load the first bool item"
    );
    compile_mir_to_ebpf_with_hints(&result.program, None, Some(&result.type_hints))
        .expect("typed bool array where consumed by length should compile through codegen");
}

#[test]
fn test_lower_global_define_type_bool_array_where_true_preserves_non_empty_for_first() {
    let define_decl = DeclId::new(10_689);
    let global_get_decl = DeclId::new(10_690);
    let where_decl = DeclId::new(10_691);
    let first_decl = DeclId::new(10_692);
    let closure_block_id = nu_protocol::BlockId::new(1);
    let decl_names = HashMap::from([
        (define_decl, "global-define".to_string()),
        (global_get_decl, "global-get".to_string()),
    ...
    compile_mir_to_ebpf_with_hints(&result.program, None, Some(&result.type_hints))
        .expect("constant-true where feeding first should compile through codegen");
}

#[test]
fn test_lower_global_define_type_bool_array_where_false_materializes_empty_without_pushes() {
    let define_decl = DeclId::new(10_693);
    let global_get_decl = DeclId::new(10_694);
    let where_decl = DeclId::new(10_695);
    let is_empty_decl = DeclId::new(10_696);
    let closure_block_id = nu_protocol::BlockId::new(1);
    let decl_names = HashMap::from([
        (define_decl, "global-define".to_string()),
        (global_get_decl, "global-get".to_string()),
    ...
    .expect("constant-false where on typed bool arrays should materialize a known-empty list");
    ...
    assert!(
        result
            ...
            .flat_map(|block| block.instructions.iter())
            .all(|inst| !matches!(inst, MirInst::ListPush { .. })),
        "expected where false on typed fixed arrays to avoid filter pushes"
    );
    compile_mir_to_ebpf_with_hints(&result.program, None, Some(&result.type_hints))
        .expect("constant-false fixed-array where feeding is-empty should compile through codegen");
}
...
#[test]
fn test_lower_global_define_type_u64_array_each_identity_first_folds_zero_fill() {
    let define_decl = DeclId::new(11_330);
    let global_get_decl = DeclId::new(11_331);
    let each_decl = DeclId::new(11_332);
    let first_decl = DeclId::new(11_333);
    let closure_block_id = nu_protocol::BlockId::new(12);
    let closure_param_var = VarId::new(76);
    let decl_names = HashMap::from([
        (define_decl, "global-define".to_string()),
        (global_get_decl, "global-get".to_string()),
        (each_decl, "each".to_string()),
    ...
    .expect("u64 fixed-array each identity feeding first should fold zero fill");
    let instructions = result
        .program
        .main
        .blocks
    ...
    assert!(
        instructions
            .iter()
            .all(|inst| !matches!(inst, MirInst::ListPush { .. } | MirInst::ListNew { .. })),
        "zero-filled u64 each identity first should not materialize a runtime list"
    );
    assert!(
        instructions.iter().any(|inst| matches!(
            inst,
        ...
        "expected identity-projected first element to fold to zero"
    );
    compile_mir_to_ebpf_with_hints(&result.program, None, Some(&result.type_hints))
        .expect("u64 fixed-array each identity feeding first should compile through codegen");
}

#[test]
fn test_lower_global_define_type_string_array_each_length_materializes_numeric_list() {
    let define_decl = DeclId::new(10_697);
    let global_get_decl = DeclId::new(10_698);
    let each_decl = DeclId::new(10_699);
    let length_decl = DeclId::new(10_700);
    let get_decl = DeclId::new(10_701);
    let closure_block_id = nu_protocol::BlockId::new(1);
    let item_var = VarId::new(10_697);
    let decl_names = HashMap::from([
        (define_decl, "global-define".to_string()),
        (global_get_decl, "global-get".to_string()),
        (each_decl, "each".to_string()),
    ...
    .expect("global-define --type array{string:N:N} each should preserve string item metadata");
    ...
    assert_eq!(
        pushes, 2,
        "expected typed fixed-array each to push transformed string lengths"
    );
    compile_mir_to_ebpf_with_hints(&result.program, None, Some(&result.type_hints))
        .expect("typed string array each consumed by get should compile through codegen");
}
...
#[test]
fn test_lower_global_define_type_record_select_reject_preserves_string_fields() {
    for (case_idx, (cmd_name, field_names)) in
        [("select", vec!["comm"]), ("reject", vec!["pid", "uid"])]
            .into_iter()
            .enumerate()
    {
        let base_decl = 10_390 + case_idx * 10;
        let define_decl = DeclId::new(base_decl);
        let global_get_decl = DeclId::new(base_decl + 1);
        let transform_decl = DeclId::new(base_decl + 2);
        let get_decl = DeclId::new(base_decl + 3);
        ...
        let comm_field_reg = RegId::new(next_reg);
        next_reg += 1;
        stmts.push(HirStmt::LoadLiteral {
            dst: comm_field_reg,
        ...
        let comm_result_reg = RegId::new(next_reg);
        next_reg += 1;
        stmts.push(HirStmt::Call {
            decl_id: get_decl,
            src_dst: comm_result_reg,
            args: HirCallArgs {
                positional: vec![comm_field_reg],
                pipeline_input: Some(transform_reg),
                ..HirCallArgs::default()
            },
        });
        ...
        stmts.push(HirStmt::Call {
            ...
            args: HirCallArgs {
                pipeline_input: Some(comm_result_reg),
                ..HirCallArgs::default()
            },
        });
        ...
        .unwrap_or_else(|err| {
            panic!("typed record {cmd_name} should preserve string fields: {err:?}")
        });

        compile_mir_to_ebpf_with_hints(&result.program, None, Some(&result.type_hints))
            .unwrap_or_else(|err| {
                panic!("typed record {cmd_name} string field should compile: {err:?}")
            });
    }
}
...
fn test_lower_global_define_type_record_values_supports_scalar_fields() {
    ...
    let sum_decl = DeclId::new(10_393);
    let decl_names = HashMap::from([
        (define_decl, "global-define".to_string()),
        (global_get_decl, "global-get".to_string()),
...
fn test_lower_global_define_type_record_values_get_string_field_direct_projection() {
    ...
    let values_decl = DeclId::new(10_396);
    let get_decl = DeclId::new(10_397);
    let length_decl = DeclId::new(10_398);
    let decl_names = HashMap::from([
        (define_decl, "global-define".to_string()),
        (global_get_decl, "global-get".to_string()),
        (values_decl, "values".to_string()),
    ...
    .unwrap_or_else(|err| {
        panic!("typed record values | get string field should lower directly: {err:?}")
    });
    let instructions = result
        .program
        .main
    ...
    assert!(
        ...
        "expected values | get string field to avoid materializing a numeric values list"
    );
    compile_mir_to_ebpf_with_hints(&result.program, None, Some(&result.type_hints)).unwrap_or_else(
        |err| {
            panic!("typed record values | get string field should compile through codegen: {err:?}")
        },
    );
}
...
#[test]
fn test_lower_global_define_type_record_insert_update_upsert_preserves_string_fields() {
    for (case_idx, (cmd_name, replacement_field)) in
        [("insert", "cpu"), ("update", "pid"), ("upsert", "pid")]
            .into_iter()
            .enumerate()
    {
        let base_decl = 17_010 + case_idx * 10;
        let define_decl = DeclId::new(base_decl);
        let global_get_decl = DeclId::new(base_decl + 1);
        let record_cmd_decl = DeclId::new(base_decl + 2);
        let get_decl = DeclId::new(base_decl + 3);
...
#[test]
fn test_lower_global_define_type_record_default_preserves_string_fields() {
    let define_decl = DeclId::new(17_040);
    let global_get_decl = DeclId::new(17_041);
    let default_decl = DeclId::new(17_042);
    let get_decl = DeclId::new(17_043);
    let length_decl = DeclId::new(17_044);
    let decl_names = HashMap::from([
        (define_decl, "global-define".to_string()),
        (global_get_decl, "global-get".to_string()),
        (default_decl, "default".to_string()),
    ...
    .unwrap_or_else(|err| panic!("typed record default should preserve string fields: {err:?}"));

    compile_mir_to_ebpf_with_hints(&result.program, None, Some(&result.type_hints))
        .unwrap_or_else(|err| panic!("typed record default string field should compile: {err:?}"));
}
...
#[test]
fn test_lower_global_define_type_record_merge_preserves_string_fields() {
    let define_decl = DeclId::new(10_470);
    let global_get_decl = DeclId::new(10_471);
    let merge_decl = DeclId::new(10_472);
    let get_decl = DeclId::new(10_473);
    let length_decl = DeclId::new(10_474);
    let decl_names = HashMap::from([
        (define_decl, "global-define".to_string()),
        (global_get_decl, "global-get".to_string()),
        (merge_decl, "merge".to_string()),
    ...
    let func = HirFunction {
        blocks: vec![HirBlock {
            ...
            stmts: vec![
                ...
                HirStmt::LoadLiteral {
                    dst: RegId::new(3),
                    lit: HirLiteral::String("merge_state".into()),
                },
                HirStmt::LoadLiteral {
                    dst: RegId::new(4),
                    lit: HirLiteral::String("record{comm:string:8,cpu:u32}".into()),
                },
                HirStmt::Call {
                    decl_id: define_decl,
                    src_dst: RegId::new(5),
    ...
    .unwrap_or_else(|err| panic!("typed record merge should preserve string fields: {err:?}"));

    compile_mir_to_ebpf_with_hints(&result.program, None, Some(&result.type_hints))
        .unwrap_or_else(|err| panic!("typed record merged string field should compile: {err:?}"));
}
