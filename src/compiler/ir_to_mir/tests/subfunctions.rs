...
use crate::compiler::ir_to_mir::tests::helpers::{int_member, string_member};
use crate::compiler::{
...
fn test_multiblock_user_function_string_return_preserves_string_semantics() {
        ...
        blocks: vec![
            ...
            HirBlock {
                ...
                stmts: vec![
                    HirStmt::LoadLiteral {
                        dst: RegId::new(0),
                        lit: HirLiteral::String("o".into()),
                    },
                    HirStmt::LoadLiteral {
                        dst: RegId::new(2),
                        lit: HirLiteral::String("k".into()),
                    },
                    HirStmt::StringAppend {
                        src_dst: RegId::new(0),
                        val: RegId::new(2),
    ...
    compile_mir_to_ebpf_with_hints(&result.program, None, Some(&result.type_hints))
        .expect("multiblock string return should compile after caller append");
}

#[test]
fn test_multiblock_user_function_list_return_preserves_list_semantics() {
    use nu_protocol::{DeclId, RegId};
...
#[test]
fn test_user_function_list_return_uses_subfn_aggregate_out_slot() {
    use nu_protocol::{DeclId, RegId, VarId};
    ...
    .expect("single-block list return should lower");

    assert!(
        result.program.subfunctions.len() == 1,
        "single-block list-returning user function should lower as a BPF subfunction with an aggregate out slot"
    );
    assert!(
        result
            .program
            ...
            .flat_map(|block| block.instructions.iter())
            .any(|inst| matches!(inst, MirInst::CallSubfn { .. })),
        "single-block list-returning user function should emit a BPF subfunction call"
    );
    ...
    compile_mir_to_ebpf_with_hints(&result.program, None, Some(&result.type_hints))
        .expect("single-block list return should compile after caller projection");
}
...
#[test]
fn test_view_ir_style_user_function_constant_list_spread_return_can_flow_through_local() {
    use nu_protocol::ir::{Instruction, IrBlock, Literal, RedirectMode};
    use nu_protocol::{DeclId, RegId};
    use std::sync::Arc;
    ...
    .expect("view-ir style constant list-spread program should infer HIR types");
    let probe_ctx = ProbeContext::new(crate::compiler::EbpfProgramType::Kprobe, "ksys_read");
    ...
    .expect("view-ir style constant list-spread return through local should lower");
    ...
    .expect("view-ir style constant list-spread return through local should compile");
}

#[test]
fn test_view_ir_style_user_function_constant_record_spread_return_can_flow_through_local() {
    use nu_protocol::ast::{CellPath, PathMember};
    use nu_protocol::casing::Casing;
    use nu_protocol::ir::{DataSlice, Instruction, IrBlock, Literal, RedirectMode};
    use nu_protocol::{DeclId, RegId};
    ...
    let user_ir = IrBlock {
        instructions: vec![
            ...
            Instruction::LoadLiteral {
                dst: RegId::new(4),
                lit: Literal::String(DataSlice { start: 5, len: 3 }),
            },
            Instruction::LoadLiteral {
                dst: RegId::new(5),
                lit: Literal::Int(7),
            ...
            Instruction::Return { src: RegId::new(3) },
        ],
        spans: vec![],
        data: Arc::from(b"msgokpid".as_slice()),
        ast: vec![],
    ...
    let main_ir = IrBlock {
        instructions: vec![
            Instruction::LoadLiteral {
                dst: RegId::new(1),
                lit: Literal::String(DataSlice { start: 0, len: 2 }),
            },
            Instruction::PushPositional { src: RegId::new(1) },
            Instruction::RedirectOut {
                mode: RedirectMode::Value,
    ...
    .expect("view-ir style constant record-spread program should infer HIR types");
    let probe_ctx = ProbeContext::new(crate::compiler::EbpfProgramType::Kprobe, "ksys_read");
    ...
    .expect("view-ir style constant record-spread return through local should lower");
    ...
    .expect("view-ir style constant record-spread return through local should compile");
}

#[test]
fn test_view_ir_style_user_function_constant_record_upsert_return_can_flow_through_local() {
    use nu_protocol::ast::{CellPath, PathMember};
    use nu_protocol::casing::Casing;
    use nu_protocol::ir::{DataSlice, Instruction, IrBlock, Literal, RedirectMode};
    use nu_protocol::{DeclId, RegId};
    ...
    let user_ir = IrBlock {
        ...
        spans: vec![],
        data: Arc::from(b"msghiok".as_slice()),
        ast: vec![],
        comments: vec!["".into(); 8],
        register_count: 5,
        file_count: 0,
    ...
    .expect("view-ir style constant record-upsert program should infer HIR types");
    let probe_ctx = ProbeContext::new(crate::compiler::EbpfProgramType::Kprobe, "ksys_read");
    ...
    .expect("view-ir style constant record-upsert return through local should lower");
    ...
    .expect("view-ir style constant record-upsert return through local should compile");
}

#[test]
fn test_view_ir_style_user_function_constant_list_upsert_return_can_flow_through_local() {
    use nu_protocol::ir::{Instruction, IrBlock, Literal, RedirectMode};
    use nu_protocol::{DeclId, RegId};
    use std::sync::Arc;
    ...
    .expect("view-ir style constant list-upsert program should infer HIR types");
    let probe_ctx = ProbeContext::new(crate::compiler::EbpfProgramType::Kprobe, "ksys_read");
    ...
    .expect("view-ir style constant list-upsert return through local should lower");
