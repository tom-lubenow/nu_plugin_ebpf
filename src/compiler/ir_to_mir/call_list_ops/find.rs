...
impl<'a> HirToMirLowering<'a> {
    pub(in crate::compiler::ir_to_mir) fn lower_stack_list_find(
        ...
        if let Some((builder_reg, values)) = input_reg.and_then(|reg| {
            ...
            let Some(needles_values) = constant_needles else {
            ...
            let vals = values
                .into_iter()
                .filter(|value| Self::value_matches_any_needle(value, &needles_values, invert))
                .collect::<Vec<_>>();
    ...
    fn find_numeric_needle_values(
        ...
        for (needle_vreg, needle_reg) in needles {
            ...
            if let Some(needle) = needle_const {
                needle_values.push(self.large_const_operand(&MirType::I64, needle));
                needle_consts.push(needle);
                continue;
            }
            ...
            if !matches!(
                ...
                return Err(CompileError::UnsupportedInstruction(
                    runtime_type_error.into(),
                ));
            }
            needle_values.push(MirValue::VReg(*needle_vreg));
        }

        Ok((needle_values, all_constant.then_some(needle_consts)))
    }

    fn emit_find_any_numeric_match(&mut self, item_vreg: VReg, needle_values: &[MirValue]) -> VReg {
        let mut found_vreg = None;
        for needle_value in needle_values {
            ...
            self.vreg_type_hints.insert(eq_vreg, MirType::Bool);

            found_vreg = Some(if let Some(previous_vreg) = found_vreg {
                let combined_vreg = self.func.alloc_vreg();
                self.emit(MirInst::BinOp {
                    dst: combined_vreg,
                    op: BinOpKind::Or,
                    lhs: MirValue::VReg(previous_vreg),
                    rhs: MirValue::VReg(eq_vreg),
                });
                self.vreg_type_hints.insert(combined_vreg, MirType::Bool);
                combined_vreg
            } else {
                eq_vreg
            });
        }
        found_vreg.expect("find requires at least one search argument")
    }
    ...
    fn lower_typed_fixed_array_find(
        ...
        let (needle_values, needle_consts) = self.find_numeric_needle_values(
            &needles,
        ...
        for source_index in 0..array_len {
            ...
            let found_vreg = self.emit_find_any_numeric_match(item_vreg, &needle_values);
        ...
        if !values.is_empty() {
            ...
            for (index, value) in values.iter().enumerate() {
                ...
                for (_, needle_reg) in needles {
                    ...
                    let eq_vreg = self.get_vreg(literal_reg);
                    found_vreg = Some(if let Some(previous_vreg) = found_vreg {
                        let combined_vreg = self.func.alloc_vreg();
                        self.emit(MirInst::BinOp {
                            dst: combined_vreg,
                ...
                let found_vreg = found_vreg.expect("find requires at least one search argument");
                let push_block = self.func.alloc_block();
    ...
    fn typed_fixed_array_find_constant_value(
        input_meta: &RegMetadata,
        needle_consts: Option<&[i64]>,
        invert: bool,
    ) -> Option<nu_protocol::Value> {
        match (&input_meta.constant_value, needle_consts) {
            (Some(nu_protocol::Value::List { vals, .. }), Some(needles)) => {
                let vals = vals
                    .iter()
                    .filter(|value| Self::numeric_value_matches_any(value, needles, invert))
                    .cloned()
                    .collect::<Vec<_>>();
    ...
    fn zero_initialized_typed_fixed_array_find_constant_value(
        ...
        let needles = needle_consts?;
        if !input_meta.zero_initialized_global {
            return None;
        }
        let vals = if needles.contains(&0) != invert {
            (0..array_len)
    ...
    fn lower_stack_list_find_materialized(
        ...
        let constant_value = match (input_meta.constant_value, needle_consts) {
            (Some(nu_protocol::Value::List { vals, .. }), Some(needles)) => {
                let vals = vals
                    .into_iter()
                    .filter(|value| Self::numeric_value_matches_any(value, &needles, invert))
    ...
    fn numeric_value_matches_any(
        value: &nu_protocol::Value,
        needles: &[i64],
        invert: bool,
    ) -> bool {
        Self::numeric_value_from_value(value)
            .is_some_and(|value| needles.contains(&value) != invert)
    }
        ...
        assert!(
            lowering
                .lower_typed_fixed_array_find(TypedFixedArrayFind {
                    ...
                    input_vreg,
                    input_meta: &input_meta,
                    needles: vec![(needle_vreg, needle_reg)],
                    invert: false,
                })
