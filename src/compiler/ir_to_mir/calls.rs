...
impl<'a> HirToMirLowering<'a> {
    fn fixed_binary_input_len(
        ...
        let input_ty = self
            ...
            .ok_or_else(|| {
                CompileError::UnsupportedInstruction(format!(
                    "{cmd_name} requires typed fixed-size binary input in eBPF"
                ))
            })?;
        let MirType::Ptr {
            pointee,
    ...
    fn lower_fixed_binary_array_predicate(
        ...
        if array_len > MAX_FIXED_BINARY_ARRAY_RESULT_ITEMS {
            return Err(CompileError::UnsupportedInstruction(format!(
                "{cmd_name} output exceeds stack-backed bool-list capacity {MAX_FIXED_BINARY_ARRAY_RESULT_ITEMS} in eBPF"
            )));
        }
    ...
    fn lower_fixed_binary_index_of(
        ...
        if pattern.len() > input_len {
            ...
            if probe_count > MAX_FIXED_BINARY_INDEX_OF_PROBES {
                return Err(CompileError::UnsupportedInstruction(format!(
                    "{cmd_name} fixed-size binary input has {probe_count} candidate offsets; eBPF lowering supports at most {MAX_FIXED_BINARY_INDEX_OF_PROBES}"
                )));
            }
    ...
    fn lower_fixed_binary_array_index_of(
        ...
        if array_len > MAX_FIXED_BINARY_ARRAY_RESULT_ITEMS {
            return Err(CompileError::UnsupportedInstruction(format!(
                "{cmd_name} output exceeds stack-backed numeric-list capacity {MAX_FIXED_BINARY_ARRAY_RESULT_ITEMS} in eBPF"
            )));
        }
        ...
        for index in 0..array_len {
            ...
            if probe_count > MAX_FIXED_BINARY_INDEX_OF_PROBES {
                return Err(CompileError::UnsupportedInstruction(format!(
                    "{cmd_name} typed fixed-array binary element has {probe_count} candidate offsets; eBPF lowering supports at most {MAX_FIXED_BINARY_INDEX_OF_PROBES}"
                )));
            }
            ...
            for (probe_index, offset) in offsets.iter().copied().enumerate() {
                ...
                let candidate = self.lower_fixed_binary_pattern_match(
                    ...
                    offset,
                    pattern,
                    "fixed binary array index-of byte",
                )?;
                self.terminate(MirInst::Branch {
                    cond: candidate,
                    if_true: found_block,
    ...
    fn lower_fixed_binary_index_of_all(
        ...
        let max_matches = if pattern.len() > input_len {
            0
        } else {
            input_len / pattern.len()
        };
        if max_matches > MAX_BYTES_INDEX_OF_ALL_MATCHES {
            return Err(CompileError::UnsupportedInstruction(format!(
                "{cmd_name} --all result for fixed-size binary input may exceed eBPF numeric list capacity {MAX_BYTES_INDEX_OF_ALL_MATCHES}"
            )));
        }

        let (out_slot, out_ty) = self.create_stack_numeric_list_result(result_vreg, max_matches);

        if pattern.len() <= input_len {
            let probe_count = input_len - pattern.len() + 1;
            if probe_count > MAX_FIXED_BINARY_INDEX_OF_PROBES {
                return Err(CompileError::UnsupportedInstruction(format!(
                    "{cmd_name} --all fixed-size binary input has {probe_count} candidate offsets; eBPF lowering supports at most {MAX_FIXED_BINARY_INDEX_OF_PROBES}"
                )));
            }
            ...
            let probe_blocks = (0..=last_offset)
                ...
                .collect::<Vec<_>>();
            let first_probe = if search_from_end {
                probe_blocks[last_offset]
            } else {
                probe_blocks[0]
            };
            self.terminate(MirInst::Jump {
            ...
            for offset in offsets {
                ...
                let found_block = self.func.alloc_block();
                let next_probe = if search_from_end {
                    offset.checked_sub(1).map(|next| probe_blocks[next])
                } else {
                    (offset < last_offset).then(|| probe_blocks[offset + 1])
                };
                let skip_probe = if search_from_end {
                    offset
                        .checked_sub(pattern.len())
                        .map(|next| probe_blocks[next])
                } else {
                    let next = offset + pattern.len();
                    (next <= last_offset).then(|| probe_blocks[next])
                };
                ...
                let candidate = self.lower_fixed_binary_pattern_match(
                    ...
                    offset,
                    pattern,
                    "fixed binary index-of --all byte",
                )?;
                self.terminate(MirInst::Branch {
                    cond: candidate,
                    if_true: found_block,
                ...
                self.current_block = found_block;
                let offset_vreg = self.func.alloc_vreg();
                self.emit(MirInst::Copy {
                    dst: offset_vreg,
                    src: MirValue::Const(offset as i64),
                });
                self.vreg_type_hints.insert(offset_vreg, MirType::I64);
                self.emit(MirInst::ListPush {
                    list: result_vreg,
                    item: offset_vreg,
                });
        ...
        out_meta.list_buffer = Some((out_slot, max_matches));
        out_meta.annotated_semantics = Some(AnnotatedValueSemantics::NumericList {
            max_len: max_matches,
            known_len: (max_matches == 0).then_some(0),
        });
        self.vreg_type_hints.insert(
            result_vreg,
            MirType::Ptr {
                pointee: Box::new(MirType::Array {
                    ...
                    len: max_matches.saturating_add(1),
                }),
    ...
    fn lower_fixed_array_binary_at(
        &mut self,
        src_dst: RegId,
        dst_vreg: VReg,
        src_dst_had_value: bool,
            ...
            && matches!(base_runtime_ty, MirType::Array { .. })
        {
            ...
            base_runtime_ty = self
                ...
                .ok_or_else(|| {
                    CompileError::UnsupportedInstruction(
                        "bytes at requires typed fixed-array input in eBPF".into(),
                    )
                })?;
        }
        ...
        if *len != array_len || elem_ty.byte_array_len() != Some(elem_len) {
            return Err(CompileError::UnsupportedInstruction(
                "bytes at requires typed fixed-array binary input with matching byte-array layout in eBPF"
                    .into(),
            ));
        }

        let (start, end) = Self::bytes_at_bounds(elem_len, range);
        let out_len = end.saturating_sub(start);
        if out_len == 0 || array_len == 0 {
        ...
        for item_index in 0..array_len {
            let input_base = item_index.checked_mul(input_stride).ok_or_else(|| {
                CompileError::UnsupportedInstruction(
                    "bytes at typed fixed-array input offset overflowed in eBPF".into(),
                )
            })?;
            let output_base = item_index.checked_mul(output_stride).ok_or_else(|| {
                CompileError::UnsupportedInstruction(
                    "bytes at typed fixed-array output offset overflowed in eBPF".into(),
                )
            })?;
            let input_offset =
                Self::checked_byte_offset(input_base, start, "fixed binary array bytes at source")?;
            self.emit_ptr_to_slot_copy(out_slot, output_base, input_vreg, input_offset, out_len)?;
        }
        ...
        out_meta.annotated_semantics = Some(AnnotatedValueSemantics::FixedArray {
            elem: Box::new(AnnotatedValueSemantics::Binary { len: out_len }),
            len: array_len,
        });
        Ok(true)
    }
    ...
    pub(super) fn lower_call(
        ...
        match cmd_name.as_str() {
                    ...
                    .is_some()
                {
                            ...
                            .flatten()
                    {
                        ...
                        meta.constant_value.as_ref().and_then(|value| match value {
                            ...
                            | nu_protocol::Value::Duration { .. } => Some(false),
                            _ => None,
                        })
                    }
                }) {
            ...
            "bytes starts-with" | "bytes ends-with" => {
                ...
                match input_value {
                    Some(input_value) => match input_value {
            ...
            "bytes at" => {
                ...
                match input_value {
                    ...
                    None => {
                        ...
                        let input_vreg = self.pipeline_input.unwrap_or(dst_vreg);
                        if self.lower_fixed_array_binary_at(
                            src_dst,
                            dst_vreg,
                            src_dst_had_value,
                            input_reg,
