...
impl<'a> HirToMirLowering<'a> {
                ...
                self.materialize_metadata_record_value(&src_meta)?
        {
            return Ok((
                ...
                Some(SubfunctionReturnSeed {
                    ...
                    root_ctx_field: materialized_meta.root_ctx_field,
                    is_context: false,
                }),
            ));
        }
    ...
    pub fn lower_block(&mut self, hir: &HirFunction) -> Result<(), CompileError> {
        ...
        for block in &hir.blocks {
            ...
            for (stmt_index, stmt) in block.stmts.iter().enumerate() {
                ...
                self.current_call_result_list_shape_metadata_only = match stmt {
                    HirStmt::Call { src_dst, .. } => self
                        .call_result_flows_to_list_shape_metadata_only_consumer(
                            &block.stmts,
                            stmt_index,
                            *src_dst,
                        ),
    ...
    fn is_compile_time_only_math_log_value(
        &self,
        stmts: &[HirStmt],
        stmt_index: usize,
        dst: RegId,
    ...
    fn is_compile_time_only_math_variance_stddev_value(
        &self,
        stmts: &[HirStmt],
        stmt_index: usize,
        dst: RegId,
    ...
    fn call_result_flows_to_list_shape_metadata_only_consumer(
        ...
        .into_iter()
        .any(|consumer| {
            compile_time_value_flows_to_fixed_layout_consumer_without_transforms(
                stmts,
                stmt_index,
                dst,
                self.decl_names,
    ...
    pub(super) fn lower_stmt(&mut self, instruction: &HirStmt) -> Result<(), CompileError> {
        match instruction {
            ...
            HirStmt::LoadEnv { key, .. } | HirStmt::LoadEnvOpt { key, .. } => {
                ...
                return Err(CompileError::UnsupportedInstruction(format!(
                    "Environment variable access ($env.{}) is not supported in eBPF because kernel programs cannot access the user environment.",
                    key_name
                )));
            }
            ...
            HirStmt::StoreEnv { key, .. } => {
                let key_name = std::str::from_utf8(key).unwrap_or("<unknown>");
                return Err(CompileError::UnsupportedInstruction(format!(
                    "Setting environment variable ($env.{}) is not supported in eBPF because kernel programs cannot access the user environment.",
                    key_name
                )));
            }
