...
impl VccVerifier {
            ...
            if let Some(refinement) = state.cond_refinement(cond_reg) {
                match refinement {
                    ...
                    VccCondRefinement::ContextBufferEnd { ptr_reg, op } => {
                        self.refine_context_buffer_end_compare(&mut true_state, ptr_reg, op, true);
                        self.refine_context_buffer_end_compare(
                            &mut false_state,
                            ptr_reg,
                            op,
                            false,
    ...
    pub(super) fn refine_ptr_nullability(
        ...
        let ambiguous_map = state.map_value_ambiguous_map_source(ptr_reg).cloned();
        if !non_null {
        ...
        if map_lookup_ambiguous {
            if let Some(map) = ambiguous_map {
                state.set_ambiguous_map_lookup_source_with_map(ptr_reg, map);
            } else {
    ...
    pub(super) fn refine_context_buffer_end_compare(
        &self,
        state: &mut VccState,
        ptr_reg: VccReg,
        op: VccBinOp,
        ...
        let (Some(root), Some(bounds)) = (ptr.context_buffer_root, ptr.bounds) else {
            return;
        };
        let safe_limit = bounds.max.saturating_sub(1);
        if safe_limit < 0 {
        ...
        state.refine_context_buffer_prefix_limit(root, safe_limit);
    }
