...
impl VccVerifier {
    ...
    pub(super) fn context_buffer_end_comparison(
        &self,
        lhs: VccValue,
        lhs_ty: VccValueType,
        rhs: VccValue,
        ...
        let normalize =
            |ptr_value: VccValue, ptr_ty: VccValueType, end_ty: VccValueType, op: VccBinOp| {
                ...
                if ptr.space != VccAddrSpace::Kernel
                    || end.space != VccAddrSpace::Kernel
                    || ptr.context_buffer_root.is_none()
                    || !end.context_buffer_end
                {
                    return None;
                }
                if !matches!(op, VccBinOp::Lt | VccBinOp::Le | VccBinOp::Gt | VccBinOp::Ge) {
