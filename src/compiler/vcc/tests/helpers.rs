...
fn test_verify_mir_bpf_spin_lock_rejects_calls_while_held() {
    ...
    let err = verify_mir(&func, &types).expect_err("expected call-in-lock errors");
    assert!(
        err.iter().any(|e| e
            .message
...
fn make_snprintf_vcc_call(str_size: i64, data_len: i64) -> (MirFunction, HashMap<VReg, MirType>) {
    let (mut func, entry) = new_mir_function();
    let fmt = func.alloc_vreg();
    let out_slot = func.alloc_stack_slot(32, 8, StackSlotKind::StringBuffer);
    let data_slot = func.alloc_stack_slot(16, 8, StackSlotKind::StringBuffer);
...
fn test_verify_mir_helper_snprintf_rejects_size_out_of_range() {
    for (str_size, data_len, expected) in [
        ...
        let (func, types) = make_snprintf_vcc_call(str_size, data_len);
...
fn test_verify_mir_for_probe_context_seq_write_rejects_negative_len() {
    ...
    assert!(
        err.iter().any(|e| e
            .message
            .contains("helper 'bpf_seq_write' requires arg2 len to be between 0 and u32::MAX")),
        "unexpected errors: {:?}",
        err
    );
}
...
fn test_verify_mir_for_probe_context_sys_bpf_requires_positive_attr_size() {
    ...
    let types = HashMap::from([(sys_bpf, MirType::I64)]);
    let probe_ctx = ProbeContext::new(EbpfProgramType::Syscall, "demo");
    let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
        .expect_err("expected bpf_sys_bpf attr-size error");
    assert!(
        err.iter()
...
fn test_verify_mir_for_probe_context_btf_find_by_name_kind_requires_zero_flags() {
    ...
    let probe_ctx = ProbeContext::new(EbpfProgramType::Syscall, "demo");
    let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
        .expect_err("expected btf_find_by_name_kind flags error");
    assert!(err.iter().any(|e| {
        e.message
...
#[test]
fn test_verify_mir_for_probe_context_kallsyms_lookup_name_requires_zero_flags() {
    let (mut func, entry) = new_mir_function();
    let name_slot = func.alloc_stack_slot(16, 1, StackSlotKind::StringBuffer);
    let res_slot = func.alloc_stack_slot(8, 8, StackSlotKind::StringBuffer);
    let kallsyms = func.alloc_vreg();
    ...
    let types = HashMap::from([(kallsyms, MirType::I64)]);
    let probe_ctx = ProbeContext::new(EbpfProgramType::Syscall, "demo");
    let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
        .expect_err("expected kallsyms_lookup_name flags error");
    assert!(err.iter().any(|e| {
        e.message
...
#[test]
fn test_verify_mir_helper_per_cpu_ptr_helpers_reject_unchecked_nullable_kernel_pointer() {
...
#[test]
fn test_verify_mir_helper_per_cpu_ptr_helpers_reject_stack_pointer() {
    for helper in [BpfHelper::PerCpuPtr, BpfHelper::ThisCpuPtr] {
        let (mut func, entry) = new_mir_function();
...
#[test]
fn test_verify_mir_helper_ringbuf_reserve_dynptr_rejects_size_above_u32_max() {
    let (mut func, entry) = new_mir_function();
    let map_slot = func.alloc_stack_slot(8, 8, StackSlotKind::StringBuffer);
    let dynptr_slot = func.alloc_stack_slot(16, 8, StackSlotKind::StringBuffer);
    let dst = func.alloc_vreg();
...
#[test]
fn test_verify_mir_helper_ringbuf_reserve_helpers_reject_zero_size() {
...
#[test]
fn test_verify_mir_helper_ringbuf_subfn_submit_releases_caller_reference() {
    let (mut func, entry) = new_mir_function();
    let submit = func.alloc_block();
    let done = func.alloc_block();
    let map_slot = func.alloc_stack_slot(8, 8, StackSlotKind::StringBuffer);
...
#[test]
fn test_verify_mir_helper_ringbuf_subfn_reserve_return_releases_caller_reference() {
    let (mut func, entry) = new_mir_function();
    let submit = func.alloc_block();
    let done = func.alloc_block();
    let record = func.alloc_vreg();
...
fn make_cgroup_membership_vcc_call(
    helper: BpfHelper,
    idx: i64,
...
fn cgroup_membership_vcc_probe_context(helper: BpfHelper) -> ProbeContext {
    match helper {
...
fn test_verify_mir_for_probe_context_cgroup_membership_helpers_reject_invalid_idx() {
    for helper in [BpfHelper::SkbUnderCgroup, BpfHelper::CurrentTaskUnderCgroup] {
        for idx in [-1_i64, 0x1_0000_0000] {
            let (func, types) = make_cgroup_membership_vcc_call(helper, idx);
            let probe_ctx = cgroup_membership_vcc_probe_context(helper);
            let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
fn test_verify_mir_for_program_redirect_requires_zero_flags_in_xdp() {
    let (func, types) = make_redirect_vcc_call(1, 1);
...
fn make_redirect_vcc_call(ifindex: i64, flags: i64) -> (MirFunction, HashMap<VReg, MirType>) {
    let (mut func, entry) = new_mir_function();
    let dst = func.alloc_vreg();
...
fn test_verify_mir_for_program_redirect_rejects_invalid_ifindex() {
    for ifindex in [-1_i64, 0x1_0000_0000] {
        let (func, types) = make_redirect_vcc_call(ifindex, 0);
...
fn make_sysctl_get_name_vcc_call(flags: i64) -> (MirFunction, HashMap<VReg, MirType>) {
    let (mut func, entry) = new_mir_function();
    let ctx = func.alloc_vreg();
    let dst = func.alloc_vreg();
    let buf_slot = func.alloc_stack_slot(16, 8, StackSlotKind::StringBuffer);
...
fn test_verify_mir_sysctl_get_name_rejects_invalid_flags() {
    let (func, types) = make_sysctl_get_name_vcc_call(2);
...
fn test_verify_mir_for_program_sockopt_helpers_reject_invalid_program() {
    for (helper, probe_ctx, expected) in [
        ...
        let (func, types) = make_sockopt_helper_vcc_call(helper, 16, 16);
        let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
fn test_verify_mir_for_probe_context_sockopt_helpers_reject_optlen_above_i32_max() {
    for helper in [BpfHelper::SetSockOpt, BpfHelper::GetSockOpt] {
        let (func, types) = make_sockopt_helper_vcc_call(helper, 0x8000_0000, 16);
        let probe_ctx = ProbeContext::new(EbpfProgramType::CgroupSockopt, "/sys/fs/cgroup:get");
        let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
fn test_verify_mir_for_probe_context_sockopt_helpers_reject_zero_optlen() {
    for (helper, expected) in [
        ...
        let (func, types) = make_sockopt_helper_vcc_call(helper, 0, 16);
        let probe_ctx = ProbeContext::new(EbpfProgramType::CgroupSockopt, "/sys/fs/cgroup:get");
        let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
fn make_cgroup_retval_vcc_call(helper: BpfHelper) -> (MirFunction, HashMap<VReg, MirType>) {
    let (mut func, entry) = new_mir_function();
    let dst = func.alloc_vreg();
...
fn test_verify_mir_for_probe_context_cgroup_retval_helpers_accept_supported_contexts() {
    for (helper, probe_ctx) in [
        ...
        let (func, types) = make_cgroup_retval_vcc_call(helper);
        verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
fn test_verify_mir_for_program_bind_helper_rejects_invalid_program_or_attach() {
    for (probe_ctx, expected) in [
        ...
        let (func, types) = make_bind_helper_vcc_call(16, 16);
        let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
fn test_verify_mir_for_probe_context_bind_helper_rejects_addr_len_above_i32_max() {
    let (func, types) = make_bind_helper_vcc_call(0x8000_0000, 16);
    let probe_ctx = ProbeContext::new(EbpfProgramType::CgroupSockAddr, "/sys/fs/cgroup:connect4");
    let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
fn test_verify_mir_for_probe_context_bind_helper_rejects_zero_addr_len() {
    let (func, types) = make_bind_helper_vcc_call(0, 16);
    let probe_ctx = ProbeContext::new(EbpfProgramType::CgroupSockAddr, "/sys/fs/cgroup:connect4");
    let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
fn make_perf_prog_read_value_vcc_call(
    size: i64,
    buf_size: usize,
) -> (MirFunction, HashMap<VReg, MirType>) {
    let (mut func, entry) = new_mir_function();
...
fn test_verify_mir_perf_event_read_helpers() {
    for helper in [BpfHelper::PerfEventRead, BpfHelper::PerfEventReadValue] {
        let (func, types) = make_perf_event_read_vcc_call(helper, 0, 24, 24);
        verify_mir_for_program(&func, &types, EbpfProgramType::Xdp.info())
...
fn test_verify_mir_perf_prog_read_value_helper() {
    let (func, types) = make_perf_prog_read_value_vcc_call(24, 24);
...
fn test_verify_mir_perf_prog_read_value_requires_exact_size() {
    let (func, types) = make_perf_prog_read_value_vcc_call(8, 24);
...
fn test_verify_mir_perf_value_helpers_reject_zero_size() {
    let cases = [
        (
            make_perf_prog_read_value_vcc_call(0, 24),
            EbpfProgramType::PerfEvent,
            "helper 56 arg2 must be > 0",
        ),
        (
            make_perf_event_read_vcc_call(BpfHelper::PerfEventReadValue, 0, 0, 24),
            EbpfProgramType::Xdp,
...
fn test_verify_mir_perf_prog_read_value_rejects_small_buffer() {
    let (func, types) = make_perf_prog_read_value_vcc_call(24, 8);
    let err = verify_mir_for_program(&func, &types, EbpfProgramType::PerfEvent.info())
...
fn test_verify_mir_perf_event_read_value_requires_exact_size() {
    let (func, types) = make_perf_event_read_vcc_call(BpfHelper::PerfEventReadValue, 0, 8, 24);
    let err = verify_mir_for_program(&func, &types, EbpfProgramType::Xdp.info())
...
fn test_verify_mir_perf_event_read_value_rejects_small_buffer() {
    let (func, types) = make_perf_event_read_vcc_call(BpfHelper::PerfEventReadValue, 0, 24, 8);
    let err = verify_mir_for_program(&func, &types, EbpfProgramType::Xdp.info())
...
fn test_verify_mir_perf_event_read_helpers_reject_invalid_flags() {
    for helper in [BpfHelper::PerfEventRead, BpfHelper::PerfEventReadValue] {
        let (func, types) = make_perf_event_read_vcc_call(helper, 0x1_0000_0000, 24, 24);
        let err = verify_mir_for_program(&func, &types, EbpfProgramType::Xdp.info())
...
fn make_get_ns_current_pid_tgid_vcc_call(
    size: i64,
    buf_size: usize,
) -> (MirFunction, HashMap<VReg, MirType>) {
    let (mut func, entry) = new_mir_function();
...
fn test_verify_mir_get_ns_current_pid_tgid_helper() {
    let (func, types) = make_get_ns_current_pid_tgid_vcc_call(8, 8);
    verify_mir_for_program(&func, &types, EbpfProgramType::Xdp.info())
...
fn test_verify_mir_get_ns_current_pid_tgid_rejects_zero_size() {
    let (func, types) = make_get_ns_current_pid_tgid_vcc_call(0, 8);
    let err = verify_mir_for_program(&func, &types, EbpfProgramType::Xdp.info())
...
fn test_verify_mir_get_ns_current_pid_tgid_requires_exact_size() {
    let (func, types) = make_get_ns_current_pid_tgid_vcc_call(4, 8);
    let err = verify_mir_for_program(&func, &types, EbpfProgramType::Xdp.info())
...
fn test_verify_mir_get_ns_current_pid_tgid_rejects_small_buffer() {
    let (func, types) = make_get_ns_current_pid_tgid_vcc_call(8, 4);
    let err = verify_mir_for_program(&func, &types, EbpfProgramType::Xdp.info())
...
fn test_verify_mir_strtox_helpers() {
    for helper in [BpfHelper::Strtol, BpfHelper::Strtoul] {
        let (func, types) = make_strtox_vcc_call(helper, 8, 16, 8);
        verify_mir_for_program(&func, &types, EbpfProgramType::Xdp.info())
...
fn test_verify_mir_strtox_helpers_reject_zero_buf_len() {
    for helper in [BpfHelper::Strtol, BpfHelper::Strtoul] {
        let (func, types) = make_strtox_vcc_call(helper, 0, 0, 8);
        let err = verify_mir_for_program(&func, &types, EbpfProgramType::Xdp.info())
...
fn test_verify_mir_strtox_helper_rejects_small_buffer() {
    let (func, types) = make_strtox_vcc_call(BpfHelper::Strtol, 16, 0, 8);
    let err = verify_mir_for_program(&func, &types, EbpfProgramType::Xdp.info())
...
fn test_verify_mir_strtox_helper_rejects_invalid_flags() {
    for (helper, flags) in [(BpfHelper::Strtol, 2), (BpfHelper::Strtoul, 32)] {
        let (func, types) = make_strtox_vcc_call(helper, 8, flags, 8);
        let err = verify_mir_for_program(&func, &types, EbpfProgramType::Xdp.info())
...
fn make_csum_update_vcc_call(csum: i64) -> (MirFunction, HashMap<VReg, MirType>) {
    let (mut func, entry) = new_mir_function();
    let ctx = func.alloc_vreg();
    let dst = func.alloc_vreg();
...
#[test]
fn test_verify_mir_for_probe_context_csum_update_rejects_invalid_csum() {
    for csum in [-1_i64, 0x1_0000_0000] {
        let (func, types) = make_csum_update_vcc_call(csum);
        let probe_ctx = ProbeContext::new(EbpfProgramType::Tc, "lo:ingress");
        let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
fn make_set_hash_vcc_call(hash: i64) -> (MirFunction, HashMap<VReg, MirType>) {
    let (mut func, entry) = new_mir_function();
    let ctx = func.alloc_vreg();
    let dst = func.alloc_vreg();
...
#[test]
fn test_verify_mir_for_probe_context_set_hash_rejects_invalid_hash() {
    for hash in [-1_i64, 0x1_0000_0000] {
        let (func, types) = make_set_hash_vcc_call(hash);
        let probe_ctx = ProbeContext::new(EbpfProgramType::Tc, "lo:ingress");
        let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
#[test]
fn test_verify_mir_for_probe_context_clone_redirect_rejects_invalid_ifindex() {
    for ifindex in [-1_i64, 0x1_0000_0000] {
        let (func, types) = make_clone_redirect_vcc_call(ifindex, 0);
        let probe_ctx = ProbeContext::new(EbpfProgramType::Tc, "lo:ingress");
        let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
fn make_skb_adjust_room_vcc_call_with_flags(
    ...
    make_skb_adjust_room_vcc_call_with_len_diff(14, mode, flags)
}

fn make_skb_adjust_room_vcc_call_with_len_diff(
    len_diff: i64,
    mode: i64,
    flags: i64,
) -> (MirFunction, HashMap<VReg, MirType>) {
...
fn test_verify_mir_for_probe_context_skb_adjust_room_rejects_invalid_mode() {
    let (func, types) = make_skb_adjust_room_vcc_call(2);
    let probe_ctx = ProbeContext::new(EbpfProgramType::Tc, "lo:ingress");
    let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
fn test_verify_mir_for_probe_context_skb_adjust_room_rejects_len_diff_out_of_range() {
    for len_diff in [0x1000_i64, -0x1000] {
        let (func, types) = make_skb_adjust_room_vcc_call_with_len_diff(len_diff, 0, 0);
        let probe_ctx = ProbeContext::new(EbpfProgramType::Tc, "lo:ingress");
        let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
fn make_skb_change_proto_vcc_call(flags: i64) -> (MirFunction, HashMap<VReg, MirType>) {
    make_skb_change_proto_vcc_call_with_proto(0x86dd, flags)
}

fn make_skb_change_proto_vcc_call_with_proto(
    proto: i64,
    flags: i64,
) -> (MirFunction, HashMap<VReg, MirType>) {
    let (mut func, entry) = new_mir_function();
...
fn test_verify_mir_for_probe_context_skb_change_proto_rejects_invalid_proto() {
    let (func, types) = make_skb_change_proto_vcc_call_with_proto(0x0806, 0);
    let probe_ctx = ProbeContext::new(EbpfProgramType::Tc, "lo:ingress");
    let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
fn make_csum_replace_vcc_call_with_vreg_flags(
    helper: BpfHelper,
    from: i64,
    flags_value: i64,
) -> (MirFunction, HashMap<VReg, MirType>) {
...
fn test_verify_mir_for_probe_context_csum_replace_helpers_reject_invalid_flags() {
    for (helper, flags, expected) in [
        ...
        let (func, types) = make_csum_replace_vcc_call(helper, 0, 0, flags);
        let probe_ctx = ProbeContext::new(EbpfProgramType::Tc, "lo:ingress");
        let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
fn test_verify_mir_for_probe_context_csum_replace_helpers_reject_nonzero_from_with_diff_size() {
    for (helper, flags) in [
        ...
        let (func, types) = make_csum_replace_vcc_call(helper, 0, 1, flags);
        let probe_ctx = ProbeContext::new(EbpfProgramType::Tc, "lo:ingress");
        let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
#[test]
fn test_verify_mir_for_probe_context_csum_replace_helpers_reject_nonzero_from_with_vreg_diff_size()
{
    let (func, types) =
        make_csum_replace_vcc_call_with_vreg_flags(BpfHelper::L4CsumReplace, 1, 0x10);
    let probe_ctx = ProbeContext::new(EbpfProgramType::Tc, "lo:ingress");
    let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
        .expect_err("expected checksum replacement helper from validation error");
    assert!(
...
fn test_verify_mir_for_probe_context_csum_replace_helpers_reject_invalid_offsets() {
    for (helper, offset, expected) in [
        ...
        let (func, types) = make_csum_replace_vcc_call(helper, offset, 0, 2);
        let probe_ctx = ProbeContext::new(EbpfProgramType::Tc, "lo:ingress");
        let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
fn make_csum_level_vcc_call(level: i64) -> (MirFunction, HashMap<VReg, MirType>) {
    let (mut func, entry) = new_mir_function();
    let ctx = func.alloc_vreg();
    let dst = func.alloc_vreg();
...
fn test_verify_mir_for_probe_context_csum_level_helper_rejects_invalid_level() {
    let (func, types) = make_csum_level_vcc_call(4);
    let probe_ctx = ProbeContext::new(EbpfProgramType::Tc, "lo:ingress");
    let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
fn make_get_branch_snapshot_vcc_call(
    size: i64,
    buf_size: usize,
    flags: i64,
) -> (MirFunction, HashMap<VReg, MirType>) {
...
fn test_verify_mir_get_branch_snapshot_helper() {
    let (func, types) = make_get_branch_snapshot_vcc_call(24, 24, 0);
    verify_mir_for_program(&func, &types, EbpfProgramType::Xdp.info())
...
fn test_verify_mir_get_branch_snapshot_rejects_small_buffer() {
    let (func, types) = make_get_branch_snapshot_vcc_call(24, 8, 0);
    let err = verify_mir_for_program(&func, &types, EbpfProgramType::Xdp.info())
...
fn test_verify_mir_get_branch_snapshot_rejects_size_over_u32() {
    let (func, types) = make_get_branch_snapshot_vcc_call(0x1_0000_0000, 8, 0);
    let err = verify_mir_for_program(&func, &types, EbpfProgramType::Xdp.info())
...
fn test_verify_mir_get_branch_snapshot_requires_zero_flags() {
    let (func, types) = make_get_branch_snapshot_vcc_call(24, 24, 1);
    let err = verify_mir_for_program(&func, &types, EbpfProgramType::Xdp.info())
...
fn test_verify_mir_for_probe_context_lirc_helpers_reject_out_of_range_scalars() {
    for (helper, scalar_args, expected) in [
        ...
        let (func, types) = make_lirc_scalar_vcc_call(helper, scalar_args);
...
fn test_verify_mir_for_probe_context_packet_byte_helpers_reject_zero_len() {
    for helper in [
        ...
        let (func, types) = make_skb_bytes_vcc_call(helper, 0, 0);
...
fn test_verify_mir_for_probe_context_xdp_byte_helpers_reject_invalid_offset_or_len() {
    for (helper, offset, len, expected) in [
        ...
        let (func, types) = make_xdp_bytes_vcc_call(helper, offset, len);
        let probe_ctx = ProbeContext::new(EbpfProgramType::Xdp, "lo");
        let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
#[test]
fn test_verify_mir_for_probe_context_skb_load_bytes_relative_rejects_invalid_start_header() {
    let (mut func, entry) = new_mir_function();
    let ctx = func.alloc_vreg();
    let dst = func.alloc_vreg();
    let buf_slot = func.alloc_stack_slot(16, 8, StackSlotKind::StringBuffer);
...
fn make_socket_lookup_vcc_call(
    helper: BpfHelper,
    flags: i64,
) -> (MirFunction, HashMap<VReg, MirType>) {
...
fn test_verify_mir_for_probe_context_socket_lookup_helpers_reject_nonzero_flags() {
    for helper in [
        ...
        let (func, types) = make_socket_lookup_vcc_call(helper, 1);
        let probe_ctx = ProbeContext::new(EbpfProgramType::Xdp, "lo");
        let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
fn test_verify_mir_for_probe_context_socket_lookup_rejects_tuple_size_above_u32_max() {
    for helper in [
        ...
        let (func, types) =
            make_socket_lookup_vcc_call_with_tuple_size(helper, u32::MAX as i64 + 1, 0);
        let probe_ctx = ProbeContext::new(EbpfProgramType::Xdp, "lo");
        let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
fn test_verify_mir_for_probe_context_socket_lookup_rejects_invalid_tuple_size() {
    for helper in [
        ...
        for tuple_size in [0, 16] {
            let (func, types) = make_socket_lookup_vcc_call_with_tuple_size(helper, tuple_size, 0);
            let probe_ctx = ProbeContext::new(EbpfProgramType::Xdp, "lo");
            let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
                .expect_err("expected socket lookup tuple_size exact-size error");
            let expected = if tuple_size == 0 {
                "arg2 must be > 0"
            } else {
...
fn make_skb_change_tail_vcc_call(
    new_len: i64,
    flags: i64,
) -> (MirFunction, HashMap<VReg, MirType>) {
    let (mut func, entry) = new_mir_function();
...
fn make_skb_pull_data_vcc_call(len: i64) -> (MirFunction, HashMap<VReg, MirType>) {
    let (mut func, entry) = new_mir_function();
    let ctx = func.alloc_vreg();
    let dst = func.alloc_vreg();
...
fn test_verify_mir_for_probe_context_skb_change_tail_rejects_invalid_new_len() {
    for new_len in [-1_i64, 0x8000_0000] {
        let (func, types) = make_skb_change_tail_vcc_call(new_len, 0);
        let probe_ctx = ProbeContext::new(EbpfProgramType::Tc, "lo:ingress");
        let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
fn test_verify_mir_for_probe_context_skb_pull_data_rejects_invalid_len() {
    for len in [-1_i64, 0x1_0000_0000] {
        let (func, types) = make_skb_pull_data_vcc_call(len);
        let probe_ctx = ProbeContext::new(EbpfProgramType::Tc, "lo:ingress");
        let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
fn test_verify_mir_for_probe_context_check_mtu_rejects_unknown_tc_flags() {
    let (func, types) = make_check_mtu_vcc_call(0x02, 4);
    let probe_ctx = ProbeContext::new(EbpfProgramType::Tc, "lo:ingress");
    let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
fn test_verify_mir_for_probe_context_check_mtu_rejects_known_mtu_len_with_segment_flags() {
    let (func, types) = make_check_mtu_vcc_call_with_len_diff_and_mtu_len_value(1, 4, 0, Some(1));
    let probe_ctx = ProbeContext::new(EbpfProgramType::Tc, "lo:ingress");
    let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
fn test_verify_mir_for_probe_context_fib_lookup_rejects_zero_plen() {
    let (func, types) = make_fib_lookup_vcc_call(0, 64, 0);
    let probe_ctx = ProbeContext::new(EbpfProgramType::Xdp, "lo");
    let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
fn test_verify_mir_for_probe_context_skb_tunnel_helpers_accept_tc_and_lwt_xmit_programs() {
    for helper in [
        ...
        for probe_ctx in [
            ...
            let (func, types) = make_skb_tunnel_vcc_call(helper, size, size as usize, 0);
            verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
fn test_verify_mir_for_probe_context_skb_tunnel_helper_rejects_small_buffer() {
    let (func, types) = make_skb_tunnel_vcc_call(BpfHelper::SkbGetTunnelKey, 44, 8, 0);
    let probe_ctx = ProbeContext::new(EbpfProgramType::Tc, "lo:ingress");
    let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
fn test_verify_mir_for_probe_context_skb_tunnel_helpers_reject_zero_size() {
    for helper in [
        ...
        let (func, types) = make_skb_tunnel_vcc_call(helper, 0, 16, 0);
        let probe_ctx = ProbeContext::new(EbpfProgramType::Tc, "lo:ingress");
        let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
fn test_verify_mir_for_probe_context_skb_tunnel_key_helpers_reject_invalid_size() {
    let (func, types) = make_skb_tunnel_vcc_call(BpfHelper::SkbGetTunnelKey, 16, 16, 0);
    let probe_ctx = ProbeContext::new(EbpfProgramType::Tc, "lo:ingress");
    let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
fn test_verify_mir_for_probe_context_skb_get_tunnel_option_helper_rejects_size_above_u32() {
    let (func, types) = make_skb_tunnel_vcc_call(BpfHelper::SkbGetTunnelOpt, 0x1_0000_0000, 16, 0);
    let probe_ctx = ProbeContext::new(EbpfProgramType::Tc, "lo:ingress");
    let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
fn test_verify_mir_skb_set_tunnel_option_rejects_size_above_opts_max() {
    let (func, types) = make_skb_tunnel_vcc_call(BpfHelper::SkbSetTunnelOpt, 256, 256, 0);
    let probe_ctx = ProbeContext::new(EbpfProgramType::Tc, "lo:ingress");
    let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
fn test_verify_mir_for_probe_context_skb_set_tunnel_option_helper_rejects_unaligned_size() {
    let (func, types) = make_skb_tunnel_vcc_call(BpfHelper::SkbSetTunnelOpt, 18, 18, 0);
    let probe_ctx = ProbeContext::new(EbpfProgramType::Tc, "lo:ingress");
    let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
fn test_verify_mir_for_probe_context_skb_tunnel_key_helpers_reject_invalid_flags() {
    for (helper, flags, expected) in [
        ...
        let (func, types) = make_skb_tunnel_vcc_call(helper, 44, 44, flags);
        let probe_ctx = ProbeContext::new(EbpfProgramType::Tc, "lo:ingress");
        let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
fn make_skb_get_xfrm_state_vcc_call(
    ...
    make_skb_get_xfrm_state_vcc_call_with_index(0, flags, size, buffer_size)
}

fn make_skb_get_xfrm_state_vcc_call_with_index(
    index: i64,
    flags: i64,
    size: i64,
    buffer_size: usize,
...
fn test_verify_mir_for_probe_context_skb_get_xfrm_state_accepts_tc_programs() {
    for probe_ctx in [
        ...
        let (func, types) = make_skb_get_xfrm_state_vcc_call(0, 28, 28);
        verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
fn test_verify_mir_for_probe_context_skb_get_xfrm_state_requires_zero_flags() {
    let (func, types) = make_skb_get_xfrm_state_vcc_call(1, 28, 28);
    let probe_ctx = ProbeContext::new(EbpfProgramType::Tc, "lo:ingress");
    let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
fn test_verify_mir_for_probe_context_skb_get_xfrm_state_rejects_index_above_u32() {
    let (func, types) = make_skb_get_xfrm_state_vcc_call_with_index(0x1_0000_0000, 0, 28, 28);
    let probe_ctx = ProbeContext::new(EbpfProgramType::Tc, "lo:ingress");
    let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
fn test_verify_mir_for_probe_context_skb_get_xfrm_state_rejects_size_above_u32() {
    let (func, types) = make_skb_get_xfrm_state_vcc_call(0, 0x1_0000_0000, 28);
    let probe_ctx = ProbeContext::new(EbpfProgramType::Tc, "lo:ingress");
    let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
fn test_verify_mir_for_probe_context_skb_get_xfrm_state_rejects_small_buffer() {
    let (func, types) = make_skb_get_xfrm_state_vcc_call(0, 28, 16);
    let probe_ctx = ProbeContext::new(EbpfProgramType::Tc, "lo:ingress");
    let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
fn test_verify_mir_for_probe_context_skb_get_xfrm_state_rejects_zero_size() {
    let (func, types) = make_skb_get_xfrm_state_vcc_call(0, 0, 28);
    let probe_ctx = ProbeContext::new(EbpfProgramType::Tc, "lo:ingress");
    let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
fn test_verify_mir_for_probe_context_skb_get_xfrm_state_rejects_size_not_struct_size() {
    let (func, types) = make_skb_get_xfrm_state_vcc_call(0, 16, 28);
    let probe_ctx = ProbeContext::new(EbpfProgramType::Tc, "lo:ingress");
    let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
fn make_lwt_buffer_vcc_call_with_selector(
    helper: BpfHelper,
    selector: i64,
    size: i64,
    buffer_size: usize,
...
fn make_lwt_buffer_vcc_call(
    ...
    make_lwt_buffer_vcc_call_with_selector(helper, selector, size, buffer_size)
}

fn make_lwt_seg6_adjust_srh_vcc_call_with_args(
    offset: i64,
    delta: i64,
) -> (MirFunction, HashMap<VReg, MirType>) {
    let (mut func, entry) = new_mir_function();
...
fn make_lwt_seg6_adjust_srh_vcc_call() -> (MirFunction, HashMap<VReg, MirType>) {
    make_lwt_seg6_adjust_srh_vcc_call_with_args(0, 4)
}
...
fn test_verify_mir_for_probe_context_lwt_push_encap_rejects_invalid_type() {
    let (func, types) = make_lwt_buffer_vcc_call_with_selector(BpfHelper::LwtPushEncap, 3, 16, 16);
...
fn test_verify_mir_for_probe_context_lwt_push_encap_requires_ip_type_on_lwt_xmit() {
    let (func, types) = make_lwt_buffer_vcc_call_with_selector(BpfHelper::LwtPushEncap, 0, 16, 16);
...
fn test_verify_mir_for_probe_context_lwt_seg6_helpers_accept_lwt_seg6local_programs() {
    for (func, types) in [
        ...
        make_lwt_buffer_vcc_call_with_selector(BpfHelper::LwtSeg6Action, 3, 4, 4),
...
fn test_verify_mir_for_probe_context_lwt_seg6_action_rejects_invalid_action() {
    let (func, types) = make_lwt_buffer_vcc_call_with_selector(BpfHelper::LwtSeg6Action, 4, 16, 16);
    let probe_ctx = ProbeContext::new(EbpfProgramType::LwtSeg6Local, "demo-route");
    let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
fn test_verify_mir_for_probe_context_lwt_seg6_action_rejects_invalid_param_len_for_action() {
    for (action, size, expected) in [
        ...
        let (func, types) =
            make_lwt_buffer_vcc_call_with_selector(BpfHelper::LwtSeg6Action, action, size, 16);
        let probe_ctx = ProbeContext::new(EbpfProgramType::LwtSeg6Local, "demo-route");
        let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
fn test_verify_mir_for_probe_context_lwt_buffer_helpers_reject_size_over_u32() {
    for (helper, program_type, target) in [
        ...
        let (func, types) = make_lwt_buffer_vcc_call(helper, 0x1_0000_0000, 16);
        let probe_ctx = ProbeContext::new(program_type, target);
        let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
fn test_verify_mir_for_probe_context_lwt_buffer_helpers_reject_zero_size() {
    for (helper, program_type, target) in [
        ...
        let (func, types) = make_lwt_buffer_vcc_call(helper, 0, 16);
        let probe_ctx = ProbeContext::new(program_type, target);
        let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
fn test_verify_mir_for_probe_context_lwt_seg6_helpers_reject_offset_over_u32() {
    for (func, types, helper_name) in [
        {
            let (func, types) = make_lwt_buffer_vcc_call_with_selector(
                BpfHelper::LwtSeg6StoreBytes,
                0x1_0000_0000,
                16,
                16,
        ...
        {
            let (func, types) = make_lwt_seg6_adjust_srh_vcc_call_with_args(0x1_0000_0000, 4);
...
fn test_verify_mir_for_probe_context_lwt_seg6_adjust_srh_rejects_delta_outside_i32_range() {
    for delta in [i32::MAX as i64 + 1, i32::MIN as i64 - 1] {
        let (func, types) = make_lwt_seg6_adjust_srh_vcc_call_with_args(0, delta);
        let probe_ctx = ProbeContext::new(EbpfProgramType::LwtSeg6Local, "demo-route");
        let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
fn make_msg_data_vcc_call(helper: BpfHelper, flags: i64) -> (MirFunction, HashMap<VReg, MirType>) {
    make_msg_data_vcc_call_with_range(helper, 0, 8, flags)
}

fn make_msg_byte_count_vcc_call(
    helper: BpfHelper,
    bytes: i64,
) -> (MirFunction, HashMap<VReg, MirType>) {
    let (mut func, entry) = new_mir_function();
...
fn make_msg_data_vcc_call_with_range(
    helper: BpfHelper,
    start: i64,
    end: i64,
    flags: i64,
...
fn test_verify_mir_for_probe_context_msg_byte_count_helpers_reject_invalid_bytes() {
    for helper in [BpfHelper::MsgApplyBytes, BpfHelper::MsgCorkBytes] {
        for bytes in [-1_i64, 0x1_0000_0000] {
            let (func, types) = make_msg_byte_count_vcc_call(helper, bytes);
            let probe_ctx = ProbeContext::new(EbpfProgramType::SkMsg, "/sys/fs/bpf/demo_sockmap");
            let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
fn test_verify_mir_for_probe_context_msg_data_helpers_reject_nonzero_flags() {
    for helper in [
        ...
        let (func, types) = make_msg_data_vcc_call(helper, 1);
        let probe_ctx = ProbeContext::new(EbpfProgramType::SkMsg, "/sys/fs/bpf/demo_sockmap");
        let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
fn test_verify_mir_for_probe_context_msg_data_helpers_reject_invalid_u32_args() {
    for (helper, start, end_or_len, expected) in [
        ...
        let (func, types) = make_msg_data_vcc_call_with_range(helper, start, end_or_len, 0);
        let probe_ctx = ProbeContext::new(EbpfProgramType::SkMsg, "/sys/fs/bpf/demo_sockmap");
        let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
fn test_verify_mir_for_probe_context_msg_pull_data_rejects_invalid_range() {
    for (start, end) in [(8, 8), (9, 8)] {
        let (func, types) =
            make_msg_data_vcc_call_with_range(BpfHelper::MsgPullData, start, end, 0);
        let probe_ctx = ProbeContext::new(EbpfProgramType::SkMsg, "/sys/fs/bpf/demo_sockmap");
        let err = verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
#[test]
fn test_verify_mir_helper_ringbuf_submit_rejected_after_partial_reserve_join() {
    let (mut func, entry) = new_mir_function();
    ...
    assert!(
        err.iter().any(|e| {
            e.message.contains("ringbuf record pointer is not tracked")
...
#[test]
fn test_verify_mir_helper_get_stack_rejects_user_ctx_pointer() {
    let (mut func, entry) = new_mir_function();
    let call = func.alloc_block();
    let done = func.alloc_block();
    let ctx = func.alloc_vreg();
...
#[test]
fn test_verify_mir_helper_trace_printk_rejects_fmt_size_over_u32() {
    let (func, types) = make_trace_printk_vcc_call(0x1_0000_0000, 8);
...
#[test]
fn test_verify_mir_helper_trace_vprintk_rejects_size_out_of_range() {
    for (fmt_size, data_len, expected) in [
        ...
        let (func, types) = make_trace_vprintk_vcc_call(fmt_size, 8, data_len, 16);
...
fn make_tcp_syncookie_vcc_call(
    helper: BpfHelper,
    iph_len: i64,
    th_len: i64,
) -> (MirFunction, HashMap<VReg, MirType>) {
...
fn test_verify_mir_helper_tcp_check_syncookie_rejects_lengths_above_u32_max() {
    for (iph_len, th_len, expected) in [
        ...
        let (func, types) =
            make_tcp_syncookie_vcc_call(BpfHelper::TcpCheckSyncookie, iph_len, th_len);
        let err =
            verify_mir(&func, &types).expect_err("expected tcp_check_syncookie length range error");
        assert!(
            err.iter().any(|e| e.message.contains(expected)),
            "unexpected error messages: {:?}",
            err
...
#[test]
fn test_verify_mir_helper_tcp_gen_syncookie_rejects_lengths_above_u32_max() {
    for (iph_len, th_len, expected) in [
        (
            u32::MAX as i64 + 1,
            20,
        ...
        let (func, types) =
            make_tcp_syncookie_vcc_call(BpfHelper::TcpGenSyncookie, iph_len, th_len);
        let err =
            verify_mir(&func, &types).expect_err("expected tcp_gen_syncookie length range error");
        assert!(
            err.iter().any(|e| e.message.contains(expected)),
            "unexpected error messages: {:?}",
            err
...
#[test]
fn test_verify_mir_helper_tcp_raw_syncookie_gen_accepts_zero_size_null_tcp_header() {
...
#[test]
fn test_verify_mir_helper_tcp_raw_syncookie_rejects_len_above_u32_max() {
    for helper in [
        ...
        func.block_mut(entry)
            ...
            .push(MirInst::CallHelper {
                ...
                args: vec![
                    MirValue::StackSlot(ip_slot),
                    MirValue::StackSlot(th_slot),
                    MirValue::Const(u32::MAX as i64 + 1),
                ],
            });
        func.block_mut(entry).terminator = MirInst::Return { val: None };
        ...
        let err = verify_mir(&func, &types).expect_err("expected raw syncookie length range error");
        assert!(
            err.iter().any(|e| e.message.contains(
...
#[test]
fn test_verify_mir_helper_get_local_storage_accepts_zero_flags() {
    let (mut func, entry) = new_mir_function();
    let map_slot = func.alloc_stack_slot(8, 8, StackSlotKind::StringBuffer);
    let storage = func.alloc_vreg();
    func.block_mut(entry)
...
#[test]
fn test_verify_mir_helper_get_local_storage_rejects_nonzero_flags() {
    let (mut func, entry) = new_mir_function();
    let map_slot = func.alloc_stack_slot(8, 8, StackSlotKind::StringBuffer);
    let storage = func.alloc_vreg();
    func.block_mut(entry)
...
#[test]
fn test_verify_mir_helper_get_local_storage_rejects_non_cgroup_program() {
    let (mut func, entry) = new_mir_function();
    let map_slot = func.alloc_stack_slot(8, 8, StackSlotKind::StringBuffer);
    let storage = func.alloc_vreg();
    func.block_mut(entry)
...
#[test]
fn test_verify_mir_helper_cgrp_storage_get_allows_maybe_null_cgroup_pointer() {
    let (mut func, entry) = new_mir_function();
    func.param_count = 1;
...
#[test]
fn test_verify_mir_helper_kptr_xchg_rejects_unchecked_map_dst_arg0() {
    let (mut func, entry) = new_mir_function();
    func.param_count = 1;
...
#[test]
fn test_verify_mir_helper_kptr_xchg_rejects_unchecked_kernel_ptr_arg1() {
    let (mut func, entry) = new_mir_function();
    let call = func.alloc_block();
    let done = func.alloc_block();
    func.param_count = 2;
...
#[test]
fn test_verify_mir_helper_kptr_xchg_clear_returns_typed_cgroup_reference() {
    let (mut func, entry) = new_mir_function();
    let swap = func.alloc_block();
    let release_old = func.alloc_block();
    let done = func.alloc_block();
...
#[test]
fn test_verify_mir_helper_sk_release_accepts_explicit_null_after_lookup_join() {
    let (mut func, entry) = new_mir_function();
    let lookup = func.alloc_block();
    let null_path = func.alloc_block();
    let join = func.alloc_block();
    ...
    verify_mir(&func, &types)
        .expect("explicit null/socket-ref join should preserve socket ref identity until guard");
}

#[test]
fn test_verify_mir_helper_sk_release_accepts_explicit_null_phi_after_lookup_join() {
    let (mut func, entry) = new_mir_function();
    let lookup = func.alloc_block();
    let null_path = func.alloc_block();
    let join = func.alloc_block();
    ...
    func.block_mut(join).instructions.push(MirInst::Phi {
        dst: sock,
        args: vec![(lookup, acquired), (null_path, null_sock)],
    });
    func.block_mut(join).instructions.push(MirInst::BinOp {
        dst: sock_non_null,
        op: BinOpKind::Ne,
    ...
    types.insert(select_cond, MirType::Bool);
    types.insert(null_sock, MirType::I64);
    for reg in [acquired, sock] {
        types.insert(
            reg,
            MirType::Ptr {
                pointee: Box::new(MirType::Unknown),
    ...
    verify_mir(&func, &types)
        .expect("explicit null/socket-ref phi should preserve socket ref identity until guard");
}

#[test]
fn test_verify_mir_helper_sk_release_rejects_nonzero_scalar_phi_after_lookup_join() {
    let (mut func, entry) = new_mir_function();
    let lookup = func.alloc_block();
    let scalar_path = func.alloc_block();
    let join = func.alloc_block();
    let release = func.alloc_block();
    let done = func.alloc_block();
    func.param_count = 1;
    ...
    let tuple_slot = func.alloc_stack_slot(16, 8, StackSlotKind::StringBuffer);
    let acquired = func.alloc_vreg();
    let scalar_sock = func.alloc_vreg();
    let sock = func.alloc_vreg();
    let sock_non_null = func.alloc_vreg();
    let release_ret = func.alloc_vreg();
    ...
    func.block_mut(scalar_path)
        .instructions
        .push(MirInst::Copy {
            dst: scalar_sock,
            src: MirValue::Const(1),
        });
    func.block_mut(scalar_path).terminator = MirInst::Jump { target: join };

    func.block_mut(join).instructions.push(MirInst::Phi {
        dst: sock,
        args: vec![(lookup, acquired), (scalar_path, scalar_sock)],
    });
    func.block_mut(join).instructions.push(MirInst::BinOp {
        dst: sock_non_null,
        op: BinOpKind::Ne,
    ...
    types.insert(select_cond, MirType::Bool);
    types.insert(scalar_sock, MirType::I64);
    for reg in [acquired, sock] {
        types.insert(
            reg,
            MirType::Ptr {
    ...
    let err = verify_mir(&func, &types)
        .expect_err("nonzero scalar/socket-ref phi should not release as a socket ref");
    assert!(
        err.iter().any(|e| {
            e.message.contains("requires pointer operand")
                || e.message.contains("expects acquired socket reference")
                || e.message.contains("expects socket reference")
                || e.message
                    .contains("requires kernel socket reference pointer")
        }),
        "unexpected error messages: {:?}",
        err
    );
...
#[test]
fn test_verify_mir_helper_sk_release_rejects_use_after_release_projection() {
    let (mut func, entry) = new_mir_function();
    let lookup = func.alloc_block();
    let release = func.alloc_block();
    let done = func.alloc_block();
    ...
    let err = verify_mir(&func, &types).expect_err("expected sk_release use-after-release error");
    assert!(
        err.iter()
            .any(|e| e.message.contains("uses released reference")),
        "unexpected error messages: {:?}",
        err
    );
}
...
#[test]
fn test_verify_mir_helper_sk_storage_get_rejects_non_socket_reference() {
    let (mut func, entry) = new_mir_function();
    let call = func.alloc_block();
    let done = func.alloc_block();
    ...
    let err = verify_mir(&func, &types).expect_err("expected sk_storage_get ref-kind mismatch");
    assert!(
        err.iter().any(|e| e
            .message
            .contains("helper 107 arg1 expects socket reference, got task reference")),
        "unexpected error messages: {:?}",
        err
    );
}
...
#[test]
fn test_verify_mir_helper_sk_storage_delete_rejects_non_socket_reference() {
    let (mut func, entry) = new_mir_function();
    let call = func.alloc_block();
    let done = func.alloc_block();
    ...
    assert!(
        err.iter().any(|e| e
            .message
            .contains("helper 108 arg1 expects socket reference, got task reference")),
        "unexpected error messages: {:?}",
        err
    );
}
...
fn test_verify_mir_helper_additional_skc_casts_reject_non_socket_reference() {
    let helpers = [
        ...
        (BpfHelper::SkcToMptcpSock, 196u32),
...
fn make_get_task_stack_vcc_call(
    size: i64,
    buf_size: usize,
) -> (MirFunction, HashMap<VReg, MirType>) {
    make_get_task_stack_vcc_call_with_flags(size, buf_size, 0)
}

fn make_get_task_stack_vcc_call_with_flags(
    size: i64,
    buf_size: usize,
    flags: i64,
) -> (MirFunction, HashMap<VReg, MirType>) {
...
fn test_verify_mir_helper_get_task_stack_accepts_current_task() {
    let (func, types) = make_get_task_stack_vcc_call(24, 24);
    let probe_ctx = ProbeContext::new(EbpfProgramType::Kprobe, "tcp_connect");
    verify_mir_for_probe_context(&func, &types, &probe_ctx)
...
fn test_verify_mir_helper_get_task_stack_rejects_build_id_without_user_stack() {
    let (func, types) = make_get_task_stack_vcc_call_with_flags(24, 24, 0x0800);
    let err = verify_mir(&func, &types)
...
fn test_verify_mir_helper_get_task_stack_rejects_small_buffer() {
    let (func, types) = make_get_task_stack_vcc_call(24, 8);
...
fn test_verify_mir_helper_get_task_stack_rejects_negative_size() {
    let (func, types) = make_get_task_stack_vcc_call(-1, 8);
...
fn test_verify_mir_helper_get_task_stack_rejects_size_over_u32() {
    let (func, types) = make_get_task_stack_vcc_call(0x1_0000_0000, 8);
...
fn make_trampoline_arg_vcc_call(
    helper: BpfHelper,
    output_size: usize,
    ctx_non_null: bool,
    arg_index: i64,
...
fn test_verify_mir_trampoline_arg_helpers_reject_small_output_buffer() {
    for helper in [BpfHelper::GetFuncArg, BpfHelper::GetFuncRet] {
        ...
        assert!(
            err.iter().any(|e| e.kind == VccErrorKind::PointerBounds),
            "expected pointer bounds error for {helper:?}, got {:?}",
            err
        );
    }
}
...
fn test_verify_mir_helper_d_path_rejects_negative_size() {
    let (func, types) = make_d_path_vcc_call(-1, 8);
...
fn test_verify_mir_helper_d_path_rejects_size_over_u32() {
    let (func, types) = make_d_path_vcc_call(0x1_0000_0000, 8);
...
fn test_verify_mir_helper_bprm_opts_set_rejects_invalid_flags() {
    let (func, types) = make_bprm_opts_set_vcc_call(2);
...
fn test_verify_mir_helper_ima_hash_helpers_require_positive_size() {
    for (helper, object_type_name, expected) in [
        ...
        let (func, types) = make_ima_hash_vcc_call(helper, object_type_name, 0, 16);
...
fn test_verify_mir_helper_ima_inode_hash_rejects_size_over_u32() {
    let (func, types) = make_ima_hash_vcc_call(BpfHelper::ImaInodeHash, "inode", 0x1_0000_0000, 16);
...
fn test_verify_mir_helper_ima_file_hash_rejects_size_over_u32() {
    let (func, types) = make_ima_hash_vcc_call(BpfHelper::ImaFileHash, "file", 0x1_0000_0000, 16);
...
fn make_copy_from_user_null_dst_vcc_call(
    size: i64,
    with_task: bool,
    flags: i64,
) -> (MirFunction, HashMap<VReg, MirType>) {
...
fn make_probe_write_user_vcc_call(
    size: i64,
    src_size: usize,
) -> (MirFunction, HashMap<VReg, MirType>) {
    let (mut func, entry) = new_mir_function();
...
fn make_probe_read_kernel_null_dst_vcc_call(size: i64) -> (MirFunction, HashMap<VReg, MirType>) {
    let (mut func, entry) = new_mir_function();
...
fn test_verify_mir_helper_copy_from_user_accepts_zero_size_null_dst() {
    let (func, types) = make_copy_from_user_null_dst_vcc_call(0, false, 0);
...
fn test_verify_mir_helper_copy_from_user_task_accepts_zero_size_null_dst() {
    let (func, types) = make_copy_from_user_null_dst_vcc_call(0, true, 0);
...
fn test_verify_mir_helper_probe_read_kernel_rejects_nonzero_size_null_dst() {
    let (func, types) = make_probe_read_kernel_null_dst_vcc_call(1);
    let err = verify_mir(&func, &types)
...
fn test_verify_mir_helper_override_return_accepts_ctx() {
    let (func, types) = make_override_return_vcc_call(false);
...
fn test_verify_mir_helper_copy_from_user_rejects_size_over_u32() {
    for with_task in [false, true] {
        let (func, types) = make_copy_from_user_vcc_call(0x1_0000_0000, 16, with_task, 0);
...
fn test_verify_mir_helper_copy_from_user_rejects_null_dst_with_nonzero_size() {
    let (func, types) = make_copy_from_user_null_dst_vcc_call(8, false, 0);
...
fn test_verify_mir_helper_copy_from_user_task_rejects_null_dst_with_nonzero_size() {
    let (func, types) = make_copy_from_user_null_dst_vcc_call(8, true, 0);
    let err =
...
fn test_verify_mir_helper_override_return_rejects_stack_ctx() {
    let (func, types) = make_override_return_vcc_call(true);
...
fn test_verify_mir_helper_probe_write_user_rejects_small_source_buffer() {
    let (func, types) = make_probe_write_user_vcc_call(16, 8);
...
fn make_guarded_sock_ops_hdr_opt_vcc_call(
    helper: BpfHelper,
    len: i64,
) -> (MirFunction, HashMap<VReg, MirType>) {
