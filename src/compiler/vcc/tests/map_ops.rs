...
#[test]
fn test_verify_mir_helper_call_rejects_unencodable_helper_id() {
    ...
    let err = verify_mir(&func, &HashMap::new())
        .expect_err("unencodable helper ID should fail VCC verification");
    assert!(
        err.iter().any(|err| {
            err.kind == VccErrorKind::UnsupportedInstruction
                && err
                    .message
...
fn test_verify_mir_dynamic_map_update_requires_null_check() {
    ...
    let err = verify_mir(&func, &types).expect_err("expected dynamic map null-check error");
    assert!(
        err.iter().any(|e| e.message.contains("dynamic map update")),
        "unexpected errors: {:?}",
        err
    );
}
...
#[test]
fn test_verify_mir_dynamic_map_delete_requires_null_check() {
    let (mut func, entry) = new_mir_function();
    let outer_key = func.alloc_vreg();
    let inner_key = func.alloc_vreg();
    let inner_map_ptr = func.alloc_vreg();
    ...
    let err = verify_mir(&func, &types).expect_err("expected dynamic map null-check error");
    assert!(
        err.iter().any(|e| e.message.contains("dynamic map delete")),
        "unexpected errors: {:?}",
        err
    );
}
...
#[test]
fn test_verify_mir_dynamic_map_update_and_delete_null_check_ok() {
    let (mut func, entry) = new_mir_function();
    let ok = func.alloc_block();
    let done = func.alloc_block();
    let outer_key = func.alloc_vreg();
    ...
    verify_mir(&func, &types).expect("expected null-checked dynamic map update/delete to pass");
}
...
#[test]
fn test_verify_mir_map_lookup_rejects_queue_map_kind() {
    let (mut func, entry) = new_mir_function();
    let key = func.alloc_vreg();
    let dst = func.alloc_vreg();
...
#[test]
fn test_verify_mir_map_lookup_accepts_lru_hash_kind() {
    let (mut func, entry) = new_mir_function();
    let load_block = func.alloc_block();
    let done = func.alloc_block();
    let key = func.alloc_vreg();
...
#[test]
fn test_verify_mir_map_lookup_rejects_out_of_bounds_key_pointer() {
    let (mut func, entry) = new_mir_function();
    let key = func.alloc_vreg();
    let dst = func.alloc_vreg();
    let key_slot = func.alloc_stack_slot(8, 8, StackSlotKind::StringBuffer);
...
fn test_verify_mir_map_push_rejects_out_of_bounds_value_pointer() {
    ...
    assert!(
        err.iter().any(|e| e.kind == VccErrorKind::PointerBounds
            && e.message.contains("map value out of bounds")),
        "unexpected error messages: {:?}",
        err
    );
}
...
#[test]
fn test_verify_mir_map_update_rejects_oversized_key_layout() {
    let (mut func, entry) = new_mir_function();
    ...
    let err = verify_mir(&func, &types).expect_err("expected oversized map key layout rejection");
    assert!(
        err.iter().any(|e| {
            e.kind == VccErrorKind::UnsupportedInstruction
                && e.message.contains("key size")
                && e.message.contains("u32 eBPF map definition range")
        }),
        "unexpected error messages: {:?}",
        err
    );
