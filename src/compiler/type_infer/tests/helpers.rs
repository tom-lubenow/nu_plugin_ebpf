...
#[test]
fn test_infer_for_each_map_elem_callback_subprogram_type() {
    let mut callback = MirFunction::with_name("for_each_cb");
    callback.param_count = 4;
    let callback_entry = callback.alloc_block();
    callback.entry = callback_entry;
    callback.block_mut(callback_entry).terminator = MirInst::Return {
        val: Some(MirValue::Const(0)),
    ...
    let types = ti
        .infer(&func)
        .expect("expected bpf_for_each_map_elem callback subprogram to infer");
...
#[test]
fn test_type_error_for_each_map_elem_rejects_wrong_callback_signature() {
    let mut callback = MirFunction::with_name("bad_for_each_cb");
    callback.param_count = 3;
    let callback_entry = callback.alloc_block();
    callback.entry = callback_entry;
    callback.block_mut(callback_entry).terminator = MirInst::Return {
...
#[test]
fn test_infer_find_vma_callback_subprogram_type() {
    ...
    let types = ti
        .infer(&func)
        .expect("expected bpf_find_vma callback subprogram to infer");

    match types.get(&callback_fn) {
        Some(MirType::Subprogram { args, ret }) => {
            assert_eq!(args.len(), 3);
            assert!(args.first().is_some_and(MirType::is_task_struct_ptr));
            assert!(args.get(1).is_some_and(MirType::is_vm_area_struct_ptr));
            assert!(matches!(
                args.get(2),
                Some(MirType::Ptr {
                    address_space: AddressSpace::Stack,
...
#[test]
fn test_type_error_find_vma_rejects_wrong_callback_signature() {
    let mut callback = MirFunction::with_name("bad_find_vma_cb");
    callback.param_count = 2;
    let callback_entry = callback.alloc_block();
    callback.entry = callback_entry;
    callback.block_mut(callback_entry).terminator = MirInst::Return {
    ...
    let errs = ti
        .infer(&func)
        .expect_err("expected bpf_find_vma callback signature error");
    assert!(errs.iter().any(|e| {
        e.message.contains(
...
#[test]
fn test_infer_snprintf_helper_accepts_zero_size_null_buffers() {
    let mut func = make_test_function();
    let fmt = func.alloc_vreg();
    let dst = func.alloc_vreg();
    let block = func.block_mut(BlockId(0));
    ...
    let types = ti
        .infer(&func)
        .expect("expected bpf_snprintf zero-size null buffers to infer");
    assert_eq!(types.get(&dst), Some(&MirType::I64));
}
...
#[test]
fn test_type_error_snprintf_rejects_size_out_of_range() {
...
#[test]
fn test_type_error_seq_output_helpers_reject_size_out_of_range() {
    let cases = [
        (
            BpfHelper::SeqPrintf,
            vec![
...
fn test_type_error_skb_adjust_room_helper_rejects_invalid_mode() {
    let func = make_skb_adjust_room_type_call(2, 0);
    let probe_ctx = ProbeContext::new(EbpfProgramType::Tc, "lo:ingress");
    let mut ti = TypeInference::new(Some(probe_ctx));
    let errs = ti
        .infer(&func)
...
#[test]
fn test_type_error_skb_adjust_room_helper_rejects_invalid_flags() {
    let func = make_skb_adjust_room_type_call(0, 1 << 20);
    let probe_ctx = ProbeContext::new(EbpfProgramType::Tc, "lo:ingress");
    let mut ti = TypeInference::new(Some(probe_ctx));
    let errs = ti
        .infer(&func)
...
#[test]
fn test_type_error_skb_adjust_room_helper_rejects_incompatible_flags() {
    let func = make_skb_adjust_room_type_call(0, 0x06);
    let probe_ctx = ProbeContext::new(EbpfProgramType::Tc, "lo:ingress");
    let mut ti = TypeInference::new(Some(probe_ctx));
    let errs = ti
        .infer(&func)
...
#[test]
fn test_type_error_skb_adjust_room_helper_rejects_sk_skb_nonzero_mode_and_flags() {
    let func = make_skb_adjust_room_type_call(1, 1);
...
fn make_skb_change_type_call() -> (MirFunction, VReg) {
    make_skb_change_type_call_with_type(0)
}

fn make_skb_change_type_call_with_type(pkt_type: i64) -> (MirFunction, VReg) {
    let mut func = make_test_function();
    let ctx = func.alloc_vreg();
    let dst = func.alloc_vreg();
    let block = func.block_mut(BlockId(0));
...
#[test]
fn test_type_error_skb_change_type_helper_rejects_invalid_type() {
    let (func, _) = make_skb_change_type_call_with_type(4);
    let probe_ctx = ProbeContext::new(EbpfProgramType::Tc, "lo:ingress");
    let mut ti = TypeInference::new(Some(probe_ctx));
    let errs = ti
        .infer(&func)
...
fn make_check_mtu_call_with_len_diff(
    ...
    make_check_mtu_call_with_ifindex_len_diff_and_mtu_len_value(
        0,
        flags,
        mtu_len_size,
        len_diff,
...
fn make_check_mtu_call_with_ifindex(
    ifindex: i64,
    flags: i64,
    mtu_len_size: usize,
) -> (MirFunction, VReg) {
...
fn make_check_mtu_call_with_ifindex_len_diff_and_mtu_len_value(
    ifindex: i64,
    flags: i64,
    mtu_len_size: usize,
    len_diff: i64,
...
#[test]
fn test_type_error_check_mtu_helper_rejects_ifindex_above_u32_max() {
    let (func, _) = make_check_mtu_call_with_ifindex(0x1_0000_0000, 0, 4);
    let probe_ctx = ProbeContext::new(EbpfProgramType::Tc, "lo:ingress");
    let mut ti = TypeInference::new(Some(probe_ctx));
    let errs = ti
        .infer(&func)
...
#[test]
fn test_type_error_check_mtu_helper_rejects_len_diff_above_i32_max() {
    let (func, _) = make_check_mtu_call_with_len_diff(0, 4, 0x8000_0000);
    let probe_ctx = ProbeContext::new(EbpfProgramType::Tc, "lo:ingress");
    let mut ti = TypeInference::new(Some(probe_ctx));
    let errs = ti
        .infer(&func)
...
#[test]
fn test_type_error_fib_lookup_helper_rejects_small_plen() {
    let (func, _) = make_fib_lookup_call(8, 64, 0);
    let probe_ctx = ProbeContext::new(EbpfProgramType::Xdp, "lo");
    let mut ti = TypeInference::new(Some(probe_ctx));
    let errs = ti
        .infer(&func)
...
#[test]
fn test_type_error_fib_lookup_helper_rejects_tbid_without_direct() {
    let (func, _) = make_fib_lookup_call(64, 64, 0x08);
    let probe_ctx = ProbeContext::new(EbpfProgramType::Xdp, "lo");
    let mut ti = TypeInference::new(Some(probe_ctx));
    let errs = ti
        .infer(&func)
...
#[test]
fn test_type_error_fib_lookup_helper_rejects_mark_with_direct() {
    let (func, _) = make_fib_lookup_call(64, 64, 0x21);
    let probe_ctx = ProbeContext::new(EbpfProgramType::Xdp, "lo");
    let mut ti = TypeInference::new(Some(probe_ctx));
    let errs = ti
        .infer(&func)
...
fn make_lwt_seg6_adjust_srh_call() -> (MirFunction, VReg) {
    make_lwt_seg6_adjust_srh_call_with_args(0, 4)
}
...
fn test_infer_lwt_seg6_helpers_in_lwt_seg6local_programs() {
    for (func, dst) in [
        ...
        make_lwt_buffer_helper_call_with_selector(BpfHelper::LwtSeg6Action, 3, 4, 4),
...
#[test]
fn test_type_error_lwt_seg6_action_rejects_invalid_param_len_for_action() {
    for (action, size, expected) in [
        ...
        let (func, _) =
            make_lwt_buffer_helper_call_with_selector(BpfHelper::LwtSeg6Action, action, size, 16);
        let probe_ctx = ProbeContext::new(EbpfProgramType::LwtSeg6Local, "demo-route");
        let mut ti = TypeInference::new(Some(probe_ctx));
        let errs = ti
            .infer(&func)
...
#[test]
fn test_type_error_lwt_buffer_helpers_reject_size_over_u32() {
    for (helper, program_type, target) in [
        ...
        let (func, _) = make_lwt_buffer_helper_call(helper, 0x1_0000_0000, 16);
...
fn test_type_error_lwt_seg6_helpers_reject_offset_over_u32() {
    for (func, helper_name) in [
        (
            make_lwt_buffer_helper_call_with_selector(
                BpfHelper::LwtSeg6StoreBytes,
                0x1_0000_0000,
                16,
                16,
        ...
        (
            make_lwt_seg6_adjust_srh_call_with_args(0x1_0000_0000, 4).0,
...
fn make_msg_data_call(helper: BpfHelper, flags: i64) -> (MirFunction, VReg) {
...
fn test_type_error_msg_data_helpers_reject_nonzero_flags() {
    for helper in [
        ...
        let (func, _) = make_msg_data_call(helper, 1);
        let probe_ctx = ProbeContext::new(EbpfProgramType::SkMsg, "/sys/fs/bpf/demo_sockmap");
        let mut ti = TypeInference::new(Some(probe_ctx));
        let errs = ti
            .infer(&func)
...
#[test]
fn test_type_error_msg_pull_data_rejects_invalid_range() {
    for (start, end) in [(8, 8), (9, 8)] {
        let (func, _) = make_msg_data_call_with_range(BpfHelper::MsgPullData, start, end, 0);
        let probe_ctx = ProbeContext::new(EbpfProgramType::SkMsg, "/sys/fs/bpf/demo_sockmap");
        let mut ti = TypeInference::new(Some(probe_ctx));
        let errs = ti
            .infer(&func)
...
fn make_guarded_sock_ops_hdr_opt_call(helper: BpfHelper, len: i64) -> MirFunction {
...
#[test]
fn test_infer_get_stack_helper_allows_zero_size_null_buffer_in_kprobe() {
    let mut func = make_test_function();
    let ctx = func.alloc_vreg();
    let dst = func.alloc_vreg();
    let block = func.block_mut(BlockId(0));
    ...
    let types = ti
        .infer(&func)
        .expect("expected bpf_get_stack zero-size null buffer to infer");
    assert_eq!(types.get(&dst), Some(&MirType::I64));
}
...
fn make_sk_ancestor_cgroup_id_call(level: i64) -> MirFunction {
    ...
    func.block_mut(call).instructions.push(MirInst::CallHelper {
        ...
        args: vec![MirValue::VReg(sock), MirValue::Const(level)],
    });
    func.block_mut(call).instructions.push(MirInst::CallHelper {
        dst: cleanup_ret,
        helper: BpfHelper::SkRelease as u32,
...
#[test]
fn test_type_error_sk_ancestor_cgroup_id_helper_rejects_invalid_level() {
    for level in [-1, i64::from(i32::MAX) + 1] {
        let func = make_sk_ancestor_cgroup_id_call(level);
        let probe_ctx = ProbeContext::new(EbpfProgramType::CgroupSkb, "/sys/fs/cgroup:ingress");
        let mut ti = TypeInference::new(Some(probe_ctx));
        let errs = ti
            .infer(&func)
...
#[test]
fn test_type_error_get_current_ancestor_cgroup_id_helper_rejects_invalid_level() {
    for level in [-1, i64::from(i32::MAX) + 1] {
        let mut func = make_test_function();
        let dst = func.alloc_vreg();
        let block = func.block_mut(BlockId(0));
...
#[test]
fn test_type_error_skb_ancestor_cgroup_id_helper_rejects_invalid_level() {
    for level in [-1, i64::from(i32::MAX) + 1] {
        let mut func = make_test_function();
        let ctx = func.alloc_vreg();
        let dst = func.alloc_vreg();
...
#[test]
fn test_type_error_sk_assign_helper_requires_zero_flags_in_tc_action() {
    let mut func = make_test_function();
    let ctx = func.alloc_vreg();
    let dst = func.alloc_vreg();
    let block = func.block_mut(BlockId(0));
    ...
    let errs = ti
        .infer(&func)
        .expect_err("expected bpf_sk_assign flags to require zero on tc_action");
    assert!(errs.iter().any(|e| {
        e.message
...
fn make_tcp_syncookie_type_call(helper: BpfHelper, iph_len: i64, th_len: i64) -> MirFunction {
    let mut func = make_test_function();
    let pid = func.alloc_vreg();
    let sk = func.alloc_vreg();
    let dst = func.alloc_vreg();
...
#[test]
fn test_type_error_tcp_syncookie_helpers_reject_short_header_lengths() {
    for (helper, iph_len, th_len, expected) in [
        ...
        let func = make_tcp_syncookie_type_call(helper, iph_len, th_len);
        let mut ti = TypeInference::new(None);
        let errs = ti
            .infer(&func)
