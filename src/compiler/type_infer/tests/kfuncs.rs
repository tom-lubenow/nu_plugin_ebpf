...
#[test]
fn test_type_error_kfunc_scx_dsq_move_requires_stack_iterator_pointer_arg0() {
    let mut func = make_test_function();
    let pid = func.alloc_vreg();
    let task = func.alloc_vreg();
    let dsq_id = func.alloc_vreg();
    ...
    let errs = ti
        .infer(&func)
        .expect_err("expected scx dsq_move stack-pointer type error");
    assert!(
        errs.iter()
            .any(|e| e.message.contains("arg0 expects stack pointer")),
        "unexpected errors: {:?}",
...
#[test]
fn test_type_error_kfunc_scx_dsq_move_vtime_requires_stack_iterator_pointer_arg0() {
    let mut func = make_test_function();
    let pid = func.alloc_vreg();
    let task = func.alloc_vreg();
    let dsq_id = func.alloc_vreg();
    ...
    let errs = ti
        .infer(&func)
        .expect_err("expected scx dsq_move_vtime stack-pointer type error");
    assert!(
        errs.iter()
            .any(|e| e.message.contains("arg0 expects stack pointer")),
        "unexpected errors: {:?}",
...
#[test]
fn test_type_error_kfunc_path_d_path_requires_stack_slot_base_buffer() {
    let mut func = make_test_function();
    let pid = func.alloc_vreg();
    let path = func.alloc_vreg();
    let buf = func.alloc_vreg();
    ...
    block.instructions.push(MirInst::BinOp {
        dst: buf,
        op: BinOpKind::Add,
        lhs: MirValue::StackSlot(buf_slot),
    ...
    let errs = ti
        .infer(&func)
        .expect_err("expected path_d_path stack-slot-base type error");
    assert!(
        errs.iter()
...
#[test]
fn test_type_error_kfunc_copy_from_user_str_src_requires_user_pointer() {
    let mut func = make_test_function();
    let dst = func.alloc_vreg();
    let size = func.alloc_vreg();
    ...
    let errs = ti
        .infer(&func)
        .expect_err("expected copy_from_user_str user-pointer source type error");
    assert!(
        errs.iter().any(|e| e
            .message
...
#[test]
fn test_type_error_kfunc_copy_from_user_str_rejects_zero_size() {
    let mut func = make_test_function();
    let dst = func.alloc_vreg();
    let size = func.alloc_vreg();
    let src = func.alloc_vreg();
    ...
    let errs = ti
        .infer(&func)
        .expect_err("expected copy_from_user_str zero-size type error");
    assert!(
        errs.iter().any(|e| e
            .message
...
#[test]
fn test_type_error_kfunc_copy_from_user_str_requires_stack_slot_base_dst() {
    let mut func = make_test_function();
    let dst = func.alloc_vreg();
    let size = func.alloc_vreg();
    let src = func.alloc_vreg();
    ...
    let errs = ti
        .infer(&func)
        .expect_err("expected copy_from_user_str stack-slot-base type error");
    assert!(
        errs.iter()
...
#[test]
fn test_type_error_kfunc_copy_from_user_task_str_requires_kernel_task_arg() {
    let mut func = make_test_function();
    let dst = func.alloc_vreg();
    let size = func.alloc_vreg();
    let src = func.alloc_vreg();
    ...
    let errs = ti
        .infer(&func)
        .expect_err("expected copy_from_user_task_str task-arg type error");
    assert!(
        errs.iter()
...
#[test]
fn test_type_error_kfunc_copy_from_user_task_str_rejects_zero_size() {
    let mut func = make_test_function();
    let dst = func.alloc_vreg();
    let size = func.alloc_vreg();
    let src = func.alloc_vreg();
    ...
    let errs = ti
        .infer(&func)
        .expect_err("expected copy_from_user_task_str zero-size type error");
    assert!(
        errs.iter().any(|e| e
            .message
...
#[test]
fn test_type_error_dynptr_from_skb_rejects_indirect_skb_raw_context() {
    for probe_ctx in [
...
#[test]
fn test_type_error_kfunc_scx_dump_bstr_requires_stack_slot_base_data() {
    let mut func = make_test_function();
    let fmt = func.alloc_vreg();
    let data = func.alloc_vreg();
    let size = func.alloc_vreg();
    ...
    let errs = ti
        .infer(&func)
        .expect_err("expected scx_bpf_dump_bstr stack-slot-base type error");
    assert!(
        errs.iter()
            .any(|e| e.message.contains("arg1 expects stack slot base pointer")),
        "unexpected errors: {:?}",
...
#[test]
fn test_type_error_kfunc_object_new_rejects_dynamic_meta() {
    for kfunc in ["bpf_obj_new_impl", "bpf_percpu_obj_new_impl"] {
        let mut func = make_test_function();
        let type_id = func.alloc_vreg();
        let meta = func.alloc_vreg();
        ...
        let hints = HashMap::from([(meta, MirType::I64)]);
        let mut ti = TypeInference::new_with_env(None, None, None, Some(&hints), None);
        let errs = ti
            .infer(&func)
            .expect_err("expected object-new dynamic meta type error");
        assert!(
            errs.iter().any(|e| e
                .message
                .contains(&format!("kfunc '{kfunc}' arg1 must be known zero"))),
...
#[test]
fn test_type_error_kfunc_object_drop_rejects_dynamic_meta() {
    for (new_kfunc, drop_kfunc) in [
        ("bpf_obj_new_impl", "bpf_obj_drop_impl"),
        ("bpf_percpu_obj_new_impl", "bpf_percpu_obj_drop_impl"),
    ] {
        ...
        let hints = HashMap::from([(drop_meta, MirType::I64)]);
        let mut ti = TypeInference::new_with_env(None, None, None, Some(&hints), None);
        let errs = ti
            .infer(&func)
            .expect_err("expected object-drop dynamic meta type error");
        assert!(
            errs.iter().any(|e| e
                .message
                .contains(&format!("kfunc '{drop_kfunc}' arg1 must be known zero"))),
...
#[test]
fn test_type_error_kfunc_bpf_wq_init_rejects_dynamic_flags() {
    let mut func = make_test_function();
    let wq = func.alloc_vreg();
    let map_fd = func.alloc_vreg();
    let flags = func.alloc_vreg();
    ...
    let errs = ti
        .infer(&func)
        .expect_err("expected bpf_wq_init dynamic flags error");
    assert!(
        errs.iter().any(|e| e
            .message
            .contains("kfunc 'bpf_wq_init' arg2 must be known zero")),
...
#[test]
fn test_type_error_kfunc_bpf_wq_set_callback_rejects_nonzero_flags() {
    let mut func = make_test_function();
    let wq = func.alloc_vreg();
    let callback = func.alloc_vreg();
    let flags = func.alloc_vreg();
...
#[test]
fn test_type_error_kfunc_bpf_wq_set_callback_rejects_dynamic_flags() {
    let mut func = make_test_function();
    let wq = func.alloc_vreg();
    let callback = func.alloc_vreg();
    let flags = func.alloc_vreg();
...
#[test]
fn test_type_error_kfunc_bpf_wq_set_callback_rejects_nonzero_aux() {
    let mut func = make_test_function();
    let wq = func.alloc_vreg();
    let callback = func.alloc_vreg();
    let flags = func.alloc_vreg();
    ...
    let errs = ti
        .infer(&func)
        .expect_err("expected bpf_wq_set_callback_impl nonzero aux error");
    assert!(
        errs.iter().any(|e| {
            e.message
...
#[test]
fn test_type_error_kfunc_bpf_wq_set_callback_rejects_dynamic_aux() {
    let mut func = make_test_function();
    let wq = func.alloc_vreg();
    let callback = func.alloc_vreg();
    let flags = func.alloc_vreg();
    ...
    let errs = ti
        .infer(&func)
        .expect_err("expected bpf_wq_set_callback_impl dynamic aux error");
    assert!(
        errs.iter().any(|e| {
            e.message
                .contains("kfunc 'bpf_wq_set_callback_impl' arg3 must be known zero")
...
#[test]
fn test_type_error_kfunc_bpf_wq_start_rejects_dynamic_flags() {
    let mut func = make_test_function();
    let wq = func.alloc_vreg();
    let flags = func.alloc_vreg();
    let dst = func.alloc_vreg();
    ...
    let errs = ti
        .infer(&func)
        .expect_err("expected bpf_wq_start dynamic flags error");
    assert!(
        errs.iter().any(|e| e
            .message
            .contains("kfunc 'bpf_wq_start' arg1 must be known zero")),
...
#[test]
fn test_type_error_kfunc_refcount_acquire_rejects_graph_object_without_refcount() {
    let node_size = BpfGraphRootKind::ListHead.node_size();
    let object_ty = MirType::Struct {
        name: Some("node_data".to_string()),
        kernel_btf_type_id: None,
    ...
    let errs = ti
        .infer(&func)
        .expect_err("expected bpf_refcount_acquire_impl to require bpf_refcount payload");
    assert!(
...
#[test]
fn test_type_error_kfunc_rbtree_add_rejects_nonzero_meta() {
    let object_ty = MirType::Struct {
        name: Some("rb_item".to_string()),
        kernel_btf_type_id: None,
        fields: vec![StructField {
...
#[test]
fn test_type_error_kfunc_crypto_ctx_create_params_requires_stack_or_map_space() {
    let mut func = make_test_function();
    let pid = func.alloc_vreg();
    ...
    let errs = ti
        .infer(&func)
        .expect_err("expected crypto_ctx_create params pointer-space type error");
...
fn test_type_error_kfunc_crypto_ctx_create_err_requires_stack_slot_base_pointer() {
    ...
    block.instructions.push(MirInst::BinOp {
        dst: err_ptr,
        op: BinOpKind::Add,
        lhs: MirValue::StackSlot(err_slot),
        rhs: MirValue::Const(1),
    });
    block.instructions.push(MirInst::CallKfunc {
        dst,
    ...
    let errs = ti
        .infer(&func)
        .expect_err("expected crypto_ctx_create stack-slot-base type error");
    assert!(
        errs.iter()
            .any(|e| e.message.contains("arg2 expects stack slot base pointer")),
        "unexpected errors: {:?}",
...
#[test]
fn test_type_error_kfunc_crypto_encrypt_src_requires_stack_or_map_space() {
    let mut func = make_test_function();
    let params = func.alloc_vreg();
    let params_sz = func.alloc_vreg();
    let err_ptr = func.alloc_vreg();
    ...
    let errs = ti
        .infer(&func)
        .expect_err("expected crypto_encrypt src pointer-space type error");
    assert!(errs.iter().any(|e| {
        e.message
...
#[test]
fn test_type_error_kfunc_crypto_decrypt_src_requires_stack_or_map_space() {
    let mut func = make_test_function();
    let params = func.alloc_vreg();
    let params_sz = func.alloc_vreg();
    let err_ptr = func.alloc_vreg();
    ...
    let errs = ti
        .infer(&func)
        .expect_err("expected crypto_decrypt src pointer-space type error");
    assert!(errs.iter().any(|e| {
        e.message
...
#[test]
fn test_type_error_kfunc_crypto_encrypt_dst_requires_stack_slot_base_pointer() {
    let mut func = make_test_function();
    let params = func.alloc_vreg();
    let params_sz = func.alloc_vreg();
    let err_ptr = func.alloc_vreg();
    ...
    let errs = ti
        .infer(&func)
        .expect_err("expected crypto_encrypt stack-slot-base type error");
    assert!(
        errs.iter()
            .any(|e| e.message.contains("arg2 expects stack slot base pointer")),
        "unexpected errors: {:?}",
...
#[test]
fn test_type_error_kfunc_crypto_decrypt_siv_rejects_non_zero_scalar() {
    let mut func = make_test_function();
    let params = func.alloc_vreg();
    let params_sz = func.alloc_vreg();
    let err_ptr = func.alloc_vreg();
    ...
    let errs = ti
        .infer(&func)
        .expect_err("expected non-zero scalar bpf_crypto_decrypt siv type error");
    assert!(
        errs.iter().any(|e| e
            .message
...
fn test_type_error_kfunc_dynptr_slice_buffer_rejects_non_zero_scalar() {
    ...
    let errs = ti
        .infer(&func)
        .expect_err("expected non-zero scalar bpf_dynptr_slice buffer type error");
    assert!(
        errs.iter().any(|e| e
            .message
