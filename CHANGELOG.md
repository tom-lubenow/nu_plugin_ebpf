...
- Improved unsupported queue/stack `map-get`, `map-contains`, and `map-put`
  diagnostics so they point to `map-peek` / `map-pop` and `map-push`
  respectively.
- Improved unsupported bloom-filter `map-get`, `map-put`, `map-peek`, and
  `map-pop` diagnostics so they point to `map-push` insertion and
  `map-contains` membership tests.
- Improved shared MIR bloom-filter map-update diagnostics so lower compiler
  validation paths point to `map-push` insertion and `map-contains` membership
  tests.
...
- Improved source-level `map-define` unknown-kind diagnostics so they list the
  supported value-carrying map families.
...
- Improved annotated mutable fixed-array global diagnostics for empty
  initializers so they explain the missing element layout and point to explicit
  `global-define --type 'array{...:N}'` declarations.
