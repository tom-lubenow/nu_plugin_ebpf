...
echo "[1/28] stream attach (kprobe:ksys_read)"
run_nu 'ebpf attach -s "kprobe:ksys_read" {|ctx| $ctx.pid | emit } | first 1'

echo "[2/28] attach -> counters -> detach"
...
echo "[3/28] tracepoint + read-str with null guard"
...
echo "[4/28] fentry trampoline arg"
...
echo "[5/28] fentry pointer-backed trampoline field"
...
echo "[6/28] fentry intermediate trampoline pointer hop"
...
echo "[7/28] fentry post-binding pointer field projection"
...
echo "[8/28] fentry deeper post-binding pointer field projection"
...
echo "[9/28] fentry multi-level trampoline pointer hop"
...
echo "[10/28] fentry direct pointer index"
...
echo "[11/28] fentry bound root trampoline arg"
...
echo "[12/28] fentry bound pointer index"
...
echo "[13/28] fentry bound numeric get"
...
echo "[14/28] fentry trampoline array element"
...
echo "[15/28] fentry trampoline array leaf"
...
echo "[16/28] fentry trampoline struct leaf emit decodes record"
...
echo "[17/28] fentry trampoline struct leaf count decodes record key"
...
echo "[18/28] fexit trampoline retval"
...
echo "[19/28] bounded loop-driven numeric get"
...
echo "[20/28] bounded arithmetic-derived numeric get"
...
echo "[21/28] typed runtime-field numeric get"
...
echo "[22/28] runtime get on stack-backed array leaf"
...
echo "[23/28] runtime get on stack-backed aggregate bitfield"
...
echo "[24/28] runtime get on stack-backed aggregate bitfield struct count decodes record key"
...
echo "[25/28] runtime get on stack-backed aggregate bitfield struct emit decodes record"
...
echo "[26/28] branch-refined bound numeric get"
...
echo "[27/28] branch-refined direct numeric get"
...
echo "[28/28] verify no leaked probes"
