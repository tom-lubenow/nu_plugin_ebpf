...
echo "[1/16] stream attach (kprobe:ksys_read)"
run_nu 'ebpf attach -s "kprobe:ksys_read" {|ctx| $ctx.pid | emit } | first 1'

echo "[2/16] attach -> counters -> detach"
...
echo "[3/16] tracepoint + read-str with null guard"
...
echo "[4/16] fentry trampoline arg"
...
echo "[5/16] fentry pointer-backed trampoline field"
...
echo "[6/16] fentry intermediate trampoline pointer hop"
...
echo "[7/16] fentry post-binding pointer field projection"
...
echo "[8/16] fentry deeper post-binding pointer field projection"
...
echo "[9/16] fentry multi-level trampoline pointer hop"
...
echo "[10/16] fentry bound root trampoline arg"
...
echo "[11/16] fentry trampoline array element"
...
echo "[12/16] fentry trampoline array leaf"
...
echo "[13/16] fentry trampoline struct leaf emit decodes record"
...
echo "[14/16] fentry trampoline struct leaf count decodes record key"
...
echo "[15/16] fexit trampoline retval"
...
echo "[16/16] verify no leaked probes"
