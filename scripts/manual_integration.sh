...
echo "[1/35] stream attach (kprobe:ksys_read)"
run_nu 'ebpf attach -s "kprobe:ksys_read" {|ctx| $ctx.pid | emit } | first 1'

echo "[2/35] attach -> counters -> detach"
...
echo "[3/35] tracepoint + read-str with null guard"
...
echo "[4/35] fentry trampoline arg"
...
echo "[5/35] fentry pointer-backed trampoline field"
...
echo "[6/35] fentry intermediate trampoline pointer hop"
...
echo "[7/35] fentry post-binding pointer field projection"
...
echo "[8/35] fentry deeper post-binding pointer field projection"
...
echo "[9/35] fentry multi-level trampoline pointer hop"
...
echo "[10/35] fentry direct pointer index"
...
echo "[11/35] fentry bound root trampoline arg"
...
echo "[12/35] fentry bound pointer index"
...
echo "[13/35] fentry bound numeric get"
...
echo "[14/35] fentry trampoline array element"
...
echo "[15/35] fentry trampoline array leaf"
...
echo "[16/35] fentry trampoline struct leaf emit decodes record"
...
echo "[17/35] fentry trampoline struct leaf count decodes record key"
...
echo "[18/35] fexit trampoline retval"
...
echo "[19/35] bounded loop-driven numeric get"
...
echo "[20/35] bounded arithmetic-derived numeric get"
...
echo "[21/35] typed runtime-field numeric get"
...
echo "[22/35] runtime get on stack-backed array leaf"
...
echo "[23/35] runtime get on stack-backed aggregate bitfield"
...
echo "[24/35] runtime get on stack-backed aggregate bitfield struct count decodes record key"
...
echo "[25/35] runtime get on stack-backed aggregate bitfield struct emit decodes record"
...
echo "[26/35] branch-refined bound numeric get"
...
echo "[27/35] branch-refined direct numeric get"
...
echo "[28/35] typed generic map put/get projection"
...
echo "[29/35] typed generic map whole-value count decodes record key"
...
echo "[30/35] typed generic map whole-value emit decodes record"
map_emit_out="$(mktemp)"
...
echo "[31/35] typed generic map value wrapped into emitted record"
...
echo "[32/35] typed generic map value through user function emits typed record"
...
echo "[33/35] typed generic map value copied into second map"
...
echo "[34/35] typed generic map schema persists across pinned programs"
...
echo "[35/35] verify no leaked probes"
