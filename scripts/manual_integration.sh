...
echo "[1/18] stream attach (kprobe:ksys_read)"
run_nu 'ebpf attach -s "kprobe:ksys_read" {|ctx| $ctx.pid | emit } | first 1'

echo "[2/18] attach -> counters -> detach"
...
echo "[3/18] tracepoint + read-str with null guard"
...
echo "[4/18] fentry trampoline arg"
...
echo "[5/18] fentry pointer-backed trampoline field"
...
echo "[6/18] fentry intermediate trampoline pointer hop"
...
echo "[7/18] fentry post-binding pointer field projection"
...
echo "[8/18] fentry deeper post-binding pointer field projection"
...
echo "[9/18] fentry multi-level trampoline pointer hop"
...
echo "[10/18] fentry direct pointer index"
...
echo "[11/18] fentry bound root trampoline arg"
...
echo "[12/18] fentry bound pointer index"
...
echo "[13/18] fentry trampoline array element"
...
echo "[14/18] fentry trampoline array leaf"
...
echo "[15/18] fentry trampoline struct leaf emit decodes record"
...
echo "[16/18] fentry trampoline struct leaf count decodes record key"
...
echo "[17/18] fexit trampoline retval"
...
echo "[18/18] verify no leaked probes"
