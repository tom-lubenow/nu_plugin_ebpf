...
echo "[1/17] stream attach (kprobe:ksys_read)"
run_nu 'ebpf attach -s "kprobe:ksys_read" {|ctx| $ctx.pid | emit } | first 1'

echo "[2/17] attach -> counters -> detach"
...
echo "[3/17] tracepoint + read-str with null guard"
...
echo "[4/17] fentry trampoline arg"
...
echo "[5/17] fentry pointer-backed trampoline field"
...
echo "[6/17] fentry intermediate trampoline pointer hop"
...
echo "[7/17] fentry post-binding pointer field projection"
...
echo "[8/17] fentry deeper post-binding pointer field projection"
...
echo "[9/17] fentry multi-level trampoline pointer hop"
...
echo "[10/17] fentry bound root trampoline arg"
...
echo "[11/17] fentry bound pointer index"
...
echo "[12/17] fentry trampoline array element"
...
echo "[13/17] fentry trampoline array leaf"
...
echo "[14/17] fentry trampoline struct leaf emit decodes record"
...
echo "[15/17] fentry trampoline struct leaf count decodes record key"
...
echo "[16/17] fexit trampoline retval"
...
echo "[17/17] verify no leaked probes"
