...
echo "[1/15] stream attach (kprobe:ksys_read)"
run_nu 'ebpf attach -s "kprobe:ksys_read" {|ctx| $ctx.pid | emit } | first 1'

echo "[2/15] attach -> counters -> detach"
...
echo "[3/15] tracepoint + read-str with null guard"
...
echo "[4/15] fentry trampoline arg"
...
echo "[5/15] fentry pointer-backed trampoline field"
...
echo "[6/15] fentry intermediate trampoline pointer hop"
...
echo "[7/15] fentry post-binding pointer field projection"
...
echo "[8/15] fentry deeper post-binding pointer field projection"
...
echo "[9/15] fentry multi-level trampoline pointer hop"
...
echo "[10/15] fentry trampoline array element"
...
echo "[11/15] fentry trampoline array leaf"
...
echo "[12/15] fentry trampoline struct leaf emit decodes record"
...
echo "[13/15] fentry trampoline struct leaf count decodes record key"
...
echo "[14/15] fexit trampoline retval"
...
echo "[15/15] verify no leaked probes"
