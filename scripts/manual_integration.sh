...
echo "[1/36] stream attach (kprobe:ksys_read)"
run_nu 'ebpf attach -s "kprobe:ksys_read" {|ctx| $ctx.pid | emit } | first 1'

echo "[2/36] attach -> counters -> detach"
...
echo "[3/36] tracepoint + read-str with null guard"
...
echo "[4/36] fentry trampoline arg"
...
echo "[5/36] fentry pointer-backed trampoline field"
...
echo "[6/36] fentry intermediate trampoline pointer hop"
...
echo "[7/36] fentry post-binding pointer field projection"
...
echo "[8/36] fentry deeper post-binding pointer field projection"
...
echo "[9/36] fentry multi-level trampoline pointer hop"
...
echo "[10/36] fentry direct pointer index"
...
echo "[11/36] fentry bound root trampoline arg"
...
echo "[12/36] fentry bound pointer index"
...
echo "[13/36] fentry bound numeric get"
...
echo "[14/36] fentry trampoline array element"
...
echo "[15/36] fentry trampoline array leaf"
...
echo "[16/36] fentry trampoline struct leaf emit decodes record"
...
echo "[17/36] fentry trampoline struct leaf count decodes record key"
...
echo "[18/36] fexit trampoline retval"
...
echo "[19/36] bounded loop-driven numeric get"
...
echo "[20/36] bounded arithmetic-derived numeric get"
...
echo "[21/36] typed runtime-field numeric get"
...
echo "[22/36] runtime get on stack-backed array leaf"
...
echo "[23/36] runtime get on stack-backed aggregate bitfield"
...
echo "[24/36] runtime get on stack-backed aggregate bitfield struct count decodes record key"
...
echo "[25/36] runtime get on stack-backed aggregate bitfield struct emit decodes record"
...
echo "[26/36] branch-refined bound numeric get"
...
echo "[27/36] branch-refined direct numeric get"
...
echo "[28/36] typed generic map put/get projection"
...
echo "[29/36] typed generic map whole-value count decodes record key"
...
echo "[30/36] typed generic map whole-value emit decodes record"
map_emit_out="$(mktemp)"
...
echo "[31/36] typed generic map value wrapped into emitted record"
map_record_emit_out="$(mktemp)"
trap 'rm -f "$map_record_emit_out"' EXIT
run_nu "ebpf attach -s 'fentry:security_file_open' {|ctx| \$ctx.arg0.f_path | map-put cached_path \$ctx.pid --kind hash; let entry = (\$ctx.pid | map-get cached_path --kind hash); if \$entry != 0 { { path: \$entry } | emit } } | first 1 | get path | columns | sort | to nuon" >"$map_record_emit_out" &
map_record_emit_pid=$!
sleep 1
cat "$REPO_ROOT/Cargo.toml" >/dev/null
wait "$map_record_emit_pid"
if ! grep -qx '\[dentry, mnt\]' "$map_record_emit_out"; then
    echo "expected nested map record emit to produce path fields dentry/mnt, got:" >&2
    cat "$map_record_emit_out" >&2
    exit 1
fi
rm -f "$map_record_emit_out"
trap - EXIT

echo "[32/36] typed generic map value through user function emits typed record"
map_user_emit_out="$(mktemp)"
trap 'rm -f "$map_user_emit_out"' EXIT
run_nu "def project-entry [entry] { \$entry }; ebpf attach -s 'fentry:security_file_open' {|ctx| \$ctx.arg0.f_path | map-put cached_path \$ctx.pid --kind hash; let entry = (\$ctx.pid | map-get cached_path --kind hash); if \$entry != 0 { (project-entry \$entry) | emit } } | first 1 | columns | sort | to nuon" >"$map_user_emit_out" &
map_user_emit_pid=$!
sleep 1
cat "$REPO_ROOT/Cargo.toml" >/dev/null
wait "$map_user_emit_pid"
if ! grep -qx '\[cpu, dentry, mnt\]' "$map_user_emit_out"; then
    echo "expected user-function map emit to produce record fields cpu/dentry/mnt, got:" >&2
    cat "$map_user_emit_out" >&2
    exit 1
fi
rm -f "$map_user_emit_out"
trap - EXIT

echo "[33/36] typed user function projects typed trampoline arg"
run_nu "let path = '$REPO_ROOT/Cargo.toml'; def project-inode-flags [file] { \$file.f_inode.i_flags }; let id = (ebpf attach 'fentry:security_file_open' {|ctx| (project-inode-flags \$ctx.arg0) | count }); let _ = (open --raw \$path | str length); sleep 1sec; let rows = ((ebpf counters \$id) | length); ebpf detach \$id; if \$rows < 1 { error make { msg: 'expected at least one typed user-function trampoline projection row' } }; { id: \$id, rows: \$rows }"

echo "[34/36] typed generic map value copied into second map"
run_nu "let path = '$REPO_ROOT/Cargo.toml'; let id = (ebpf attach 'fentry:security_file_open' {|ctx| \$ctx.arg0.f_path | map-put cached_path \$ctx.pid --kind hash; let entry = (\$ctx.pid | map-get cached_path --kind hash); if \$entry != 0 { \$entry | map-put copied_path \$ctx.pid --kind hash; let copied = (\$ctx.pid | map-get copied_path --kind hash); if \$copied != 0 { \$copied.dentry.d_flags | count } }}); let _ = (open --raw \$path | str length); sleep 1sec; let rows = ((ebpf counters \$id) | length); ebpf detach \$id; if \$rows < 1 { error make { msg: 'expected at least one map-to-map copy row' } }; { id: \$id, rows: \$rows }"

echo "[35/36] typed generic map schema persists across pinned programs"
...
echo "[36/36] verify no leaked probes"
