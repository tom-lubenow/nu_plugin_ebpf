...
# Dry-run a cgroup LSM hook section using named BTF-backed hook arguments
ebpf attach --dry-run 'lsm_cgroup:socket_bind' {|ctx| ($ctx.arg.address.sa_family + $ctx.arg.addrlen) | count; 1 }
...
ebpf attach --dry-run 'xdp:lo:drv:frags' {|ctx| $ctx.xdp_buff_len | count; 'pass' }
...
# Count cgroup UNIX socket-address connects
let id = ebpf attach 'cgroup_sock_addr:/sys/fs/cgroup:connect_unix' {|ctx| $ctx.family | count; 'allow' }
