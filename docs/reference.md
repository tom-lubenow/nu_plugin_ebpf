...
| `bytes add` | Compile-time known binary or list<binary> input/data with optional non-negative `--index` and `--end`; scalar input returns a fixed binary value, while materialized list output requires non-empty equal-length binary values and empty or unequal list outputs are supported when folded by `bytes collect`, scalar `first` / `last`, constant-index `get`, `length`, or empty predicates; typed fixed-size binary globals/fields and scalar typed fixed-array binary item projections declared with `bytes:N` or `binary:N` support compile-time binary data insertion into stack-backed fixed binary values |
