...
| `str length` | Tracked strings and literal strings, returning the byte length tracked by the string buffer; typed fixed `array{string:N:M}` input returns a stack numeric list of byte lengths; compile-time known string-list input returns a numeric list; `--chars`, `--grapheme-clusters`, and `--unicode-width` are supported for compile-time known scalar string or string-list input |
