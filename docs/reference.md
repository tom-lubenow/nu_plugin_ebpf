...
| `cgroup` / `current_cgroup` | Current task default `cgroup *`; lowers through the BTF-backed `$ctx.task.cgroups.dfl_cgrp` projection when kernel BTF is available, so it can be used as the owner pointer for `--kind cgrp-storage` without spelling task internals | kprobe, kretprobe, kprobe.multi, kretprobe.multi, ksyscall, kretsyscall, uprobe, uretprobe, uprobe.multi, uretprobe.multi, perf_event, raw_tracepoint, raw_tracepoint.w, tracepoint, fentry, fexit, fmod_ret, tp_btf, lsm, lsm_cgroup |
