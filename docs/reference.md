...
| `default` | Compiler-known null/empty scalar replacement, runtime non-null scalar passthrough for `--empty`, metadata-proven non-empty typed fixed-binary and fixed-array passthrough for stack/context or compiler-managed global storage, metadata-backed record missing/null field replacement, and tracked runtime string branching for `--empty`; closure default values and runtime nullable-pointer defaults are rejected |
