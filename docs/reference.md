...
| `bits not` / `bits not --number-bytes 1|2|4` / `bits not --signed` | Integer scalar input and integer stack-backed or compile-time known lists; default and supported `--number-bytes` modes return width-masked unsigned results, while `--signed` returns signed two's-complement bit negation; binary input and `--number-bytes 8` are not supported |
