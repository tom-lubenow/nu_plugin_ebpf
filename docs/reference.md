...
| `bytes index-of` | Compile-time known binary or list<binary> input and non-empty binary pattern, returning the first byte index or `-1`, or a numeric list of first indexes for list input; `--end` returns the last index, and scalar `--all` returns a stack-backed numeric list of up to 60 non-overlapping match offsets, in reverse order when combined with `--end`; typed fixed-size binary globals/fields and scalar typed fixed-array binary item projections declared with `bytes:N` or `binary:N` support first/last and `--all` index lookup with compile-time binary patterns through bounded runtime byte comparisons, when the candidate-offset probe count fits the compiler's fixed-comparison cap |
