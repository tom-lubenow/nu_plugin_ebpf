...
| `math sum` / `math product` / `math min` / `math max` | Stack-backed numeric lists with known non-empty length, plus compile-time known integer/float lists for `math min` and `math max` when the selected result is an integer; empty-list input is rejected to match Nushell semantics, and float min/max results are not supported |
| `math median` | Compile-time known odd-length integer lists, compile-time known odd-length integer/float lists whose sorted median item is an integer, and stack-backed numeric lists with known odd length up to 16 items, returning the integer median; empty lists, even-length lists, unknown-length runtime lists, and float median results are not supported |
