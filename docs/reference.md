...
| `is-empty` / `is-not-empty` | Stack-backed lists, tracked strings, literal binary, typed fixed-size binary globals/fields, literal null, compile-time known bool/int/float/filesize/duration scalars, literal list constants and fixed-layout list builders, and metadata-backed fixed records |
