...
sizes remain kernel-enforced. `tc_action` and TC also model
...
Perf-event-array counter reads are also modeled through `helper-call "bpf_perf_event_read" MAP FLAGS` and `helper-call "bpf_perf_event_read_value" MAP FLAGS BUF 24`; both require perf-event-array maps, and the value form requires a 24-byte `struct bpf_perf_event_value` buffer.
