...
| `bytes starts-with` / `bytes ends-with` | Compile-time known binary or list<binary> input and binary pattern, returning a constant boolean or boolean list; typed fixed-size binary globals/fields and scalar typed fixed-array binary item projections declared with `bytes:N` or `binary:N` support compile-time binary patterns through bounded runtime byte comparisons |
