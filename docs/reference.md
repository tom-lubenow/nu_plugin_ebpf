...
Raw dynptr helpers are partially modeled for explicit `helper-call` use. `bpf_dynptr_from_mem` initializes a 16-byte stack-slot-base dynptr object from a map-value pointer, while `bpf_dynptr_read`, `bpf_dynptr_write`, and `bpf_dynptr_data` require an initialized dynptr object. The compiler enforces stack-slot-base dynptr arguments, map/stack buffer bounds, positive read/write lengths, reserved flags `0`, and constant `bpf_dynptr_data` length. Ringbuf dynptr reservation helpers are not modeled yet because they need reservation-release lifetime tracking distinct from ordinary dynptr initialization.
