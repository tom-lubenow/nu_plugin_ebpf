...
An empty binary literal `0x[]` does not establish a fixed byte-buffer layout on its own. Use a non-empty binary initializer when you want layout inference, or provide an explicit top-level `bytes:N` / `binary:N` type spec when you want a zero-filled byte buffer. For nested typed record byte fields, omit the field to zero-fill it from the declared `record{...}` layout.
