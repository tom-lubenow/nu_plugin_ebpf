...
Raw dynptr helpers are partially modeled for explicit `helper-call` use. `bpf_dynptr_from_mem` initializes a 16-byte stack-slot-base dynptr object from a map-value pointer, while `bpf_dynptr_read`, `bpf_dynptr_write`, and `bpf_dynptr_data` require an initialized dynptr object. Ringbuf dynptr helpers `bpf_ringbuf_reserve_dynptr`, `bpf_ringbuf_submit_dynptr`, and `bpf_ringbuf_discard_dynptr` add a separate reservation obligation: every reserved dynptr stack slot must be submitted or discarded on all paths, release helpers only accept live ringbuf dynptr reservations, and release consumes the dynptr. The compiler enforces stack-slot-base dynptr arguments, map/stack buffer bounds, positive read/write/reservation lengths, reserved/wakeup flag ranges, double-release rejection, and constant `bpf_dynptr_data` length. Dynptrs are not valid map-value schema fields; use stack-slot dynptr helper arguments instead.
