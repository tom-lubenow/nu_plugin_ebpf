...
| `str ends-with` | Tracked strings with a literal suffix, lowered as bounded fixed-offset stack-buffer comparisons over the tracked runtime length; `--ignore-case` is supported for compile-time known input strings, and compile-time known string-list input returns a bool list |
