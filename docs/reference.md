...
- Prefer ordinary Nushell syntax plus the small first-class eBPF command set (`emit`, `count`, `histogram`, `start-timer`, `stop-timer`, `read-str`, `read-kernel-str`, `redirect-map`, and `redirect-socket`) whenever the operation has an honest language form.
