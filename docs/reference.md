...
|-----------|-----------------------|
| `where` | Stack-backed numeric lists and typed numeric fixed arrays with a closure predicate; scalar pipeline filtering also lowers to an early return when the predicate is false |
| `each` | Stack-backed numeric lists and typed numeric fixed arrays with a closure transform, preserving runtime/fixed list length; scalar pipeline transforms are also supported |
| `all` / `any` | Stack-backed numeric lists and typed numeric or bool fixed arrays with a closure predicate, verifier-bounded constant-index reads, and Nushell empty-list identities |
...
| `uniq` | Stack-backed numeric lists and typed numeric fixed arrays, preserving first occurrences through verifier-bounded duplicate checks; compile-time known fixed lists are deduplicated as constants, including metadata-only float-list outputs when folded by metadata consumers |
...
| `find` | Stack-backed numeric lists and typed numeric fixed arrays with one numeric search argument, returning a bounded equality-filtered stack list; compile-time known fixed lists with one compile-time constant search argument, including metadata-only float-list outputs when folded by metadata consumers |
