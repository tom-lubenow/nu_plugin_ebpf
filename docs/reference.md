...
| `bytes split` | Compile-time known binary input and non-empty binary/string separator; materialized lists require non-empty equal-length binary parts, while empty or unequal parts are supported when folded by `bytes collect`, constant-index `get`, `length`, or empty predicates |
