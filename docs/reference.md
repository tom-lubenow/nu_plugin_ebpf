...
| `str trim` | Compile-time known string or string-list input in default, `--left`, `--right`, and single-character `--char` trim modes; typed fixed `array{string:N:M}` input supports runtime single-sided `--left --char` or `--right --char` trimming when `--char` is one byte, returning a typed fixed string array |
