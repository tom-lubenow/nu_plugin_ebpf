...
| `str length` | Tracked strings and literal strings, returning the byte length tracked by the string buffer; compile-time known string-list input returns a numeric list; `--chars` and `--grapheme-clusters` are supported for compile-time known scalar string or string-list input |
