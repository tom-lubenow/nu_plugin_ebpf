...
| `str contains` | Tracked strings with a literal substring, lowered as bounded fixed-offset stack-buffer comparisons guarded by the tracked runtime length; `--ignore-case` is supported for compile-time known input strings, and compile-time known string-list input returns a bool list |
