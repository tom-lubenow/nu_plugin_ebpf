...
Local-storage helpers use the ordinary modeled `helper-call` escape hatch, but the map kind is fixed by the helper. Passing a literal map name as arg0 to `bpf_sk_storage_{get,delete}`, `bpf_inode_storage_{get,delete}`, or `bpf_task_storage_{get,delete}` materializes a `sk_storage`, `inode_storage`, or `task_storage` map FD respectively; no `--kind` argument is required or accepted for these helpers.
