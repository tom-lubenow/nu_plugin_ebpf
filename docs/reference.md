...
| `first` / `last` | Scalar first/last element access for stack-backed numeric lists, typed fixed arrays, and compile-time known fixed lists, including metadata-only float scalars when folded by metadata consumers; counted forms rebuild bounded prefix/suffix stack-list slices for stack-backed numeric lists and constant-fold compile-time known fixed lists, including metadata-only float-list outputs when folded by metadata consumers |
