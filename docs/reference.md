...
| `bits not --signed` | Integer scalar input and integer stack-backed or compile-time known lists as signed two's-complement bit negation; default width-masked `bits not`, `--number-bytes`, and binary input are not supported |
