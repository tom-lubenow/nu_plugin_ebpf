...
| `bits rol --signed --number-bytes 8` / `bits ror --signed --number-bytes 8` | Integer scalar input and integer stack-backed or compile-time known lists with one compile-time rotate count from `0` through `64`; default byte-width-masked rotates, binary input, and runtime rotate counts are not supported |
