...
| `map-define` | Declare named generic map key/value schemas with `--key-type` and `--value-type`, including aggregate keys and fixed-layout values with verifier-managed fields such as `bpf_timer` and `bpf_spin_lock` |
