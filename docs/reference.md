...
| `is-empty` / `is-not-empty` | Stack-backed lists, tracked strings, literal binary, literal null, compile-time known bool/int/float/filesize/duration scalars, literal list constants and fixed-layout list builders, and metadata-backed fixed records |
...
| `bytes length` | Compile-time known binary input returning its byte length, typed fixed-size binary globals/fields declared with `bytes:N` or `binary:N` returning their declared byte length, or compile-time known list<binary> input returning a numeric list of byte lengths |
