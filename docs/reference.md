...
| `find` | Stack-backed numeric lists and typed fixed arrays with signed integer or <=32-bit unsigned integer elements and one numeric search argument, returning a bounded equality-filtered stack list; compile-time known fixed lists with one compile-time constant search argument, including metadata-only float-list outputs when folded by metadata consumers |
