...
`FMT_SIZE` and `LEN` must fit `0..=u32::MAX`; `DATA_LEN` must be a multiple of
8 capped to `MAX_BPRINTF_VARARGS * 8` (`96` bytes). `bpf_seq_printf` may use
