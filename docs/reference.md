...
| `bytes reverse` | Compile-time known binary input returning a reversed binary value, or compile-time known list<binary> input returning a materialized list of reversed binary values when the result has a non-empty equal-length fixed layout |
