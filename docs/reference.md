...
| `bits shl --signed --number-bytes 8` / `bits shr --signed --number-bytes 8` | Integer scalar input and integer stack-backed or compile-time known lists with one compile-time shift count from `0` through `63`; default byte-width-masked shifts, binary input, runtime shift counts, and rotates are not supported |
