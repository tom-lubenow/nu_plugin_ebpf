...
| `describe` | No-input, compile-time known values including metadata-only float/list values produced by folded commands, tracked runtime scalars, tracked strings, tracked records, and stack-backed numeric lists, producing a bounded tracked string with Nushell's type description |
