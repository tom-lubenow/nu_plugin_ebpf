...
| `str starts-with` | Tracked strings with a compile-time literal prefix, lowered as a bounded stack-buffer prefix comparison guarded by the tracked runtime length; `--ignore-case` is supported for compile-time known input strings, and compile-time known string-list input returns a bool list |
