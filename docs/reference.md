...
`cgroup/*_unix` sections for compile/dry-run and live attach through the
libbpf cgroup attach path. Their direct read
...
`raw_tracepoint.w` / `raw_tp.w` emits writable raw tracepoint targets such as `raw_tracepoint.w:sys_enter` as `raw_tracepoint.w/<name>` sections and reuses the ordinary raw tracepoint positional argument surface (`ctx.arg0`, `ctx.arg1`, ...). Live attach uses the libbpf-backed raw tracepoint path so writable raw-tracepoint verifier semantics are preserved.

`flow_dissector` targets a network-namespace path such as `flow_dissector:/proc/self/ns/net`. It emits a `flow_dissector` section and exposes the kernel's narrow `__sk_buff` flow-dissector surface: `ctx.data`, `ctx.data_end`, and `ctx.flow_keys` projections including `nhoff`, `thoff`, `addr_proto`, `is_frag`, `is_first_frag`, `is_encap`, `ip_proto`, `n_proto`, `sport`, `dport`, `ipv4_src`, `ipv4_dst`, fixed-array `ipv6_src` / `ipv6_dst` words, `flags`, and `flow_label`. Common aliases are also accepted for flow-key projections, such as `protocol` for `ip_proto`, `src_port` / `dst_port` for `sport` / `dport`, `src_ip4` / `dst_ip4` for IPv4 addresses, `src_ip6` / `dst_ip6` for IPv6 arrays, and `network_header_offset` / `transport_header_offset` for `nhoff` / `thoff`. Scalar `ctx.flow_keys` leaves are writable through ordinary assignment after shadowing the closure parameter or binding the pointer, for example `mut ctx = $ctx; $ctx.flow_keys.protocol = 6`, `mut keys = $ctx.flow_keys; $keys.dst_ip6.3 = 1`, or `mut keys = ($ctx | get flow_keys); $keys.dst_ip6.3 = 1`. User functions can return the same `get`-derived root before assignment, for example `def get_keys [event] { $event | get flow_keys }; mut keys = (get_keys $ctx); $keys.ip_proto = 17`. Length and protocol decisions should come from guarded packet reads or the dissected flow keys rather than direct `ctx.packet_len` / `ctx.protocol` fields. Return aliases are `"ok"` / `"parsed"` for `0`, `"drop"` for `2`, and `"continue"` / `"fallback"` for `129`. Live attach uses the libbpf network-namespace attach path and requires a valid netns file descriptor such as `/proc/self/ns/net`.

`netfilter` targets hooks such as `netfilter:ipv4:pre_routing[:priority=N][:defrag]`. It emits a `netfilter` section and exposes the safe scalar `bpf_nf_ctx.state` fields `ctx.hook` and `ctx.pf` / `ctx.protocol_family`, plus the verifier-provided trusted pointers `ctx.state` / `ctx.nf_state` (`nf_hook_state *`) and `ctx.skb` (`sk_buff *`) for ordinary typed projections such as `ctx.state.in.ifindex` or `ctx.skb.len`. Pointer-valued hops and scalar leaves from those trusted netfilter roots lower as direct trusted-BTF loads, preserving verifier provenance and avoiding probe-read helper calls. BPF-link specs accept `ipv4` / `ipv6` families and `pre_routing`, `local_in`, `forward`, `local_out`, or `post_routing` hooks; `defrag` requires priority greater than `-400` and Linux 6.6+. Return aliases are `"drop"` / `"deny"` for `0`, `"accept"` / `"allow"` / `"pass"` / `"ok"` for `1`, `"stolen"` for `2`, `"queue"` for `3`, and `"repeat"` for `4`. Live attach uses the libbpf BPF-link netfilter path and requires kernel/libbpf support for netfilter links.
...
`fmod_ret/FUNC` or `fmod_ret.s/FUNC` sections and live attach uses the
libbpf tracing path for modify-return programs. Direct
...
by itself. Aggregate-returning struct_ops callbacks are rejected up front
because kernel BPF trampoline function models reject struct return types for
those callbacks.
...
Stack-backed fixed arrays support the same runtime indexing, for example `let idx = ($ctx.pid mod 2); ($ctx.arg0.comm | get $idx)`. Bounded `for` loops over static integer ranges also lower to verifier-safe loops, so `for i in 0..0 { ... get $i ... }` now works, explicit negative-step descending ranges lower too, and bounded arithmetic on those indices such as `let j = (($i + 1) mod 2)` is preserved. The same range tracking now works for typed unsigned runtime fields such as `let idx = ($ctx.arg0.fdt.max_fds mod 2)`. Branch-sensitive narrowing also works for both bound and repeated direct paths, for example `let max = $ctx.arg0.fdt.max_fds; if $max > 0 { let idx = ($max - 1); ... }` or `if $ctx.arg0.fdt.max_fds > 0 { let idx = ($ctx.arg0.fdt.max_fds - 1); ... }`. Typed fixed arrays preserve layout through default no-flag `compact`; `compact --empty` is identity for numeric/bool arrays, is shape-preserving for fixed binary arrays with positive element length, can fold compile-time known fixed arrays before MIR, and can feed runtime shape-only consumers such as `length` / `is-empty` or type-only `describe` for string or numeric-list fixed arrays, including trusted BTF kernel fixed arrays. Materializing a dynamically shrunken non-numeric fixed-array result remains unsupported. Typed BTF bitfields can also be projected through the same paths, including after numeric `get`, for example `let idx = ($ctx.pid mod 2); let clamp = ($ctx.arg0.uclamp_req | get $idx); $clamp.value`.
...
| `reverse` | Stack-backed numeric lists and typed fixed arrays, preserving runtime/fixed length with descending constant-index loads or fixed-layout copies; compile-time known fixed lists are reversed as constants, including metadata-only float-list outputs when folded by metadata consumers |
| `uniq` | Stack-backed numeric lists and typed fixed arrays with signed integer, bool, or <=32-bit unsigned integer elements, preserving first occurrences through verifier-bounded duplicate checks; compile-time known fixed lists are deduplicated as constants, including metadata-only float-list outputs when folded by metadata consumers |
| `sort` | Stack-backed numeric lists and typed integer or bool fixed arrays with capacity/length <= 16, using bounded compare/swap lowering; compile-time known fixed lists and fixed arrays with boolean, integer, finite float, binary, or string elements are sorted as constants, including metadata-only float-list outputs when folded by metadata consumers; `--reverse` is supported, `--natural` and `--ignore-case` are supported for compile-time numeric or string values, `--values` is accepted for list inputs as ordinary list sorting, and `--values` sorts compile-time record fields by comparable boolean, integer, finite float, binary, or string values |
| `compact` | Stack-backed numeric lists and typed numeric or bool fixed arrays as identity operations, including `--empty`, because those elements cannot be null or empty; other annotated fixed-layout typed arrays support default no-flag `compact` as a non-null identity operation; fixed binary arrays with positive element length preserve shape for `compact --empty`, and runtime string/numeric-list fixed arrays, including trusted BTF kernel fixed arrays, can feed shape-only consumers such as `length` / `is-empty` by counting non-empty element slots, but materialized dynamically shrunken non-numeric fixed-array results are still rejected; compile-time known fixed lists are filtered as constants, with `--empty` also removing known empty strings/binaries/lists/records; metadata-only float-list outputs are supported when folded by metadata consumers; top-level column arguments are supported for compile-time known fixed lists of records |
| `find` | Stack-backed numeric lists and typed fixed arrays with signed integer, bool, or <=32-bit unsigned integer elements and one integer/bool search argument, returning a bounded equality-filtered stack list; compile-time known fixed lists with one compile-time constant search argument, including metadata-only float-list outputs when folded by metadata consumers; compile-time known string lists with tracked runtime string needles can feed shape-only consumers such as `length` / `is-empty` |
| `append` / `prepend` | Stack-backed numeric lists with scalar numeric items when the output fits the modeled capacity, typed fixed arrays with compatible scalar or fixed-layout aggregate items, and compile-time known fixed lists with compile-time constant items, including metadata-only float-list outputs when folded by metadata consumers |
| `char` | Compile-time named characters using the first name argument and ignoring extra string rest arguments, `--unicode` hexadecimal string codepoints, and `--integer` codepoints, returning a fixed string, including NUL-byte output; `--list` produces the named-character table as a compile-time record list for metadata-only consumers such as `length` and constant-index `get`; pipeline input and other flags are not supported |
| `seq` / `seq char` / `seq date` | Compile-time known integer one-, two-, or three-argument `seq` forms, including descending and zero-step ranges, producing a stack-backed numeric list up to 60 items when materialized or constant metadata when folded by metadata-only consumers; compile-time known float or mixed integer/float `seq` forms up to 60 items are supported as constant metadata when folded by metadata consumers, including list-transform chains and metadata-only `math mode`; compile-time known two-argument ASCII `seq char` forms produce a fixed string list up to 60 items; deterministic `seq date` ranges with explicit `--begin-date` plus `--end-date`, positive integer `--days`, or positive integer `--periods`, optional compile-time `--input-format` / `--output-format` including NUL bytes, optional non-zero integer day-count or duration `--increment` (negative values use their absolute duration), and optional `--reverse` produce fixed string lists up to 60 items, with `--periods` then `--days` taking precedence over `--end-date`; default-today date ranges, zero date increments, pipeline input, extra arguments, materialized float output, and longer outputs are not supported |
| `fill` | Compile-time known string, int, float, filesize, or bool scalar input or list input with optional compile-time `--width`, `--alignment`, and `--character` including NUL bytes, returning padded fixed strings; runtime unsigned integer input and runtime signed integer input proven non-negative are supported for verifier-bounded left, right, center, and center-right padding when the formatted output fits the tracked string buffer and required decimal thresholds are representable, with runtime integer trailing padding requiring a one-byte fill value while right-aligned integer padding may use longer values; runtime bool input supports left, right, center, and center-right padding when the worst-case `true` / `false` output fits the tracked string buffer; tracked runtime string input is supported only when padding is impossible (`--width 0` or `--character ""`) or when producer metadata proves the input is already at least the requested character width. Other runtime input, runtime options, null input, and closure-derived options are not supported |
| `is-empty` / `is-not-empty` | Stack-backed lists, typed fixed arrays, tracked strings, literal binary, typed fixed-size binary globals/fields, literal null, compile-time known bool/int/float/filesize/duration scalars, literal list constants and fixed-layout list builders, and metadata-backed fixed records |
| `describe` | No-input, compile-time known values including metadata-only float/list values produced by folded commands or fixed-layout list builders, tracked runtime scalars, tracked strings, tracked records, metadata-backed fixed binary values and fixed arrays with known element semantics, and stack-backed numeric lists, producing a bounded tracked string with Nushell's type description |
| `bytes length` | Compile-time known binary input returning its byte length, typed fixed-size binary globals/fields declared with `bytes:N` or `binary:N` returning their declared byte length, typed fixed arrays of fixed-size binary buffers returning numeric lists of declared byte lengths, or compile-time known list<binary> input returning a numeric list of byte lengths |
| `bytes starts-with` / `bytes ends-with` | Compile-time known binary or list<binary> input and binary pattern, returning a constant boolean or boolean list; typed fixed-size binary globals/fields and scalar typed fixed-array binary item projections declared with `bytes:N` or `binary:N` support compile-time binary patterns through bounded runtime byte comparisons; typed fixed arrays declared as `array{bytes:N:M}` or `array{binary:N:M}` return stack bool-lists through bounded per-element byte comparisons |
| `bytes index-of` | Compile-time known binary or list<binary> input and non-empty binary pattern, returning the first byte index or `-1`, or a numeric list of first indexes for list input; `--end` returns the last index, and scalar `--all` returns a stack-backed numeric list of up to 60 non-overlapping match offsets, in reverse order when combined with `--end`; typed fixed-size binary globals/fields and scalar typed fixed-array binary item projections declared with `bytes:N` or `binary:N` support first/last and `--all` index lookup with compile-time binary patterns through bounded runtime byte comparisons, when the candidate-offset probe count fits the compiler's fixed-comparison cap; typed fixed arrays declared as `array{bytes:N:M}` or `array{binary:N:M}` support first/last lookup and return stack numeric lists |
| `bytes reverse` | Compile-time known binary or list<binary> input; scalar input returns a reversed fixed binary value, while materialized list output requires non-empty equal-length binary values and empty or unequal list outputs are supported when folded by `bytes collect`, scalar `first` / `last`, constant-index `get`, `length`, `describe`, or empty predicates; typed fixed-size binary globals/fields and scalar typed fixed-array binary item projections declared with `bytes:N` or `binary:N` support runtime reversal into stack-backed fixed binary values; typed fixed arrays declared as `array{bytes:N:M}` or `array{binary:N:M}` return typed fixed arrays with each element reversed |
| `bytes build` | Compile-time known binary fragments and integer byte arguments, returning a binary value |
| `bytes at` | Compile-time known binary or list<binary> input and compile-time range using Nushell start/end bounds; scalar input returns a fixed binary slice, while materialized list output requires non-empty equal-length binary slices and empty or unequal list slices are supported when folded by `bytes collect`, scalar `first` / `last`, constant-index `get`, `length`, `describe`, or empty predicates; typed fixed-size binary globals/fields and scalar typed fixed-array binary item projections declared with `bytes:N` or `binary:N` support compile-time ranges by copying the selected runtime bytes into a stack-backed fixed binary slice, with empty slices folded as metadata-only empty binary values; typed fixed arrays declared as `array{bytes:N:M}` or `array{binary:N:M}` return typed fixed arrays with each element sliced to the selected fixed length |
| `bytes add` | Compile-time known binary or list<binary> input/data with optional non-negative `--index` and `--end`; scalar input returns a fixed binary value, while materialized list output requires non-empty equal-length binary values and empty or unequal list outputs are supported when folded by `bytes collect`, scalar `first` / `last`, constant-index `get`, `length`, `describe`, or empty predicates; typed fixed-size binary globals/fields and scalar typed fixed-array binary item projections declared with `bytes:N` or `binary:N` support compile-time binary data insertion into stack-backed fixed binary values; typed fixed arrays declared as `array{bytes:N:M}` or `array{binary:N:M}` return typed fixed arrays with the data inserted into each element |
| `bytes remove` | Compile-time known binary or list<binary> input and non-empty binary pattern with optional `--all` and `--end`; scalar input returns a fixed binary value, while materialized list output requires non-empty equal-length binary values and empty or unequal list outputs are supported when folded by `bytes collect`, scalar `first` / `last`, constant-index `get`, `length`, `describe`, or empty predicates; typed fixed-size binary globals/fields and scalar typed fixed-array binary item projections declared with `bytes:N` or `binary:N` support runtime no-op removal when the compile-time pattern is longer than the declared input length; typed fixed arrays declared as `array{bytes:N:M}` or `array{binary:N:M}` support no-op removal when the pattern is longer than each fixed-size element |
| `bytes replace` | Compile-time known binary or list<binary> input, non-empty binary pattern, and binary replacement with optional `--all`; scalar input returns a fixed binary value, while materialized list output requires non-empty equal-length binary values and empty or unequal list outputs are supported when folded by `bytes collect`, scalar `first` / `last`, constant-index `get`, `length`, `describe`, or empty predicates; typed fixed-size binary globals/fields and scalar typed fixed-array binary item projections declared with `bytes:N` or `binary:N` support runtime replacement when the compile-time replacement length equals the pattern length; typed fixed arrays declared as `array{bytes:N:M}` or `array{binary:N:M}` return typed fixed arrays after applying equal-length replacement within each element |
| `bytes collect` | Compile-time known list of binary values and optional binary separator, returning a fixed binary value; typed fixed arrays of fixed-size binary buffers declared with `array{bytes:N:M}` or `array{binary:N:M}` support runtime concatenation with an optional compile-time binary separator into a stack-backed fixed binary value |
| `bytes split` | Compile-time known binary input and non-empty binary/string separator; materialized lists require non-empty equal-length binary parts, while empty or unequal parts are supported when folded by `bytes collect`, scalar `first` / `last`, constant-index `get`, `length`, `describe`, or empty predicates; typed fixed-size binary globals/fields and scalar typed fixed-array binary item projections declared with `bytes:N` or `binary:N` support runtime no-match splitting when the compile-time separator is longer than the declared input length, producing a one-item stack-backed fixed array |
| `str length` | Tracked strings and literal strings, returning the byte length tracked by the string buffer; tracked runtime strings also support `--chars` by counting UTF-8 codepoint starts in the bounded string slot; typed fixed `array{string:N:M}` input returns a stack numeric list of byte lengths and also supports `--chars`; compile-time known string-list input returns a numeric list; `--grapheme-clusters` and `--unicode-width` remain compile-time known only |
| `str starts-with` | Tracked strings with a compile-time literal prefix, including NUL bytes, lowered as a bounded stack-buffer prefix comparison guarded by the tracked runtime length; typed fixed `array{string:N:M}` input with a literal prefix including NUL bytes returns a stack numeric bool-list; `--ignore-case` is supported for compile-time known input strings, and compile-time known string-list input returns a bool list |
| `str ends-with` | Tracked strings with a literal suffix including NUL bytes, lowered as bounded fixed-offset stack-buffer comparisons over the tracked runtime length; typed fixed `array{string:N:M}` input with a literal suffix including NUL bytes returns a stack numeric bool-list; `--ignore-case` is supported for compile-time known input strings, and compile-time known string-list input returns a bool list |
| `str contains` | Tracked strings with a literal substring including NUL bytes, lowered as bounded fixed-offset stack-buffer comparisons guarded by the tracked runtime length; typed fixed `array{string:N:M}` input with a literal substring including NUL bytes returns a stack numeric bool-list; `--ignore-case` is supported for compile-time known input strings, and compile-time known string-list input returns a bool list |
...
| `str index-of` | Tracked strings with a literal substring including NUL bytes in byte mode, returning the first byte index or `-1` through bounded fixed-offset comparisons guarded by the tracked runtime length; typed fixed `array{string:N:M}` input with a literal substring including NUL bytes returns a stack numeric list of byte indexes; `--end`, byte `--range` bounds including negative runtime bounds, and empty-substring range results are supported for runtime strings and typed fixed string arrays, while `--grapheme-clusters` requires compile-time known input strings; compile-time known string-list input returns a numeric list |
| `str join` | Compile-time known string input as a pass-through, compile-time known string/int/float/filesize/duration/binary/bool/null/list/record-list input with an optional literal separator including NUL bytes, or typed fixed `array{string:N:M}` input with elements up to 127 bytes and an optional literal separator including NUL bytes when the joined output fits the tracked string limit, returning a tracked string |
| `split chars` | Compile-time known scalar string input, returning a fixed string list of code points by default or grapheme clusters with `--grapheme-clusters`; compile-time known string-list input is supported as nested constant metadata when folded by `str join`, `length`, `describe`, or empty predicates |
| `split list` | Compile-time known fixed-list input with a compile-time known literal separator, including metadata-only nested scalar-list outputs when folded by metadata consumers such as `describe`, `str join`, `length`, `get`, scalar `first` / `last`, or empty predicates, or `--regex` string separator matching string/int/bool item text while preserving null/filesize/duration items as non-matches, and optional compile-time `--split on`/`before`/`after`, returning homogeneous fixed-layout groups when materialized; closure separators, runtime input lists, materialized heterogeneous result group layouts, and `--regex` over binary/record/list/float items are not supported |
...
| `str substring` | Compile-time known string or string-list input with one compile-time known explicit range argument using Nushell start/end bounds, with default/`--utf-8-bytes` byte indexes or `--grapheme-clusters`; typed fixed `array{string:N:M}` input supports compile-time known non-negative byte ranges and returns a typed fixed string array |
| `str replace` | Compile-time known string or string-list input with literal find/replacement arguments in the default first-substring replacement mode, `--all` substring replacement mode, or compile-time `--regex`/`--multiline` modes; `--no-expand` is supported for regex replacements that should keep replacement text literal; typed fixed `array{string:N:M}` input supports literal non-regex replacement, including NUL bytes, when find/replacement byte lengths match, including `--all`, and returns a typed fixed string array |
| `str trim` | Compile-time known string or string-list input in default, `--left`, `--right`, and single-character `--char` trim modes; tracked runtime strings and typed fixed `array{string:N:M}` input support byte-oriented default, `--left`, and `--right` trimming for ASCII whitespace (`0x09..=0x0d` and space), plus `--char`, `--left --char`, and `--right --char` when `--char` is one byte, including NUL; typed fixed string arrays return typed fixed string arrays |
...
| `length` | Stack-backed numeric lists, compile-time known list constants and fixed-layout list builders, typed fixed arrays, literal binary, typed fixed-size binary globals/fields, and literal null values |
| `bits and` / `bits or` / `bits xor` | Integer scalar input and integer stack-backed, typed numeric fixed-array, or compile-time known lists with one integer target argument, plus compile-time known binary scalar input and fixed-layout binary lists with one compile-time binary target argument and optional `--endian native|little|big`; binary-list materialization requires non-empty equal-length outputs, while empty or unequal outputs can be folded by metadata-only consumers such as `bytes collect`, `length`, `describe`, empty predicates, scalar `first` / `last`, and constant-index `get`; runtime binary input is not supported |
| `bits not` / `bits not --number-bytes 1|2|4|8` / `bits not --signed` | Integer scalar input and integer stack-backed, typed numeric fixed-array, or compile-time known lists; default auto-width matches Nushell's 1/2/4/8-byte selection, integer `--number-bytes` modes use Nushell-compatible sign-sensitive masking, and `--signed` returns signed two's-complement bit negation. Compile-time known binary scalar input and fixed-layout binary lists are supported with bytewise complement semantics, with `--signed` and `--number-bytes 1|2|4|8` accepted as no-ops for binary input; binary-list materialization requires non-empty equal-length outputs, while empty or unequal outputs can be folded by metadata-only consumers such as `describe`, `bytes collect`, `length`, empty predicates, scalar `first` / `last`, and constant-index `get`; runtime binary input is not supported |
| `bits shl` / `bits shr` | Integer scalar input and integer stack-backed, typed numeric fixed-array, or compile-time known lists with one compile-time shift count, plus signed 64-bit scalar shifts with a runtime count whose range is proven within `0..=63`. Explicit `--number-bytes 1|2|4` modes match Nushell's source-sign-sensitive truncation/sign-extension semantics, signed `--number-bytes 1|2|4` modes sign-extend the selected width, `--signed` with or without `--number-bytes 8` uses signed 64-bit shifts, and compile-time known default auto-width integer input selects Nushell's 1/2/4/8-byte width per value. Compile-time known unsigned `--number-bytes 8` integer input is supported when the result fits Nushell's integer range; runtime unsigned `bits shr --number-bytes 8` is supported with source-sign-sensitive arithmetic/logical right-shift behavior, and runtime unsigned `bits shl --number-bytes 8` is supported for `u8`, `u16`, and `u32` scalar inputs when the shift count cannot exceed Nushell's integer range (`0..=55`, `0..=47`, and `0..=31` respectively). Other runtime unsigned `bits shl --number-bytes 8` cases and runtime default auto-width input are not supported because Nu may report out-of-range results. Compile-time known binary scalar input and fixed-layout binary lists are supported with whole-buffer big-endian bitstring shift semantics, counts from `0` through the input bit length, and `--signed` / `--number-bytes 1|2|4|8` accepted as no-ops for binary input; binary-list materialization requires non-empty equal-length outputs, while empty or unequal outputs can be folded by metadata-only consumers such as `describe`, `bytes collect`, `length`, empty predicates, scalar `first` / `last`, and constant-index `get`. Runtime binary input is not supported |
| `bits rol` / `bits ror` | Integer scalar input and integer stack-backed, typed numeric fixed-array, or compile-time known lists with one compile-time rotate count, plus signed 64-bit scalar rotates with a runtime count whose range is proven within `0..=64`. Explicit `--number-bytes 1|2|4` modes rotate within the selected width and match Nushell's source-sign-sensitive integer result, signed `--number-bytes 1|2|4` modes sign-extend the selected width, `--signed` with or without `--number-bytes 8` uses signed 64-bit rotates, and compile-time known default auto-width integer input selects Nushell's 1/2/4/8-byte width per value. Compile-time known unsigned `--number-bytes 8` integer input is supported when the result fits Nushell's integer range; runtime unsigned `bits rol --number-bytes 8` is supported for `u8`, `u16`, and `u32` scalar inputs when the rotate count cannot exceed Nushell's integer range (`0..=55`, `0..=47`, and `0..=31` respectively), plus rotate count `64` as identity. Other runtime unsigned 8-byte rotates and runtime default auto-width input are not supported because Nu may report out-of-range results. Compile-time known binary scalar input and fixed-layout binary lists are supported with whole-buffer big-endian bitstring rotate semantics, counts from `0` through the input bit length, and `--signed` / `--number-bytes 1|2|4|8` accepted as no-ops for binary input; binary-list materialization requires non-empty equal-length outputs, while empty or unequal outputs can be folded by metadata-only consumers such as `describe`, `bytes collect`, `length`, empty predicates, scalar `first` / `last`, and constant-index `get`. Runtime binary input is not supported |
| `math avg` | Compile-time known integer/float lists when the float result is folded by metadata-only consumers such as `fill` or `describe`, and compile-time known homogeneous filesize/duration lists materialized as the corresponding unit type. Empty lists, mixed unit/numeric lists, runtime lists, and materialized numeric float averages are not supported |
| `math arccos` / `math arcsin` / `math arctan` | Compile-time known integer/float scalars and lists in default radians mode, or with optional `--degrees`/`-d` output conversion, when the finite float or list<float> result is folded by metadata-only consumers such as `fill`, `describe`, or `str join`. `math arccos` and `math arcsin` require input in `[-1, 1]`. Runtime input, non-finite float input, non-finite results, and materialized float/list<float> results are not supported |
| `math arccosh` / `math arcsinh` / `math arctanh` | Compile-time known integer/float scalars and lists when the finite float or list<float> result is folded by metadata-only consumers such as `fill`, `describe`, or `str join`. `math arccosh` requires input `>= 1`; `math arctanh` requires input in `(-1, 1)` so folded results stay finite. Runtime input, non-finite float input, non-finite results, and materialized float/list<float> results are not supported |
| `math cos` / `math sin` / `math tan` | Compile-time known integer/float scalars and lists in default radians mode, or with optional `--degrees`/`-d` input conversion, when the finite float or list<float> result is folded by metadata-only consumers such as `fill`, `describe`, or `str join`. Runtime input, non-finite float input, non-finite results, and materialized float/list<float> results are not supported |
| `math cosh` / `math sinh` / `math tanh` | Compile-time known integer/float scalars and lists when the finite float or list<float> result is folded by metadata-only consumers such as `fill`, `describe`, or `str join`. Runtime input, non-finite float input, non-finite results, and materialized float/list<float> results are not supported |
| `math exp` | Compile-time known integer/float scalars and lists when the finite float or list<float> result is folded by metadata-only consumers such as `fill`, `describe`, or `str join`. Runtime input, non-finite float input, non-finite results, and materialized float/list<float> results are not supported |
| `math ln` | Compile-time known positive integer/float scalars and lists when the finite float or list<float> result is folded by metadata-only consumers such as `fill`, `describe`, or `str join`. Runtime input, non-positive input, non-finite float input, non-finite results, and materialized float/list<float> results are not supported |
| `math log` | Compile-time known positive integer/float scalars and lists with one compile-time known positive base other than `1`, when the finite float or list<float> result is folded by metadata-only consumers such as `fill`, `describe`, or `str join`. Runtime input, non-positive input/base, base `1`, non-finite float input/base/results, and materialized float/list<float> results are not supported |
...
| `math variance` / `math stddev` | Compile-time known non-empty integer/float lists in default population mode or `--sample` mode when the finite float result is folded by metadata-only consumers such as `fill` or `describe`. Sample mode requires at least two items. Runtime input, table/record input, non-finite float input, non-numeric list items, non-finite results, and materialized float results are not supported |
| `math sum` / `math product` / `math min` / `math max` | Stack-backed numeric lists and typed fixed arrays with signed integer or <=32-bit unsigned integer elements and known or otherwise proven non-empty length, compile-time known filesize/duration lists for `math sum`, and compile-time known integer/filesize/duration lists for `math min` and `math max` preserving the selected value type, plus compile-time known integer/float lists for `math sum` and `math product` when their float result is folded by metadata-only consumers such as `fill` or `describe` and for `math min` and `math max` when the selected result is an integer or when a float result is folded by metadata-only consumers such as `fill` or `describe`; empty-list input is rejected to match Nushell semantics, `math product` does not support filesize/duration input, and materialized float reducer results are not supported |
| `math median` | Compile-time known odd-length integer lists, compile-time known integer/float lists whose median is an integer or whose float median is folded by metadata-only consumers such as `fill` or `describe`, compile-time known homogeneous filesize/duration lists including even-length unit medians, stack-backed numeric lists with known odd length up to 16 items, and typed fixed arrays with signed integer or <=32-bit unsigned integer elements and odd length up to 16 items, returning the integer or unit median; empty lists, unknown-length runtime lists, runtime even-length numeric lists, and materialized float median results are not supported |
| `math mode` | Compile-time known integer lists, including empty lists, returning a sorted stack-backed numeric list of modal values up to 60 items; compile-time known non-empty homogeneous finite float lists are supported when the list<float> result is folded by metadata-only consumers such as `fill`, `describe`, or `str join`; stack-backed runtime numeric lists and typed fixed arrays with signed integer or <=32-bit unsigned integer elements are supported when the modeled capacity is at most 16, with verifier-bounded sorting and duplicate counting. Mixed-type constant lists, non-finite float items, arguments, flags, materialized float-list results, and larger runtime capacities are not supported |
| `math abs` | Compiler-known integer scalar input, compile-time known integer-list input, stack-backed numeric lists, and typed fixed arrays with signed integer or <=32-bit unsigned integer elements, returning integer/list absolute values with Nushell's wrapping `i64::MIN` behavior; compile-time list output is limited to 60 values. Compile-time known finite float scalars and integer/finite-float lists are supported when float results are folded by metadata-only consumers such as `fill`, `describe`, or `str join`; materialized float results and non-finite float input are not supported |
| `math ceil` / `math floor` / `math round` | Integer scalar input and integer stack-backed lists or typed fixed arrays with signed integer or <=32-bit unsigned integer elements as identity operations, plus compile-time known integer/float scalar or list input returning integer results. `math round --precision <int>` / `-p <int>` supports compile-time known integer/float scalars and lists when the finite float or list<float> result is folded by metadata-only consumers such as `fill`, `describe`, or `str join`; materialized precision-mode float results and runtime float inputs are not supported in eBPF |
| `first` / `last` | Scalar first/last element access for stack-backed numeric lists, typed fixed arrays, and compile-time known fixed lists, including metadata-only float scalars when folded by metadata consumers; counted forms rebuild bounded prefix/suffix stack-list slices for stack-backed numeric lists, retag typed fixed-array prefix/suffix slices, and constant-fold compile-time known fixed lists, including metadata-only float-list outputs when folded by metadata consumers |
| `get` | Stack-backed numeric-list indexing, typed fixed-array indexing, constant-index item projection from compile-time known fixed lists, including metadata-only scalar projections when folded by metadata consumers, range-proven runtime indexing of compile-time known string lists by branch-selecting a tracked string result, top-level metadata-backed fixed-record field projection, typed context / BTF-backed pointer field projection with compiler-visible field paths such as `$ctx | get sk | get family`, and typed kernel/user pointer numeric indexing; list literal indexes may be literal cell paths |
...
| `values` | Metadata-backed fixed records and scalar-only typed records whose fields are integer-like scalar values, including bool/null as `1`/`0`, producing a stack-backed numeric list in record field order when materialized; metadata-backed integer/bool/null records and typed mixed-layout records support direct `values | first`, `values | last`, constant-index `values | get N`, and the same direct consumers after no-arg `reverse` or constant-count `take` / `first N` / `last N` / `skip` / `drop` transforms without materializing the full values list, with empty transformed lists returning `nothing` for scalar `first` / `last`; compile-time known homogeneous fixed-layout values produce a fixed list; compile-time known homogeneous finite-float record values can feed metadata-only `math mode`; compile-time known finite float or mixed scalar record values are otherwise supported as metadata-only lists when folded by metadata consumers or projected by scalar `first`/`last` or constant-index `get` |
| `transpose` | Compile-time known record values can materialize Nushell-style rows with default `column0` / `column1` names or the first two positional output column names; `--ignore-titles` produces one-column rows named `column0` or by the first positional output name. Homogeneous row layouts can materialize as fixed arrays. Metadata-backed runtime-valued records can feed shape-only consumers such as `length`, `is-empty`, `is-not-empty`, and `describe`, direct row projections with scalar `first` / `last` / constant-index `get`, and those same direct row projections after no-arg `reverse` or constant-count `take` / `first N` / `last N` / `skip` / `drop` transforms, with empty transformed row lists returning `nothing` for scalar `first` / `last`; `--as-record` supports shape consumers and field projection for metadata-backed runtime records. Full table materialization for mixed or runtime-valued row layouts, table input, and other transpose flags are not supported |
...
| `default` | Compiler-known null/empty scalar replacement, runtime non-null scalar passthrough for `--empty`, stack-backed numeric-list passthrough when length is proven positive, metadata-proven non-empty typed fixed-binary, fixed-array, and record passthrough for stack/context or compiler-managed global storage, metadata-backed record missing/null field replacement, and tracked runtime string branching for `--empty`; closure default values and runtime nullable-pointer defaults are rejected |
...
| `map-define` | Declare named generic map key/value schemas with `--key-type` and `--value-type`, optional capacity with `--max-entries`, map-in-map outers with `--inner-map`, or object-only arena maps with `--kind arena --max-entries PAGES [--map-extra ADDR]` |
...
`bpf_throw` is modeled as a void scalar-argument kfunc escape hatch; its return
value cannot be used, and it is rejected in lock-held regions just like other
non-allowlisted kfunc calls.
