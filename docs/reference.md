...
| `map-define` | Declare named generic map key/value schemas with `--key-type` and `--value-type`, plus optional capacity with `--max-entries`, including aggregate keys and fixed-layout values with verifier-managed fields such as `bpf_timer`, `bpf_spin_lock`, and `kptr:TYPE` |
