...
| `fill` | Compile-time known string/int scalar input or string/int list input with optional compile-time `--width`, `--alignment`, and `--character`, returning padded fixed strings; runtime input, filesize input, and closure-derived options are not supported |
