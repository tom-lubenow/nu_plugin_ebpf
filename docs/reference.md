...
Stack-backed fixed arrays support the same runtime indexing, for example `let idx = ($ctx.pid mod 2); ($ctx.arg0.comm | get $idx)`. Bounded `for` loops over static integer ranges also lower to verifier-safe loops, so `for i in 0..0 { ... get $i ... }` now works, explicit negative-step descending ranges lower too, and bounded arithmetic on those indices such as `let j = (($i + 1) mod 2)` is preserved. The same range tracking now works for typed unsigned runtime fields such as `let idx = ($ctx.arg0.fdt.max_fds mod 2)`. Branch-sensitive narrowing also works for both bound and repeated direct paths, for example `let max = $ctx.arg0.fdt.max_fds; if $max > 0 { let idx = ($max - 1); ... }` or `if $ctx.arg0.fdt.max_fds > 0 { let idx = ($ctx.arg0.fdt.max_fds - 1); ... }`. Typed fixed arrays preserve layout through default no-flag `compact`; `compact --empty` is identity for numeric/bool arrays and can fold compile-time known fixed arrays before MIR, but runtime non-numeric fixed arrays cannot shrink dynamically. Typed BTF bitfields can also be projected through the same paths, including after numeric `get`, for example `let idx = ($ctx.pid mod 2); let clamp = ($ctx.arg0.uclamp_req | get $idx); $clamp.value`.
...
| `fill` | Compile-time known string, int, float, or filesize scalar input or list input with optional compile-time `--width`, `--alignment`, and `--character`, returning padded fixed strings; runtime unsigned integer input is supported when the effective width is at most `1`; other runtime input, runtime padding, bool/null input, and closure-derived options are not supported |
...
| `bytes split` | Compile-time known binary input and non-empty binary/string separator; materialized lists require non-empty equal-length binary parts, while empty or unequal parts are supported when folded by `bytes collect`, scalar `first` / `last`, constant-index `get`, `length`, or empty predicates; typed fixed-size binary globals/fields and scalar typed fixed-array binary item projections declared with `bytes:N` or `binary:N` support runtime no-match splitting when the compile-time separator is longer than the declared input length, producing a one-item stack-backed fixed array |
| `str length` | Tracked strings and literal strings, returning the byte length tracked by the string buffer; typed fixed `array{string:N:M}` input returns a stack numeric list of byte lengths; compile-time known string-list input returns a numeric list; `--chars` and `--grapheme-clusters` are supported for compile-time known scalar string or string-list input |
| `str starts-with` | Tracked strings with a compile-time literal prefix, lowered as a bounded stack-buffer prefix comparison guarded by the tracked runtime length; typed fixed `array{string:N:M}` input with a literal prefix returns a stack numeric bool-list; `--ignore-case` is supported for compile-time known input strings, and compile-time known string-list input returns a bool list |
| `str ends-with` | Tracked strings with a literal suffix, lowered as bounded fixed-offset stack-buffer comparisons over the tracked runtime length; typed fixed `array{string:N:M}` input with a literal suffix returns a stack numeric bool-list; `--ignore-case` is supported for compile-time known input strings, and compile-time known string-list input returns a bool list |
| `str contains` | Tracked strings with a literal substring, lowered as bounded fixed-offset stack-buffer comparisons guarded by the tracked runtime length; typed fixed `array{string:N:M}` input with a literal substring returns a stack numeric bool-list; `--ignore-case` is supported for compile-time known input strings, and compile-time known string-list input returns a bool list |
...
| `math mode` | Compile-time known integer lists, including empty lists, returning a sorted stack-backed numeric list of modal values up to 60 items; stack-backed runtime numeric lists and typed fixed arrays with signed integer or <=32-bit unsigned integer elements are supported when the modeled capacity is at most 16, with verifier-bounded sorting and duplicate counting. Non-integer constant-list items, arguments, flags, and larger runtime capacities are not supported |
...
| `values` | Metadata-backed fixed records whose fields are numeric scalar values, including bool/null as `1`/`0`, producing a stack-backed numeric list in record field order; compile-time known homogeneous fixed-layout values produce a fixed list; compile-time known finite float or mixed scalar record values are supported as metadata-only lists when folded by metadata consumers or projected by scalar `first`/`last` |
