...
Generic map `--kind` now supports `hash`, `array`, `queue`, `stack`, `bloom-filter`, `lpm-trie`, `lru-hash`, `per-cpu-hash`, `per-cpu-array`, and `lru-per-cpu-hash`. `queue` and `stack` use `map-push`, `map-peek`, and `map-pop` instead of `map-put` / `map-get`. `bloom-filter` uses first-class `map-push` to insert values; membership probes are available through the raw `bpf_map_peek_elem` helper surface with a stack/map pointer value argument until a native contains-style surface exists. It does not support first-class `map-peek`, `map-pop`, `map-get`, `map-put`, or `map-delete`. `lpm-trie` uses the kernel's raw trie-key layout, so the key bytes must already begin with a `u32` prefix length followed by the trie payload.
