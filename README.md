...
- **Attach families**: kprobe/kretprobe/uprobe/uretprobe, fentry/fexit/tp_btf/raw_tracepoint/tracepoint/lsm (including sleepable `.s` BTF sections), perf_event, XDP/TC/socket_filter/cgroup and `sk_*` packet paths, compile-only `flow_dissector`, `netfilter`, and `sk_reuseport`, `lirc_mode2`, and `struct_ops`
...
- `flow_dissector`, `netfilter`, and `sk_reuseport` currently support compile/dry-run only; live attach intentionally returns an unsupported error until the loader has a safe attach implementation.
