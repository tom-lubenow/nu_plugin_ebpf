...
- Typed fixed arrays are also part of that ordinary aggregate surface when their element layout is honest: numeric arrays support the bounded list transforms/predicates, scalar and counted `first` / `last`, `get`, `length`, compatible `append` / `prepend`, non-empty math reducers, `math median` / `math mode` / `math abs`, integer identity `math ceil` / `math floor` / `math round`, and integer `bits and` / `bits or` / `bits xor` / `bits not` / `bits shl` / `bits shr` / `bits rol` / `bits ror`; bool arrays support `all` / `any` predicates and `compact --empty` as metadata-preserving identity, while other annotated fixed-layout arrays support default no-flag `compact`; fixed arrays of strings, binaries, numeric lists, and records support the subset of projections and metadata-preserving transforms documented in the reference.
