...
- Prefer ordinary Nushell expressions inside those bindings and typed `global-define --type` initializers. Arithmetic, cell-path projection, record/list construction, list spread, and string concatenation are intended surface forms when they stay compile-time constant and preserve an honest fixed layout.
