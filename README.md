...
- Typed record `mut` initializers may omit scalar or nested scalar-record fields that should start zeroed, as long as the annotation alone still fixes their honest layout. Typed list `mut` initializers may also seed fixed arrays from scalar/binary/record constants when the initializer provides the concrete length; record-array elements use the declared element layout, so zeroable fields may be omitted while fixed string or numeric-list fields still need concrete capacities.
...
- Use `map-define --key-type/--value-type` when a named map key or value needs an explicit fixed layout before ordinary map operations can infer it. This is the source-level path for aggregate keys and verifier-managed map fields such as `bpf_timer`, `bpf_spin_lock`, `bpf_wq`, `bpf_refcount`, `kptr:TYPE`, and graph roots like `bpf_list_head:node_data:node:record{refs:bpf_refcount,cookie:u64}`, for example `map-define timers --kind array --key-type u32 --value-type 'record{timer:bpf_timer,cookie:u64}' --max-entries 1024`. Bare list/rbtree graph root/node tokens are still rejected; roots must name the contained object type and node field so BTF can carry `contains:TYPE:FIELD` metadata, and optional payload records let graph object kfunc results expose fields such as `$node.cookie`. `--max-entries` sets positive map capacity for value-carrying map families that expose `max_entries`. Later map operations infer a unique prior kind by name from `map-define` or an earlier explicit `--kind` use, so non-hash maps do not need to repeat `--kind` at each use. Declared maps now emit object map definitions even when no later operation touches them. Map-in-map outer declarations emit `array-of-maps` / `hash-of-maps` object definitions with `--inner-map INNER` plus libbpf-compatible `values` BTF metadata when the inner template is also emitted; dry-run/object generation supports outer `map-get` / `map-contains` plus guarded dynamic inner `map-get` / `map-put` / `map-delete` / `map-contains` through the returned pointer. Live loading remains rejected because Aya does not materialize `inner_map_fd` from that metadata. Pinned peers attached with the same `--pin` group reuse those key/value schemas and capacity declarations when there is no conflict.
  Map-backed `bpf_wq` fields can be initialized, assigned a callback, and started with explicit `kfunc-call "bpf_wq_init"`, `kfunc-call "bpf_wq_set_callback_impl"`, and `kfunc-call "bpf_wq_start"` calls. The callback argument is an ordinary closure with `(bpf_map*, key*, bpf_wq*)` parameters; the verifier-rewritten `aux__ign` operand remains a raw ABI detail and must be written as `0`.
  A declared `bpf_refcount` field is BTF-visible map-value metadata; `bpf_refcount_acquire_impl` still requires a pointer to the containing refcounted object, so direct acquire from `$entry.refs` is intentionally rejected. Use the object pointer returned by graph object kfuncs such as `bpf_list_front`, `bpf_list_pop_front`, `bpf_rbtree_first`, or `bpf_rbtree_remove` when the root schema declares a refcounted payload.
...
- Treat `helper-call` and `kfunc-call` as escape hatches. Prefer typed context fields, ordinary Nushell control flow, and the smaller first-class command surface when it covers the operation. Known kfunc returns preserve modeled pointer types through locals, so null-checked results such as acquired task/file pointers can still use typed BTF-backed field projection. Raw kfunc operands that correspond to verifier-rewritten `__ign` metadata arguments must be written as known zero at source level. For helper calls with additional explicit arguments, write all helper ABI arguments explicitly rather than relying on the piped value as an implicit first argument.
- Bind by-reference stack objects to locals before passing them through multi-call helper/kfunc lifecycles. For example, use `let d = $rec.d` and then pass `$d` to dynptr reserve/read/release calls; repeated `$rec.d` projections are ordinary value projections and therefore do not represent the same mutable stack object.
