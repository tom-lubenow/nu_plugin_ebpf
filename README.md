...
- Use `map-define --key-type/--value-type` when a named map key or value needs an explicit fixed layout before ordinary map operations can infer it. This is the source-level path for aggregate keys and verifier-managed map fields such as `bpf_timer`, `bpf_spin_lock`, `bpf_wq`, `bpf_refcount`, `kptr:TYPE`, and graph roots like `bpf_list_head:node_data:node`, for example `map-define timers --kind array --key-type u32 --value-type 'record{timer:bpf_timer,cookie:u64}' --max-entries 1024`. Bare list/rbtree graph root/node tokens are still rejected; roots must name the contained object type and node field so BTF can carry `contains:TYPE:FIELD` metadata. `--max-entries` sets positive map capacity for value-carrying map families that expose `max_entries`. Pinned peers attached with the same `--pin` group reuse those key/value schemas and capacity declarations when there is no conflict.
  ...
  A declared `bpf_refcount` field is BTF-visible map-value metadata; `bpf_refcount_acquire_impl` still requires a pointer to the containing refcounted object, so direct acquire from `$entry.refs` is intentionally rejected until object payload schemas are modeled.
