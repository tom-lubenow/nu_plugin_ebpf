...
- Use `map-define --key-type/--value-type` when a named map key or value needs an explicit fixed layout before ordinary map operations can infer it. This is the source-level path for aggregate keys and verifier-managed map fields such as `bpf_timer`, for example `map-define timers --kind array --key-type u32 --value-type 'record{timer:bpf_timer,cookie:u64}'`.
