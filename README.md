...
- **Attach families**: kprobe/kretprobe/uprobe/uretprobe, fentry/fexit/tp_btf/raw_tracepoint/raw_tracepoint.w/tracepoint/lsm (including sleepable `.s` BTF sections), perf_event, XDP/TC/socket_filter/cgroup and `sk_*` packet paths, compile-only `flow_dissector`, `netfilter`, `lwt_*`, `tc_action`, `sk_reuseport`, and `freplace`/extension sections, `lirc_mode2`, and `struct_ops`
...
- `raw_tracepoint.w`, `flow_dissector`, `netfilter`, `lwt_*`, `tc_action`, `sk_reuseport`, and `freplace` currently support compile/dry-run only; live attach intentionally returns an unsupported error until the loader has a safe attach implementation.
