...
- Use `map-define --value-type` when a named map value needs an explicit fixed layout before any ordinary `map-put` can infer it. This is the source-level path for verifier-managed map fields such as `bpf_timer`, for example `map-define timers --kind array --value-type 'record{timer:bpf_timer,cookie:u64}'`.
