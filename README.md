...
- Treat `helper-call` and `kfunc-call` as escape hatches. Prefer typed context fields, ordinary Nushell control flow, and the smaller first-class command surface when it covers the operation. Raw kfunc operands that correspond to verifier-rewritten `__ign` metadata arguments must be written as known zero at source level.
