...
- Prefer ordinary aggregate commands when the compiler can keep the layout honest. Stack-backed numeric lists support `where`, `each`, `all`, `any`, `take`, `skip`, `drop`, `reverse`, `uniq`, small-list `sort` / `sort --reverse`, `compact`, `find`, `append`, `prepend`, `is-empty`, `is-not-empty`, `length`, known non-empty `math sum` / `math product` / `math min` / `math max`, `first`, `last`, and `get`; tracked strings support byte and compile-time known grapheme-cluster `str length`, literal-prefix `str starts-with`, known-length literal-suffix `str ends-with`, known-length literal-substring `str contains` / `str index-of`, compile-time known `--ignore-case` string predicates, compile-time known byte/grapheme `str substring`, compile-time known default/`--all` `str replace`, compile-time known `str trim` default/`--left`/`--right`/`--char`, compile-time known default `str downcase` / `str upcase` / `str reverse` / `str capitalize`, `is-empty`, `is-not-empty`, and selected `default` forms; metadata-backed fixed records support `get`, `select`, `reject`, `rename`, `merge`, integer-field `values`, `is-empty`, `is-not-empty`, `insert`, `update`, `upsert`, and selected `default` forms. These lower through the normal compiler model rather than helper wrappers.
