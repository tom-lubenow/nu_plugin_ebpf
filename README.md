...
- Prefer ordinary aggregate commands when the compiler can keep the layout honest. Stack-backed numeric lists support `where`, `each`, `take`, `skip`, `drop`, `reverse`, `append`, `prepend`, `is-empty`, `length`, `first`, `last`, and `get`; metadata-backed fixed records support `select`, `reject`, `insert`, `update`, `upsert`, and selected `default` forms. These lower through the normal compiler model rather than helper wrappers.
