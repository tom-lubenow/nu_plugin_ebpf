...
- Prefer ordinary aggregate commands when the compiler can keep the layout honest. Stack-backed numeric lists support `where`, `each`, `all`, `any`, `take`, `skip`, `drop`, `reverse`, `append`, `prepend`, `is-empty`, `is-not-empty`, `length`, known non-empty `math sum`, `first`, `last`, and `get`; metadata-backed fixed records support `select`, `reject`, `rename`, `merge`, integer-field `values`, `is-empty`, `is-not-empty`, `insert`, `update`, `upsert`, and selected `default` forms. These lower through the normal compiler model rather than helper wrappers.
