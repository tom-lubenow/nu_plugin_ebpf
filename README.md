...
- Prefer the first-class packet/socket/message/map surface (`adjust-packet`, `adjust-message`, `redirect`, `redirect-map`, `redirect-socket`, `tail-call`, `map-contains`, socket-map `map-put`, and local-storage `map-get` / `map-delete`) over raw `helper-call` forms when the operation matches.
