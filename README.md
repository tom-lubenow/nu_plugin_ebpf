...
- Use `map-define --key-type/--value-type` when a named map key or value needs an explicit fixed layout before ordinary map operations can infer it. This is the source-level path for aggregate keys and verifier-managed map fields such as `bpf_timer`, `bpf_spin_lock`, `bpf_wq`, `bpf_refcount`, and `kptr:TYPE`, for example `map-define timers --kind array --key-type u32 --value-type 'record{timer:bpf_timer,cookie:u64}' --max-entries 1024`. Bare list/rbtree graph fields are intentionally rejected until the source model can declare named object schemas and emit the required BTF `contains:TYPE:FIELD` metadata; their kernel floors are tracked for compatibility diagnostics. `--max-entries` sets positive map capacity for value-carrying map families that expose `max_entries`. Pinned peers attached with the same `--pin` group reuse those key/value schemas and capacity declarations when there is no conflict.
