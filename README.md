...
- Prefer `redirect-map` over `helper-call "bpf_redirect_map"` on XDP, and prefer `redirect-socket` over raw socket redirect helpers on `sk_msg` / `sk_skb` / `sk_skb_parser`.
