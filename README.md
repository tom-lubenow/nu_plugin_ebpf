...
- **Attach families**: kprobe/kretprobe/kprobe.multi/kretprobe.multi/ksyscall/kretsyscall/uprobe/uretprobe/uprobe.multi/uretprobe.multi, fentry/fexit/fmod_ret/tp_btf/raw_tracepoint/raw_tracepoint.w/tracepoint/lsm (including sleepable `.s` BTF and user-probe sections), perf_event, XDP/TC/socket_filter/cgroup and `sk_*` packet paths, compile-only `flow_dissector`, `netfilter`, `lwt_*`, `tc_action`, `tcx`, `sk_reuseport`, `lsm_cgroup`, cgroup socket-address UNIX, `freplace`/extension, and `syscall` sections, `lirc_mode2`, and `struct_ops`
...
- `raw_tracepoint.w`, `fmod_ret`, `kprobe.multi`, `kretprobe.multi`, `uprobe.multi`, `uretprobe.multi`, `ksyscall`, `kretsyscall`, `flow_dissector`, `netfilter`, `lwt_*`, `tc_action`, `tcx`, `sk_reuseport`, `lsm_cgroup`, `cgroup_sock_addr:*_unix`, `freplace`, and `syscall` currently support compile/dry-run only; live attach intentionally returns an unsupported error until the loader has a safe attach implementation.
